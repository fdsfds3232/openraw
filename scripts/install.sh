...
# Usage: curl -sSf https://openraw.sh | sh
#
# Environment variables:
#   OPENRAW_INSTALL_DIR  — custom install directory (default: ~/.openraw/bin)
#   OPENRAW_VERSION      — install a specific version tag (default: latest)

set -euo pipefail

REPO="RightNow-AI/openraw"
...
detect_platform() {
    ...
    case "$OS" in
        ...
        mingw*|msys*|cygwin*)
            ...
            echo "    irm https://openraw.sh/install.ps1 | iex"
            echo ""
            echo "  Or download the .msi installer from:"
            echo "    https://github.com/$REPO/releases/latest"
            echo ""
            echo "  Or install via cargo:"
            echo "    cargo install --git https://github.com/$REPO openraw-cli"
            exit 1
            ;;
...
install() {
    ...
    URL="https://github.com/$REPO/releases/download/$VERSION/openraw-$PLATFORM.tar.gz"
    CHECKSUM_URL="$URL.sha256"

    echo "  Installing OpenRaw $VERSION for $PLATFORM..."
    mkdir -p "$INSTALL_DIR"

    # Download to temp
    TMPDIR=$(mktemp -d)
    ARCHIVE="$TMPDIR/openraw.tar.gz"
    ...
    if [ -n "$SHELL_RC" ] && ! grep -q "openraw" "$SHELL_RC" 2>/dev/null; then
        case "${SHELL:-}" in
            */fish)
    ...
    # Verify installation
    if "$INSTALL_DIR/openraw" --version >/dev/null 2>&1; then
        INSTALLED_VERSION=$("$INSTALL_DIR/openraw" --version 2>/dev/null || echo "$VERSION")
        echo ""
        echo "  OpenRaw installed successfully! ($INSTALLED_VERSION)"
    else
        echo ""
        echo "  OpenRaw binary installed to $INSTALL_DIR/openraw"
    fi
