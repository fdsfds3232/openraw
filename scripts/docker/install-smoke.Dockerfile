...
#   docker build -f scripts/docker/install-smoke.Dockerfile \
#     --build-arg OPENRAW_SMOKE_FULL=1 .
...
ARG OPENRAW_SMOKE_FULL=0
RUN if [ "$OPENRAW_SMOKE_FULL" = "1" ]; then \
        bash /tmp/install.sh; \
    else \
        # 1. Syntax check
...
RUN if [ "$OPENRAW_SMOKE_FULL" = "1" ] && [ -f "$HOME/.openraw/bin/openraw" ]; then \
        $HOME/.openraw/bin/openraw --version && \
        echo "PASS: openraw binary works"; \
    else \
