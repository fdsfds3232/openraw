...
# Usage: iwr -useb https://openraw.sh/install.ps1 | iex
#   or:  powershell -c "irm https://openraw.sh/install.ps1 | iex"
#
...
function Write-Banner {
    Write-Host ""
    Write-Host "  OpenRaw Installer" -ForegroundColor Cyan
...
function Get-Architecture {
    ...
    switch ($archUpper) {
        ...
        default {
            ...
            Write-Host "  Try: cargo install --git https://github.com/RightNow-AI/openraw openraw-cli" -ForegroundColor Yellow
            exit 1
        }
    }
}
...
function Install-OpenRaw {
    ...
    if (Test-Path $installedExe) {
        try {
            ...
            Write-Host ""
            Write-Host "  OpenRaw installed successfully! ($versionOutput)" -ForegroundColor Green
        }
        catch {
            Write-Host ""
            Write-Host "  OpenRaw binary installed to $installedExe" -ForegroundColor Green
        }
    }
