//! Load & performance tests for the OpenRaw API.
//!
//! Measures throughput under concurrent access: agent spawning, API endpoint
//! latency, session management, and memory usage.
//!
//! Run: cargo test -p openraw-api --test load_test -- --nocapture

use axum::Router;
use openraw_api::middleware;
use openraw_api::routes::{self, AppState};
use openraw_kernel::OpenRawKernel;
use openraw_types::config::{DefaultModelConfig, KernelConfig};
use std::sync::Arc;
...
async fn start_test_server() -> TestServer {
    ...
    let kernel = OpenRawKernel::boot_with_config(config).expect("Kernel should boot");
...
async fn load_metrics_sustained() {
    ...
    for _ in 0..n {
        ...
        assert!(body.contains("openraw_agents_active"));
    }
