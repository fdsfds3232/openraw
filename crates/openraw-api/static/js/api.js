// OpenRaw API Client — Fetch wrapper, WebSocket manager, auth injection, toast notifications
'use strict';

// ── Toast Notification System ──
var OpenRawToast = (function() {
  var _container = null;
  var _toastId = 0;
...
function friendlyError(status, serverMsg) {
  if (status === 0 || !status) return 'Cannot reach daemon — is openraw running?';
  ...
  function request(method, path, body) {
    ...
    return fetch(BASE + path, opts).then(function(r) {
      ...
      if (e.name === 'TypeError' && e.message.includes('Failed to fetch')) {
        ...
        throw new Error('Cannot connect to daemon — is openraw running?');
      }
      throw e;
    });
  }
    ...
    try {
      ...
      _ws.onopen = function() {
        ...
        if (_reconnectAttempt > 0) {
          OpenRawToast.success('Reconnected');
          _reconnectAttempt = 0;
        }
      ...
      _ws.onclose = function(e) {
        ...
        if (_wsAgentId && _reconnectAttempts < MAX_RECONNECT && e.code !== 1000) {
          ...
          if (_reconnectAttempts === 1) {
            OpenRawToast.warn('Connection lost, reconnecting...');
          }
        ...
        if (_wsAgentId && _reconnectAttempts >= MAX_RECONNECT) {
          setConnectionState('disconnected');
          OpenRawToast.error('Connection lost — switched to HTTP mode', 0);
        }
