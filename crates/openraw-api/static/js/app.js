// OpenRaw App вЂ” Alpine.js init, hash router, global store
'use strict';
...
// Tool category icon SVGs вЂ” returns inline SVG for each tool category
...
document.addEventListener('alpine:init', function() {
  ...
  var savedKey = localStorage.getItem('openraw-api-key');
  if (savedKey) OpenRawAPI.setAuthToken(savedKey);
  ...
  Alpine.store('app', {
    ...
    agentCount: 0,
    pendingAgent: null,
    focusMode: localStorage.getItem('openraw-focus') === 'true',
    showOnboarding: false,
    showAuthPrompt: false,

    toggleFocusMode() {
      this.focusMode = !this.focusMode;
      localStorage.setItem('openraw-focus', this.focusMode);
    },

    async refreshAgents() {
      try {
        var agents = await OpenRawAPI.get('/api/agents');
    ...
    async checkStatus() {
      try {
        var s = await OpenRawAPI.get('/api/status');
        this.connected = true;
        this.booting = false;
        this.lastError = '';
        ...
        console.warn('[OpenRaw] Status check failed:', e.message);
      }
    },

    async checkOnboarding() {
      if (localStorage.getItem('openraw-onboarded')) return;
      try {
        var config = await OpenRawAPI.get('/api/config');
    ...
    async checkAuth() {
      try {
        ...
        await OpenRawAPI.get('/api/tools');
        ...
        if (e.message && (e.message.indexOf('Not authorized') >= 0 || e.message.indexOf('401') >= 0 || e.message.indexOf('Missing Authorization') >= 0 || e.message.indexOf('Unauthorized') >= 0)) {
          ...
          var saved = localStorage.getItem('openraw-api-key');
          if (saved) {
            // Saved key might be stale вЂ” clear it and show prompt
            OpenRawAPI.setAuthToken('');
            localStorage.removeItem('openraw-api-key');
          }
    ...
    submitApiKey(key) {
      ...
      OpenRawAPI.setAuthToken(key.trim());
      localStorage.setItem('openraw-api-key', key.trim());
      this.showAuthPrompt = false;
      this.refreshAgents();
    },
...
function app() {
  return {
    page: 'agents',
    themeMode: localStorage.getItem('openraw-theme-mode') || 'system',
    theme: (() => {
      var mode = localStorage.getItem('openraw-theme-mode') || 'system';
      if (mode === 'system') return window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
      return mode;
    })(),
    sidebarCollapsed: localStorage.getItem('openraw-sidebar') === 'collapsed',
    ...
    init() {
      ...
      document.addEventListener('keydown', function(e) {
        // Ctrl+K вЂ” focus agent switch / go to agents
        ...
        // Ctrl+Shift+F вЂ” toggle focus mode
        if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.key === 'F') {
          e.preventDefault();
          Alpine.store('app').toggleFocusMode();
        }
        // Escape вЂ” close mobile menu
        if (e.key === 'Escape') {
      ...
      OpenRawAPI.onConnectionChange(function(state) {
    ...
    setTheme(mode) {
      this.themeMode = mode;
      localStorage.setItem('openraw-theme-mode', mode);
      if (mode === 'system') {
    ...
    toggleSidebar() {
      ...
      localStorage.setItem('openraw-sidebar', this.sidebarCollapsed ? 'collapsed' : 'expanded');
    },
    ...
    async pollStatus() {
      ...
      this.wsConnected = OpenRawAPI.isWsConnected();
    }
  };
}
