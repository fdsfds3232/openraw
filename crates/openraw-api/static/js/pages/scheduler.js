// OpenRaw Scheduler Page вЂ” Cron job management + event triggers unified view
'use strict';
...
function schedulerPage() {
  return {
    ...
    async loadJobs() {
      var data = await OpenRawAPI.get('/api/cron/jobs');
      var raw = data.jobs || [];
    ...
    async loadTriggers() {
      ...
      this.trigLoadError = '';
      try {
        var data = await OpenRawAPI.get('/api/triggers');
    ...
    // в”Ђв”Ђ Job CRUD в”Ђв”Ђ

    async createJob() {
      if (!this.newJob.name.trim()) {
        OpenRawToast.warn('Please enter a job name');
        return;
      }
      if (!this.newJob.cron.trim()) {
        OpenRawToast.warn('Please enter a cron expression');
        return;
      }
      this.creating = true;
      try {
        ...
        await OpenRawAPI.post('/api/cron/jobs', body);
        this.showCreateForm = false;
        this.newJob = { name: '', cron: '', agent_id: '', message: '', enabled: true };
        OpenRawToast.success('Schedule "' + jobName + '" created');
        await this.loadJobs();
      } catch(e) {
        OpenRawToast.error('Failed to create schedule: ' + (e.message || e));
      }
      this.creating = false;
    },
    ...
    async toggleJob(job) {
      try {
        ...
        await OpenRawAPI.put('/api/cron/jobs/' + job.id + '/enable', { enabled: newState });
        job.enabled = newState;
        OpenRawToast.success('Schedule ' + (newState ? 'enabled' : 'paused'));
      } catch(e) {
        OpenRawToast.error('Failed to toggle schedule: ' + (e.message || e));
      }
    },
    ...
    deleteJob(job) {
      ...
      OpenRawToast.confirm('Delete Schedule', 'Delete "' + jobName + '"? This cannot be undone.', async function() {
        try {
          await OpenRawAPI.del('/api/cron/jobs/' + job.id);
          self.jobs = self.jobs.filter(function(j) { return j.id !== job.id; });
          OpenRawToast.success('Schedule "' + jobName + '" deleted');
        } catch(e) {
          OpenRawToast.error('Failed to delete schedule: ' + (e.message || e));
        }
      });
    },
    ...
    async runNow(job) {
      ...
      try {
        var result = await OpenRawAPI.post('/api/schedules/' + job.id + '/run', {});
        if (result.status === 'completed') {
          OpenRawToast.success('Schedule "' + (job.name || 'job') + '" executed successfully');
          job.last_run = new Date().toISOString();
        } else {
          OpenRawToast.error('Schedule run failed: ' + (result.error || 'Unknown error'));
        }
      } catch(e) {
        OpenRawToast.error('Run Now is not yet available for cron jobs');
      }
      this.runningJobId = '';
    },

    // в”Ђв”Ђ Trigger helpers в”Ђв”Ђ
    ...
    async toggleTrigger(trigger) {
      try {
        ...
        await OpenRawAPI.put('/api/triggers/' + trigger.id, { enabled: newState });
        trigger.enabled = newState;
        OpenRawToast.success('Trigger ' + (newState ? 'enabled' : 'disabled'));
      } catch(e) {
        OpenRawToast.error('Failed to toggle trigger: ' + (e.message || e));
      }
    },

    deleteTrigger(trigger) {
      var self = this;
      OpenRawToast.confirm('Delete Trigger', 'Delete this trigger? This cannot be undone.', async function() {
        try {
          await OpenRawAPI.del('/api/triggers/' + trigger.id);
          self.triggers = self.triggers.filter(function(t) { return t.id !== trigger.id; });
          OpenRawToast.success('Trigger deleted');
        } catch(e) {
          OpenRawToast.error('Failed to delete trigger: ' + (e.message || e));
        }
      });
    },
