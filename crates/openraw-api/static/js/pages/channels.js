// OpenRaw Channels Page вЂ” OpenClaw-style setup UX with QR code support
'use strict';
...
function channelsPage() {
  return {
    ...
    async loadChannels() {
      ...
      this.loadError = '';
      try {
        var data = await OpenRawAPI.get('/api/channels');
    ...
    // в”Ђв”Ђ QR Code Flow (WhatsApp Web style) в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
    ...
    async startQR() {
      ...
      try {
        var result = await OpenRawAPI.post('/api/channels/whatsapp/qr/start', {});
        ...
        if (this.qr.connected) {
          OpenRawToast.success('WhatsApp connected!');
    ...
    pollQR() {
      ...
      this.qrPollTimer = setInterval(async function() {
        try {
          var result = await OpenRawAPI.get('/api/channels/whatsapp/qr/status?session_id=' + encodeURIComponent(self.qr.sessionId));
          if (result.connected) {
            clearInterval(self.qrPollTimer);
            self.qrPollTimer = null;
            self.qr.connected = true;
            self.qr.message = result.message || 'Connected!';
            OpenRawToast.success('WhatsApp linked successfully!');
    ...
    // в”Ђв”Ђ Standard Form Flow в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
    ...
    async saveChannel() {
      ...
      try {
        await OpenRawAPI.post('/api/channels/' + name + '/configure', {
          fields: this.formValues
        });
        this.setupStep = 2;
        // Auto-test after save
        try {
          var testResult = await OpenRawAPI.post('/api/channels/' + name + '/test', {});
          if (testResult.status === 'ok') {
            this.testPassed = true;
            this.setupStep = 3;
            OpenRawToast.success(this.setupModal.display_name + ' activated!');
          } else {
            OpenRawToast.success(this.setupModal.display_name + ' saved. ' + (testResult.message || ''));
          }
        } catch(te) {
          OpenRawToast.success(this.setupModal.display_name + ' saved. Test to verify connection.');
        }
        await this.refreshStatus();
      } catch(e) {
        OpenRawToast.error('Failed: ' + (e.message || 'Unknown error'));
      }
    ...
    async removeChannel() {
      ...
      var self = this;
      OpenRawToast.confirm('Remove Channel', 'Remove ' + displayName + ' configuration? This will deactivate the channel.', async function() {
        try {
          await OpenRawAPI.delete('/api/channels/' + name + '/configure');
          OpenRawToast.success(displayName + ' removed and deactivated.');
          await self.refreshStatus();
    ...
    async testChannel() {
      ...
      try {
        var result = await OpenRawAPI.post('/api/channels/' + name + '/test', {});
        if (result.status === 'ok') {
          this.testPassed = true;
          this.setupStep = 3;
          OpenRawToast.success(result.message);
        } else {
          OpenRawToast.error(result.message);
        }
      } catch(e) {
        OpenRawToast.error('Test failed: ' + (e.message || 'Unknown error'));
      }
    ...
    async copyConfig(ch) {
      ...
      try {
        ...
        OpenRawToast.success('Copied to clipboard');
      } catch(e) {
        OpenRawToast.error('Copy failed');
      }
    },
