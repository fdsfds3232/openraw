// OpenRaw Analytics Page вЂ” Full usage analytics with per-model and per-agent breakdowns
...
function analyticsPage() {
  return {
    ...
    _chartColors: [
      '#1A1817', '#4A4644', '#6B6560', '#8A8380', '#9A958F',
    ...
    async loadSummary() {
      try {
        this.summary = await OpenRawAPI.get('/api/usage/summary');
      } catch(e) {
    ...
    async loadByModel() {
      try {
        var data = await OpenRawAPI.get('/api/usage/by-model');
    ...
    async loadDailyCosts() {
      try {
        var data = await OpenRawAPI.get('/api/usage/daily');
    ...
    // в”Ђв”Ђ Cost tab helpers в”Ђв”Ђ
    ...
    // в”Ђв”Ђ Provider aggregation from byModel data в”Ђв”Ђ
    ...
    // в”Ђв”Ђ Donut chart (stroke-dasharray on circles) в”Ђв”Ђ
    ...
    // в”Ђв”Ђ Bar chart (last 7 days) в”Ђв”Ђ
    ...
    // в”Ђв”Ђ Cost by model table (sorted by cost descending) в”Ђв”Ђ
