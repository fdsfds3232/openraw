// OpenRaw Setup Wizard вЂ” First-run guided setup (Provider + Agent + Channel)
'use strict';
...
function wizardPage() {
  return {
    ...
    async sendTryItMessage(text) {
      ...
      try {
        var res = await OpenRawAPI.post('/api/agents/' + this.createdAgent.id + '/message', { message: text });
    ...
    // в”Ђв”Ђ Step 2: Providers в”Ђв”Ђ
    ...
    async saveKey() {
      ...
      if (!key) {
        OpenRawToast.error('Please enter an API key');
        return;
      }
      this.savingKey = true;
      try {
        await OpenRawAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/key', { key: key });
        this.apiKeyInput = '';
        this.keySaved = true;
    ...
    // в”Ђв”Ђ Step 3: Agent creation в”Ђв”Ђ
    ...
    async createAgent() {
      ...
      if (!name) {
        OpenRawToast.error('Please enter a name for your agent');
        return;
      }
      ...
      try {
        ...
        if (res.agent_id) {
          ...
          OpenRawToast.success('Agent "' + (res.name || name) + '" created');
          await Alpine.store('app').refreshAgents();
        } else {
          OpenRawToast.error('Failed: ' + (res.error || 'Unknown error'));
        }
      } catch(e) {
        OpenRawToast.error('Failed to create agent: ' + e.message);
      }
    ...
    // в”Ђв”Ђ Step 5: Channel setup в”Ђв”Ђ
    ...
    async configureChannel() {
      ...
      if (!token) {
        OpenRawToast.error('Please enter the ' + ch.token_label);
        return;
      }
      ...
      try {
        ...
        fields.token = token;
        await OpenRawAPI.post('/api/channels/' + ch.name + '/configure', { fields: fields });
        this.channelConfigured = true;
        this.setupSummary.channel = ch.display_name;
        OpenRawToast.success(ch.display_name + ' configured and activated.');
      } catch(e) {
    ...
    // в”Ђв”Ђ Step 6: Finish в”Ђв”Ђ

    finish() {
      localStorage.setItem('openraw-onboarded', 'true');
