// OpenRaw Comms Page вЂ” Agent topology & inter-agent communication feed
'use strict';
...
function commsPage() {
  return {
    ...
    async loadData() {
      ...
      try {
        var results = await Promise.all([
          OpenRawAPI.get('/api/comms/topology'),
          OpenRawAPI.get('/api/comms/events?limit=200')
        ]);
    ...
    startSSE() {
      ...
      var self = this;
      var url = OpenRawAPI.baseUrl + '/api/comms/events/stream';
      if (OpenRawAPI.apiKey) url += '?token=' + encodeURIComponent(OpenRawAPI.apiKey);
    ...
    async refreshTopology() {
      try {
        this.topology = await OpenRawAPI.get('/api/comms/topology');
      } catch(e) { /* silent */ }
    },
    ...
    async submitSend() {
      ...
      try {
        await OpenRawAPI.post('/api/comms/send', {
          from_agent_id: this.sendFrom,
          to_agent_id: this.sendTo,
          message: this.sendMsg
        });
        OpenRawToast.success('Message sent');
        this.showSendModal = false;
      } catch(e) {
        OpenRawToast.error(e.message || 'Send failed');
      }
    ...
    async submitTask() {
      ...
      try {
        ...
        await OpenRawAPI.post('/api/comms/task', body);
        OpenRawToast.success('Task posted');
        this.showTaskModal = false;
      } catch(e) {
        OpenRawToast.error(e.message || 'Task failed');
      }
