// OpenRaw Settings Page вЂ” Provider Hub, Model Catalog, Config, Tools + Security, Network, Migration tabs
'use strict';
...
function settingsPage() {
  return {
    ...
    coreFeatures: [
      {
        ...
        impl: 'host_functions.rs вЂ” safe_resolve_path() + safe_resolve_parent()'
      },
      {
        ...
        impl: 'host_functions.rs вЂ” is_ssrf_target() + is_private_ip()'
      },
      {
        ...
        impl: 'host_functions.rs вЂ” check_capability() on every host function'
      },
      {
        ...
        impl: 'kernel_handle.rs вЂ” spawn_agent_checked()'
      },
      {
        ...
        impl: 'subprocess_sandbox.rs вЂ” env_clear() + SAFE_ENV_VARS'
      },
      {
        ...
        impl: 'middleware.rs вЂ” security_headers()'
      },
      {
        ...
        impl: 'peer.rs вЂ” hmac_sign() + hmac_verify()'
      },
      {
        ...
        impl: 'middleware.rs вЂ” request_logging()'
      }
    ],
    ...
    configurableFeatures: [
      {
        ...
        description: 'GCRA (Generic Cell Rate Algorithm) with cost-aware tokens. Different endpoints cost different amounts вЂ” spawning an agent costs 50 tokens, health check costs 1.',
      ...
      },
      {
        ...
        configHint: 'Set api_key in ~/.openraw/config.toml for remote access. Empty = localhost only.',
        valueKey: 'auth'
      }
    ],
    ...
    async loadSysInfo() {
      try {
        var ver = await OpenRawAPI.get('/api/version');
        var status = await OpenRawAPI.get('/api/status');
        this.sysInfo = {
          version: ver.version || '-',
    ...
    async loadUsage() {
      try {
        var data = await OpenRawAPI.get('/api/usage');
    ...
    async loadTools() {
      try {
        var data = await OpenRawAPI.get('/api/tools');
    ...
    async loadConfig() {
      try {
        this.config = await OpenRawAPI.get('/api/config');
      } catch(e) { this.config = {}; }
    },

    async loadProviders() {
      try {
        var data = await OpenRawAPI.get('/api/providers');
    ...
    async loadModels() {
      try {
        var data = await OpenRawAPI.get('/api/models');
    ...
    async addCustomModel() {
      ...
      try {
        await OpenRawAPI.post('/api/models/custom', {
          id: id,
    ...
    async deleteCustomModel(modelId) {
      ...
      try {
        await OpenRawAPI.del('/api/models/custom/' + encodeURIComponent(modelId));
        OpenRawToast.success('Model deleted');
        await this.loadModels();
      } catch(e) {
        OpenRawToast.error('Failed to delete: ' + (e.message || 'Unknown error'));
      }
    },
    ...
    async loadConfigSchema() {
      try {
        var results = await Promise.all([
          OpenRawAPI.get('/api/config/schema').catch(function() { return {}; }),
          OpenRawAPI.get('/api/config')
        ]);
    ...
    async saveConfigField(section, field, value) {
      ...
      try {
        await OpenRawAPI.post('/api/config/set', { path: key, value: value });
        this.configDirty[key] = false;
        OpenRawToast.success('Saved ' + key);
      } catch(e) {
        OpenRawToast.error('Failed to save: ' + e.message);
      }
    ...
    async saveProviderKey(provider) {
      ...
      if (!key || !key.trim()) { OpenRawToast.error('Please enter an API key'); return; }
      try {
        await OpenRawAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/key', { key: key.trim() });
        OpenRawToast.success('API key saved for ' + provider.display_name);
        this.providerKeyInputs[provider.id] = '';
        await this.loadProviders();
        await this.loadModels();
      } catch(e) {
        OpenRawToast.error('Failed to save key: ' + e.message);
      }
    },

    async removeProviderKey(provider) {
      try {
        await OpenRawAPI.del('/api/providers/' + encodeURIComponent(provider.id) + '/key');
        OpenRawToast.success('API key removed for ' + provider.display_name);
        await this.loadProviders();
        await this.loadModels();
      } catch(e) {
        OpenRawToast.error('Failed to remove key: ' + e.message);
      }
    },
    ...
    async startCopilotOAuth() {
      ...
      try {
        var resp = await OpenRawAPI.post('/api/providers/github-copilot/oauth/start', {});
        ...
        OpenRawToast.error('Failed to start Copilot login: ' + e.message);
    ...
    pollCopilotOAuth() {
      ...
      setTimeout(async function() {
        ...
        try {
          var resp = await OpenRawAPI.get('/api/providers/github-copilot/oauth/poll/' + self.copilotOAuth.pollId);
          if (resp.status === 'complete') {
            OpenRawToast.success('GitHub Copilot authenticated successfully!');
            ...
            OpenRawToast.error('Device code expired. Please try again.');
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          } else if (resp.status === 'denied') {
            OpenRawToast.error('Access denied by user.');
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          } else {
            OpenRawToast.error('OAuth error: ' + (resp.error || resp.status));
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          }
        } catch(e) {
          OpenRawToast.error('Poll error: ' + e.message);
          self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
        }
    ...
    async testProvider(provider) {
      ...
      try {
        var result = await OpenRawAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/test', {});
        this.providerTestResults[provider.id] = result;
        if (result.status === 'ok') {
          OpenRawToast.success(provider.display_name + ' connected (' + (result.latency_ms || '?') + 'ms)');
        } else {
          OpenRawToast.error(provider.display_name + ': ' + (result.error || 'Connection failed'));
        }
      } catch(e) {
        this.providerTestResults[provider.id] = { status: 'error', error: e.message };
        OpenRawToast.error('Test failed: ' + e.message);
      }
    ...
    async saveProviderUrl(provider) {
      ...
      if (!url || !url.trim()) { OpenRawToast.error('Please enter a base URL'); return; }
      url = url.trim();
      if (url.indexOf('http://') !== 0 && url.indexOf('https://') !== 0) {
        OpenRawToast.error('URL must start with http:// or https://'); return;
      }
      this.providerUrlSaving[provider.id] = true;
      try {
        var result = await OpenRawAPI.put('/api/providers/' + encodeURIComponent(provider.id) + '/url', { base_url: url });
        if (result.reachable) {
          OpenRawToast.success(provider.display_name + ' URL saved &mdash; reachable (' + (result.latency_ms || '?') + 'ms)');
        } else {
          OpenRawToast.warning(provider.display_name + ' URL saved but not reachable');
        }
        await this.loadProviders();
      } catch(e) {
        OpenRawToast.error('Failed to save URL: ' + e.message);
      }
    ...
    async loadSecurity() {
      ...
      try {
        this.securityData = await OpenRawAPI.get('/api/security');
      } catch(e) {
    ...
    async verifyAuditChain() {
      ...
      try {
        var res = await OpenRawAPI.get('/api/audit/verify');
    ...
    async loadPeers() {
      ...
      this.peersLoadError = '';
      try {
        var data = await OpenRawAPI.get('/api/peers');
    ...
    async autoDetect() {
      this.detecting = true;
      try {
        var data = await OpenRawAPI.get('/api/migrate/detect');
    ...
    async scanPath() {
      ...
      this.scanning = true;
      try {
        var data = await OpenRawAPI.post('/api/migrate/scan', { path: this.sourcePath });
        if (data.error) {
          OpenRawToast.error('Scan error: ' + data.error);
          this.scanning = false;
          return;
        }
        ...
        OpenRawToast.error('Scan failed: ' + e.message);
      }
      this.scanning = false;
    },
    ...
    async runMigration(dryRun) {
      ...
      try {
        ...
        if (!target) target = '';
        var data = await OpenRawAPI.post('/api/migrate', {
          source: 'openclaw',
