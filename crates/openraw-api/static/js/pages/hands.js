// OpenRaw Hands Page вЂ” curated autonomous capability packages
'use strict';
...
function handsPage() {
  return {
    ...
    // в”Ђв”Ђ Setup Wizard State в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
    setupWizard: null,
    setupStep: 1,
    setupLoading: false,
    setupChecking: false,
    ...
    async loadData() {
      ...
      this.loadError = '';
      try {
        var data = await OpenRawAPI.get('/api/hands');
    ...
    async loadActive() {
      ...
      try {
        var data = await OpenRawAPI.get('/api/hands/active');
    ...
    async showDetail(handId) {
      try {
        var data = await OpenRawAPI.get('/api/hands/' + handId);
    ...
    // в”Ђв”Ђ Setup Wizard в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
    ...
    // в”Ђв”Ђ Auto-Install Dependencies в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
    ...
    async installDeps() {
      ...
      try {
        var data = await OpenRawAPI.post('/api/hands/' + handId + '/install-deps', {});
    ...
    async recheckDeps() {
      ...
      try {
        var data = await OpenRawAPI.post('/api/hands/' + this.setupWizard.id + '/check-deps', {});
        if (data.requirements && this.setupWizard.requirements) {
          for (var i = 0; i < this.setupWizard.requirements.length; i++) {
            var existing = this.setupWizard.requirements[i];
            for (var j = 0; j < data.requirements.length; j++) {
    ...
    async launchHand() {
      ...
      try {
        var data = await OpenRawAPI.post('/api/hands/' + handId + '/activate', { config: config });
    ...
    // в”Ђв”Ђ Existing methods в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ

    async pauseHand(inst) {
      try {
        await OpenRawAPI.post('/api/hands/instances/' + inst.instance_id + '/pause', {});
        inst.status = 'Paused';
      } catch(e) {
    ...
    async resumeHand(inst) {
      try {
        await OpenRawAPI.post('/api/hands/instances/' + inst.instance_id + '/resume', {});
        inst.status = 'Active';
      } catch(e) {
    ...
    async deactivate(inst) {
      ...
      OpenRawToast.confirm('Deactivate Hand', 'Deactivate hand "' + handName + '"? This will kill its agent.', async function() {
        try {
          await OpenRawAPI.delete('/api/hands/instances/' + inst.instance_id);
          self.instances = self.instances.filter(function(i) { return i.instance_id !== inst.instance_id; });
          OpenRawToast.success('Hand deactivated.');
        } catch(e) {
          OpenRawToast.error('Deactivation failed: ' + (e.message || 'unknown error'));
        }
      });
    },
    ...
    async loadStats(inst) {
      try {
        var data = await OpenRawAPI.get('/api/hands/instances/' + inst.instance_id + '/stats');
    ...
    // в”Ђв”Ђ Browser Viewer в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
    ...
    async refreshBrowserView() {
      ...
      try {
        var data = await OpenRawAPI.get('/api/hands/instances/' + id + '/browser');
        if (data.active) {
