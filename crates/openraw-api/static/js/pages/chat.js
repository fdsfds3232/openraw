// OpenRaw Chat Page вЂ” Agent chat with markdown + streaming
'use strict';
  ...
  return {
      ...
      this.$watch('inputText', function(val) {
        ...
        if (modelMatch) {
          ...
          if (!self.modelPickerList.length) {
            OpenRawAPI.get('/api/models').then(function(data) {
    ...
    toggleModelSwitcher() {
      ...
      OpenRawAPI.get('/api/models').then(function(data) {
        ...
        OpenRawToast.error('Failed to load models: ' + e.message);
      });
    },
    ...
    switchModel(model) {
      ...
      OpenRawAPI.put('/api/agents/' + this.currentAgent.id + '/model', { model: model.id }).then(function() {
        self.currentAgent.model_name = model.id;
        self.currentAgent.model_provider = model.provider;
        OpenRawToast.success('Switched to ' + (model.display_name || model.id));
        self.showModelSwitcher = false;
        self.modelSwitching = false;
      }).catch(function(e) {
        OpenRawToast.error('Switch failed: ' + e.message);
        self.modelSwitching = false;
      });
    },
    ...
    fetchCommands: function() {
      var self = this;
      OpenRawAPI.get('/api/commands').then(function(data) {
      ...
      }).catch(function() { /* silent вЂ” use hardcoded list */ });
    },
    ...
    executeSlashCommand(cmd, cmdArgs) {
      ...
      switch (cmd) {
        case '/help':
          self.messages.push({ id: ++msgId, role: 'system', text: self.slashCommands.map(function(c) { return '`' + c.cmd + '` вЂ” ' + c.desc; }).join('\n'), meta: '', tools: [] });
          self.scrollToBottom();
          break;
        case '/agents':
        ...
        case '/new':
          if (self.currentAgent) {
            OpenRawAPI.post('/api/agents/' + self.currentAgent.id + '/session/reset', {}).then(function() {
              self.messages = [];
              OpenRawToast.success('Session reset');
            }).catch(function(e) { OpenRawToast.error('Reset failed: ' + e.message); });
          }
          break;
        case '/compact':
          if (self.currentAgent) {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Compacting session...', meta: '', tools: [] });
            OpenRawAPI.post('/api/agents/' + self.currentAgent.id + '/session/compact', {}).then(function(res) {
              self.messages.push({ id: ++msgId, role: 'system', text: res.message || 'Compaction complete', meta: '', tools: [] });
              self.scrollToBottom();
            }).catch(function(e) { OpenRawToast.error('Compaction failed: ' + e.message); });
          }
          break;
        case '/stop':
          if (self.currentAgent) {
            OpenRawAPI.post('/api/agents/' + self.currentAgent.id + '/stop', {}).then(function(res) {
              self.messages.push({ id: ++msgId, role: 'system', text: res.message || 'Run cancelled', meta: '', tools: [] });
              self.sending = false;
              self.scrollToBottom();
            }).catch(function(e) { OpenRawToast.error('Stop failed: ' + e.message); });
          }
          break;
        case '/usage':
          if (self.currentAgent) {
        ...
        case '/context':
          // Send via WS command
          if (self.currentAgent && OpenRawAPI.isWsConnected()) {
            OpenRawAPI.wsSend({ type: 'command', command: 'context', args: '' });
          } else {
        ...
        case '/verbose':
          if (self.currentAgent && OpenRawAPI.isWsConnected()) {
            OpenRawAPI.wsSend({ type: 'command', command: 'verbose', args: cmdArgs });
          } else {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Not connected. Connect to an agent first.', meta: '', tools: [] });
            self.scrollToBottom();
          }
        ...
        case '/queue':
          if (self.currentAgent && OpenRawAPI.isWsConnected()) {
            OpenRawAPI.wsSend({ type: 'command', command: 'queue', args: '' });
          } else {
          ...
          break;
        case '/status':
          OpenRawAPI.get('/api/status').then(function(s) {
        ...
        case '/model':
          if (self.currentAgent) {
            if (cmdArgs) {
              OpenRawAPI.put('/api/agents/' + self.currentAgent.id + '/model', { model: cmdArgs }).then(function() {
                self.currentAgent.model_name = cmdArgs;
                self.messages.push({ id: ++msgId, role: 'system', text: 'Model switched to: `' + cmdArgs + '`', meta: '', tools: [] });
                self.scrollToBottom();
              }).catch(function(e) { OpenRawToast.error('Model switch failed: ' + e.message); });
            } else {
        ...
        case '/exit':
          ...
          break;
        case '/budget':
          OpenRawAPI.get('/api/budget').then(function(b) {
          ...
          break;
        case '/peers':
          OpenRawAPI.get('/api/network/status').then(function(ns) {
    ...
    async loadSession(agentId) {
      var self = this;
      try {
        var data = await OpenRawAPI.get('/api/agents/' + agentId + '/session');
    ...
    async loadSessions(agentId) {
      try {
        var data = await OpenRawAPI.get('/api/agents/' + agentId + '/sessions');
    ...
    async createSession() {
      ...
      try {
        await OpenRawAPI.post('/api/agents/' + this.currentAgent.id + '/sessions', {
        ...
        this.messages = [];
        this.scrollToBottom();
        if (typeof OpenRawToast !== 'undefined') OpenRawToast.success('New session created');
      } catch(e) {
        if (typeof OpenRawToast !== 'undefined') OpenRawToast.error('Failed to create session');
      }
    },
    ...
    async switchSession(sessionId) {
      if (!this.currentAgent) return;
      try {
        await OpenRawAPI.post('/api/agents/' + this.currentAgent.id + '/sessions/' + sessionId + '/switch', {});
        this.messages = [];
        await this.loadSession(this.currentAgent.id);
        await this.loadSessions(this.currentAgent.id);
        ...
        if (typeof OpenRawToast !== 'undefined') OpenRawToast.error('Failed to switch session');
      }
    },
    ...
    handleWsMessage(data) {
      switch (data.type) {
        ...
        case 'tool_end':
          // Tool call parsed by LLM вЂ” update tool card with input params
        ...
        case 'tool_result':
          // Tool execution completed вЂ” update tool card with result
        ...
        case 'response':
          ...
          streamedTools.forEach(function(t) {
            t.running = false;
            // Text-detected tool calls (model leaked as text) вЂ” mark as not executed
        ...
        case 'silent_complete':
          ...
          this.sending = false;
          this.tokenCount = 0;
          // No message bubble added вЂ” the agent was silent
          var selfSilent = this;
        ...
        case 'canvas':
          // Agent presented an interactive canvas вЂ” render it in an iframe sandbox
    ...
    async sendMessage() {
      ...
      if (this.attachments.length) {
        for (var i = 0; i < this.attachments.length; i++) {
          ...
          att.uploading = true;
          try {
            var uploadRes = await OpenRawAPI.upload(this.currentAgent.id, att.file);
            ...
            OpenRawToast.error('Failed to upload ' + att.file.name);
    ...
    async _sendPayload(finalText, uploadedFiles, msgImages) {
      ...
      if (OpenRawAPI.wsSend(wsPayload)) {
      ...
      // HTTP fallback
      if (!OpenRawAPI.isWsConnected()) {
        OpenRawToast.info('Using HTTP mode (no streaming)');
      }
      ...
      try {
        ...
        var res = await OpenRawAPI.post('/api/agents/' + this.currentAgent.id + '/message', httpBody);
    ...
    stopAgent: function() {
      if (!this.currentAgent) return;
      var self = this;
      OpenRawAPI.post('/api/agents/' + this.currentAgent.id + '/stop', {}).then(function(res) {
    ...
    killAgent() {
      ...
      OpenRawToast.confirm('Stop Agent', 'Stop agent "' + name + '"? The agent will be shut down.', async function() {
        try {
          await OpenRawAPI.del('/api/agents/' + self.currentAgent.id);
          OpenRawAPI.wsDisconnect();
          self._wsAgent = null;
          self.currentAgent = null;
          self.messages = [];
          OpenRawToast.success('Agent "' + name + '" stopped');
    ...
    addFiles(files) {
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        if (file.size > 10 * 1024 * 1024) {
          OpenRawToast.warn('File "' + file.name + '" exceeds 10MB limit');
          continue;
        }
        ...
        if (!typeOk) {
          OpenRawToast.warn('File type not supported: ' + file.name);
          continue;
        }
        var preview = null;
    ...
    // These models don't use proper tool_use blocks вЂ” they output function calls as plain text.
    ...
    startRecording: async function() {
      ...
      try {
        ...
        if (typeof OpenRawToast !== 'undefined') OpenRawToast.error('Microphone access denied');
      }
    },
    ...
    // Voice: handle completed recording вЂ” upload and transcribe
    _handleRecordingComplete: async function() {
      ...
      try {
        ...
        var upload = await OpenRawAPI.upload(this.currentAgent.id, file);
        ...
        this.messages = this.messages.filter(function(m) { return !m.thinking || m.role !== 'system'; });
        if (typeof OpenRawToast !== 'undefined') OpenRawToast.error('Failed to upload audio: ' + (e.message || 'unknown error'));
      }
    },
