// OpenRaw Logs Page вЂ” Real-time log viewer (SSE streaming + polling fallback) + Audit Trail tab
'use strict';
...
function logsPage() {
  return {
    ...
    async fetchLogs() {
      ...
      try {
        var data = await OpenRawAPI.get('/api/audit/recent?n=200');
    ...
    exportLogs: function() {
      ...
      a.href = url;
      a.download = 'openraw-logs-' + new Date().toISOString().slice(0, 10) + '.txt';
      a.click();
    ...
    async verifyChain() {
      try {
        var data = await OpenRawAPI.get('/api/audit/verify');
        this.chainValid = data.valid === true;
        if (this.chainValid) {
          OpenRawToast.success('Audit chain verified вЂ” ' + (data.entries || 0) + ' entries valid');
        } else {
          OpenRawToast.error('Audit chain broken!');
        }
      } catch(e) {
        this.chainValid = false;
        OpenRawToast.error('Chain verification failed: ' + e.message);
      }
    },
