// OpenRaw Skills Page вЂ” OpenClaw/ClawHub ecosystem + local skills + MCP servers
'use strict';
...
function skillsPage() {
  return {
    ...
    async loadSkills() {
      ...
      this.loadError = '';
      try {
        var data = await OpenRawAPI.get('/api/skills');
    ...
    // Debounced search вЂ” fires 350ms after user stops typing
    onSearchInput() {
    ...
    async searchClawHub() {
      ...
      this.clawhubError = '';
      try {
        var data = await OpenRawAPI.get('/api/clawhub/search?q=' + encodeURIComponent(this.clawhubSearch.trim()) + '&limit=20');
    ...
    async browseClawHub(sort) {
      ...
      try {
        var data = await OpenRawAPI.get('/api/clawhub/browse?sort=' + this.clawhubSort + '&limit=20');
    ...
    async loadMoreClawHub() {
      ...
      this.clawhubLoading = true;
      try {
        var data = await OpenRawAPI.get('/api/clawhub/browse?sort=' + this.clawhubSort + '&limit=20&cursor=' + encodeURIComponent(this.clawhubNextCursor));
    ...
    async showSkillDetail(slug) {
      ...
      try {
        var data = await OpenRawAPI.get('/api/clawhub/skill/' + encodeURIComponent(slug));
        this.skillDetail = data;
      } catch(e) {
        OpenRawToast.error('Failed to load skill details');
      }
    ...
    async viewSkillCode(slug) {
      ...
      try {
        var data = await OpenRawAPI.get('/api/clawhub/skill/' + encodeURIComponent(slug) + '/code');
        this.skillCode = data.code || '';
        this.skillCodeFilename = data.filename || 'source';
        this.showSkillCode = true;
      } catch(e) {
        OpenRawToast.error('Could not load skill source code');
      }
    ...
    async installFromClawHub(slug) {
      ...
      this.installResult = null;
      try {
        var data = await OpenRawAPI.post('/api/clawhub/install', { slug: slug });
        this.installResult = data;
        if (data.warnings && data.warnings.length > 0) {
          OpenRawToast.success('Skill "' + data.name + '" installed with ' + data.warnings.length + ' warning(s)');
        } else {
          OpenRawToast.success('Skill "' + data.name + '" installed successfully');
        }
        ...
        if (msg.includes('already_installed')) {
          OpenRawToast.error('Skill is already installed');
        } else if (msg.includes('SecurityBlocked')) {
          OpenRawToast.error('Skill blocked by security scan');
        } else {
          OpenRawToast.error('Install failed: ' + msg);
        }
      }
    ...
    uninstallSkill: function(name) {
      var self = this;
      OpenRawToast.confirm('Uninstall Skill', 'Uninstall skill "' + name + '"? This cannot be undone.', async function() {
        try {
          await OpenRawAPI.post('/api/skills/uninstall', { name: name });
          OpenRawToast.success('Skill "' + name + '" uninstalled');
          await self.loadSkills();
        } catch(e) {
          OpenRawToast.error('Failed to uninstall skill: ' + e.message);
        }
      });
    },
    ...
    async createDemoSkill(skill) {
      try {
        await OpenRawAPI.post('/api/skills/create', {
          name: skill.name,
          description: skill.description,
          runtime: 'prompt_only',
          prompt_context: skill.prompt_context || skill.description
        });
        OpenRawToast.success('Skill "' + skill.name + '" created');
        this.tab = 'installed';
        await this.loadSkills();
      } catch(e) {
        OpenRawToast.error('Failed to create skill: ' + e.message);
      }
    },
    ...
    async loadMcpServers() {
      ...
      try {
        var data = await OpenRawAPI.get('/api/mcp/servers');
