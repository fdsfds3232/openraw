// OpenRaw Agents Page вЂ” Multi-step spawn wizard, detail view with tabs, file editor, personality presets
'use strict';
...
function agentsPage() {
  return {
    ...
    spawnStep: 1,
    spawnIdentity: { emoji: '', color: '#1A1817', archetype: '' },
    selectedPreset: '',
    soulContent: '',
    emojiOptions: [
    ...
    // в”Ђв”Ђ Profile Descriptions в”Ђв”Ђ
    profileDescriptions: {
    ...
    // в”Ђв”Ђ Tool Preview in Spawn Modal в”Ђв”Ђ
    spawnProfiles: [],
    spawnProfilesLoaded: false,
    async loadSpawnProfiles() {
      if (this.spawnProfilesLoaded) return;
      try {
        var data = await OpenRawAPI.get('/api/profiles');
    ...
    async loadTemplates() {
      ...
      try {
        var results = await Promise.all([
          OpenRawAPI.get('/api/templates'),
          OpenRawAPI.get('/api/providers').catch(function() { return { providers: [] }; })
        ]);
    ...
    showDetail(agent) {
      ...
      this.configForm = {
        ...
        color: (agent.identity && agent.identity.color) || '#1A1817',
    ...
    killAgent(agent) {
      var self = this;
      OpenRawToast.confirm('Stop Agent', 'Stop agent "' + agent.name + '"? The agent will be shut down.', async function() {
        try {
          await OpenRawAPI.del('/api/agents/' + agent.id);
          OpenRawToast.success('Agent "' + agent.name + '" stopped');
          self.showDetailModal = false;
          await Alpine.store('app').refreshAgents();
        } catch(e) {
          OpenRawToast.error('Failed to stop agent: ' + e.message);
        }
      });
    },
    ...
    killAllAgents() {
      ...
      OpenRawToast.confirm('Stop All Agents', 'Stop ' + list.length + ' agent(s)? All agents will be shut down.', async function() {
        var errors = [];
        for (var i = 0; i < list.length; i++) {
          try {
            await OpenRawAPI.del('/api/agents/' + list[i].id);
          } catch(e) { errors.push(list[i].name + ': ' + e.message); }
        }
        await Alpine.store('app').refreshAgents();
        if (errors.length) {
          OpenRawToast.error('Some agents failed to stop: ' + errors.join(', '));
        } else {
          OpenRawToast.success(list.length + ' agent(s) stopped');
        }
      });
    },

    // в”Ђв”Ђ Multi-step wizard navigation в”Ђв”Ђ
    openSpawnWizard() {
      this.showSpawnModal = true;
      this.spawnStep = 1;
      this.spawnMode = 'wizard';
      this.spawnIdentity = { emoji: '', color: '#1A1817', archetype: '' };
      this.selectedPreset = '';
      this.soulContent = '';
      this.spawnForm.name = '';
    ...
    nextStep() {
      if (this.spawnStep === 1 && !this.spawnForm.name.trim()) {
        OpenRawToast.warn('Please enter an agent name');
        return;
      }
    ...
    async setMode(agent, mode) {
      try {
        await OpenRawAPI.put('/api/agents/' + agent.id + '/mode', { mode: mode });
        agent.mode = mode;
        OpenRawToast.success('Mode set to ' + mode);
        await Alpine.store('app').refreshAgents();
      } catch(e) {
        OpenRawToast.error('Failed to set mode: ' + e.message);
      }
    },
    ...
    async spawnAgent() {
      ...
      if (!toml.trim()) {
        this.spawning = false;
        OpenRawToast.warn('Manifest is empty \u2014 enter agent config first');
        return;
      }

      try {
        var res = await OpenRawAPI.post('/api/agents', { manifest_toml: toml });
        if (res.agent_id) {
          ...
          if (Object.keys(patchBody).length) {
            OpenRawAPI.patch('/api/agents/' + res.agent_id + '/config', patchBody).catch(function(e) { console.warn('Post-spawn config patch failed:', e.message); });
          }
          if (this.soulContent.trim()) {
            OpenRawAPI.put('/api/agents/' + res.agent_id + '/files/SOUL.md', { content: '# Soul\n' + this.soulContent }).catch(function(e) { console.warn('SOUL.md write failed:', e.message); });
          }
          ...
          this.spawnToml = '';
          this.spawnStep = 1;
          OpenRawToast.success('Agent "' + (res.name || 'new') + '" spawned');
          await Alpine.store('app').refreshAgents();
          this.chatWithAgent({ id: res.agent_id, name: res.name, model_provider: '?', model_name: '?' });
        } else {
          OpenRawToast.error('Spawn failed: ' + (res.error || 'Unknown error'));
        }
      } catch(e) {
        OpenRawToast.error('Failed to spawn agent: ' + e.message);
      }
      this.spawning = false;
    },

    // в”Ђв”Ђ Detail modal: Files tab в”Ђв”Ђ
    async loadAgentFiles() {
      if (!this.detailAgent) return;
      this.filesLoading = true;
      try {
        var data = await OpenRawAPI.get('/api/agents/' + this.detailAgent.id + '/files');
        this.agentFiles = data.files || [];
      } catch(e) {
        this.agentFiles = [];
        OpenRawToast.error('Failed to load files: ' + e.message);
      }
    ...
    async openFile(file) {
      ...
      try {
        var data = await OpenRawAPI.get('/api/agents/' + this.detailAgent.id + '/files/' + encodeURIComponent(file.name));
        this.editingFile = file.name;
        this.fileContent = data.content || '';
      } catch(e) {
        OpenRawToast.error('Failed to read file: ' + e.message);
      }
    },
    ...
    async saveFile() {
      ...
      try {
        await OpenRawAPI.put('/api/agents/' + this.detailAgent.id + '/files/' + encodeURIComponent(this.editingFile), { content: this.fileContent });
        OpenRawToast.success(this.editingFile + ' saved');
        await this.loadAgentFiles();
      } catch(e) {
        OpenRawToast.error('Failed to save file: ' + e.message);
      }
    ...
    // в”Ђв”Ђ Detail modal: Config tab в”Ђв”Ђ
    async saveConfig() {
      if (!this.detailAgent) return;
      this.configSaving = true;
      try {
        await OpenRawAPI.patch('/api/agents/' + this.detailAgent.id + '/config', this.configForm);
        OpenRawToast.success('Config updated');
        await Alpine.store('app').refreshAgents();
      } catch(e) {
        OpenRawToast.error('Failed to save config: ' + e.message);
      }
      this.configSaving = false;
    },

    // в”Ђв”Ђ Clone agent в”Ђв”Ђ
    async cloneAgent(agent) {
      var newName = (agent.name || 'agent') + '-copy';
      try {
        var res = await OpenRawAPI.post('/api/agents/' + agent.id + '/clone', { new_name: newName });
        if (res.agent_id) {
          OpenRawToast.success('Cloned as "' + res.name + '"');
          await Alpine.store('app').refreshAgents();
          this.showDetailModal = false;
        }
      } catch(e) {
        OpenRawToast.error('Clone failed: ' + e.message);
      }
    },
    ...
    async spawnFromTemplate(name) {
      try {
        var data = await OpenRawAPI.get('/api/templates/' + encodeURIComponent(name));
        if (data.manifest_toml) {
          var res = await OpenRawAPI.post('/api/agents', { manifest_toml: data.manifest_toml });
          if (res.agent_id) {
            OpenRawToast.success('Agent "' + (res.name || name) + '" spawned from template');
            await Alpine.store('app').refreshAgents();
            this.chatWithAgent({ id: res.agent_id, name: res.name || name, model_provider: '?', model_name: '?' });
          }
        }
      } catch(e) {
        OpenRawToast.error('Failed to spawn from template: ' + e.message);
      }
    },

    // в”Ђв”Ђ Clear agent history в”Ђв”Ђ
    async clearHistory(agent) {
      var self = this;
      OpenRawToast.confirm('Clear History', 'Clear all conversation history for "' + agent.name + '"? This cannot be undone.', async function() {
        try {
          await OpenRawAPI.del('/api/agents/' + agent.id + '/history');
          OpenRawToast.success('History cleared for "' + agent.name + '"');
        } catch(e) {
          OpenRawToast.error('Failed to clear history: ' + e.message);
        }
      });
    },

    // в”Ђв”Ђ Model switch в”Ђв”Ђ
    async changeModel() {
      if (!this.detailAgent || !this.newModelValue.trim()) return;
      this.modelSaving = true;
      try {
        await OpenRawAPI.put('/api/agents/' + this.detailAgent.id + '/model', { model: this.newModelValue.trim() });
        OpenRawToast.success('Model changed (memory reset)');
        ...
        OpenRawToast.error('Failed to change model: ' + e.message);
      }
      this.modelSaving = false;
    },

    // в”Ђв”Ђ Tool filters в”Ђв”Ђ
    async loadToolFilters() {
      if (!this.detailAgent) return;
      this.toolFiltersLoading = true;
      try {
        this.toolFilters = await OpenRawAPI.get('/api/agents/' + this.detailAgent.id + '/tools');
      } catch(e) {
    ...
    async saveToolFilters() {
      if (!this.detailAgent) return;
      try {
        await OpenRawAPI.put('/api/agents/' + this.detailAgent.id + '/tools', this.toolFilters);
      } catch(e) {
        OpenRawToast.error('Failed to update tool filters: ' + e.message);
      }
    },
    ...
    async spawnBuiltin(t) {
      ...
      try {
        var res = await OpenRawAPI.post('/api/agents', { manifest_toml: toml });
        if (res.agent_id) {
          OpenRawToast.success('Agent "' + t.name + '" spawned');
          await Alpine.store('app').refreshAgents();
