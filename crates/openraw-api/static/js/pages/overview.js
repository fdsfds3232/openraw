// OpenRaw Overview Dashboard вЂ” Landing page with system stats + provider status
'use strict';
...
function overviewPage() {
  return {
    ...
    async loadHealth() {
      try {
        this.health = await OpenRawAPI.get('/api/health');
      } catch(e) { this.health = { status: 'unreachable' }; }
    },

    async loadStatus() {
      try {
        this.status = await OpenRawAPI.get('/api/status');
    ...
    async loadAudit() {
      try {
        var data = await OpenRawAPI.get('/api/audit/recent?n=8');
