...
//!
//! Allows any OpenAI-compatible client library to talk to OpenRaw agents.
//! The `model` field resolves to an agent (by name, UUID, or `openraw:<name>`),
...
use openraw_types::message::{ContentBlock, Message, MessageContent, Role, StopReason};
...
fn resolve_agent(state: &AppState, model: &str) -> Option<(AgentId, String)> {
    // 1. "openraw:<name>" → find agent by name
    if let Some(name) = model.strip_prefix("openraw:") {
...
pub async fn list_models(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let models: Vec<ModelObject> = agents
        .iter()
        .map(|e| ModelObject {
            id: format!("openraw:{}", e.name),
            object: "model",
            created,
            owned_by: "openraw".to_string(),
        })
        .collect();
