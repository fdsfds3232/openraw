//! OpenRaw daemon server — boots the kernel and serves the HTTP API.
...
/// Daemon info written to `~/.openraw/daemon.json` so the CLI can find us.
...
///
/// This is extracted from `run_daemon()` so that embedders (e.g. openraw-desktop)
...
/// Start the OpenRaw daemon: boot kernel + HTTP API server.
///
...
pub async fn run_daemon(
    ...
    info!("OpenRaw API server listening on http://{addr}");
