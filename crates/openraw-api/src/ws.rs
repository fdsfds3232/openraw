...
use openraw_runtime::llm_errors;
...
async fn handle_command(
    ...
    match cmd {
        ...
        "context" => match state.kernel.context_report(agent_id) {
            Ok(report) => {
                let formatted = openraw_runtime::compactor::format_context_report(&report);
                serde_json::json!({
                    "type": "command_result",
                    "command": cmd,
                    "message": formatted,
...
///
/// Uses the proper LLM error classifier from `openraw_runtime::llm_errors`
/// for comprehensive 20-provider coverage with actionable advice.
fn classify_streaming_error(err: &openraw_kernel::error::KernelError) -> String {
