//! Channel bridge wiring — connects the OpenRaw kernel to channel adapters.
//!
//! Implements `ChannelBridgeHandle` on `OpenRawKernel` and provides the
//! `start_channel_bridge()` entry point called by the daemon.

use openraw_channels::bridge::{BridgeManager, ChannelBridgeHandle};
use openraw_channels::discord::DiscordAdapter;
use openraw_channels::email::EmailAdapter;
use openraw_channels::google_chat::GoogleChatAdapter;
use openraw_channels::irc::IrcAdapter;
use openraw_channels::matrix::MatrixAdapter;
use openraw_channels::mattermost::MattermostAdapter;
use openraw_channels::rocketchat::RocketChatAdapter;
use openraw_channels::router::AgentRouter;
use openraw_channels::signal::SignalAdapter;
use openraw_channels::slack::SlackAdapter;
use openraw_channels::teams::TeamsAdapter;
use openraw_channels::telegram::TelegramAdapter;
use openraw_channels::twitch::TwitchAdapter;
use openraw_channels::types::ChannelAdapter;
use openraw_channels::whatsapp::WhatsAppAdapter;
use openraw_channels::xmpp::XmppAdapter;
use openraw_channels::zulip::ZulipAdapter;
// Wave 3
use openraw_channels::bluesky::BlueskyAdapter;
use openraw_channels::feishu::FeishuAdapter;
use openraw_channels::line::LineAdapter;
use openraw_channels::mastodon::MastodonAdapter;
use openraw_channels::messenger::MessengerAdapter;
use openraw_channels::reddit::RedditAdapter;
use openraw_channels::revolt::RevoltAdapter;
use openraw_channels::viber::ViberAdapter;
// Wave 4
use openraw_channels::flock::FlockAdapter;
use openraw_channels::guilded::GuildedAdapter;
use openraw_channels::keybase::KeybaseAdapter;
use openraw_channels::nextcloud::NextcloudAdapter;
use openraw_channels::nostr::NostrAdapter;
use openraw_channels::pumble::PumbleAdapter;
use openraw_channels::threema::ThreemaAdapter;
use openraw_channels::twist::TwistAdapter;
use openraw_channels::webex::WebexAdapter;
// Wave 5
use async_trait::async_trait;
use openraw_channels::dingtalk::DingTalkAdapter;
use openraw_channels::discourse::DiscourseAdapter;
use openraw_channels::gitter::GitterAdapter;
use openraw_channels::gotify::GotifyAdapter;
use openraw_channels::linkedin::LinkedInAdapter;
use openraw_channels::mumble::MumbleAdapter;
use openraw_channels::ntfy::NtfyAdapter;
use openraw_channels::webhook::WebhookAdapter;
use openraw_kernel::OpenRawKernel;
use openraw_types::agent::AgentId;
use std::sync::Arc;
use std::time::{Duration, Instant};
use tracing::{error, info, warn};

/// Wraps `OpenRawKernel` to implement `ChannelBridgeHandle`.
pub struct KernelBridgeAdapter {
    kernel: Arc<OpenRawKernel>,
    started_at: Instant,
}
...
impl ChannelBridgeHandle for KernelBridgeAdapter {
    ...
    async fn spawn_agent_by_name(&self, manifest_name: &str) -> Result<AgentId, String> {
        // Look for manifest at ~/.openraw/agents/{name}/agent.toml
    ...
    async fn uptime_info(&self) -> String {
        ...
        if hours > 0 {
            format!(
                "OpenRaw status: {}h {}m uptime, {} agent(s)",
                hours,
                mins,
                agents.len()
            )
            ...
            format!(
                "OpenRaw status: {}m uptime, {} agent(s)",
                mins,
                agents.len()
            )
        }
    ...
    async fn list_providers_text(&self) -> String {
        ...
        for p in catalog.list_providers() {
            let status = match p.auth_status {
                openraw_types::model_catalog::AuthStatus::Configured => "configured",
                openraw_types::model_catalog::AuthStatus::Missing => "not configured",
                openraw_types::model_catalog::AuthStatus::NotRequired => "local (no key needed)",
            };
            msg.push_str(&format!(
                "  {} — {} [{}, {} model(s)]\n",
    ...
    async fn list_skills_text(&self) -> String {
        ...
        if skills.is_empty() {
            return "No skills installed. Place skills in ~/.openraw/skills/ or install from the marketplace.".to_string();
        }
        ...
        let result = self
            ...
            .execute_run(
                ...
                |step_agent| match step_agent {
                    openraw_kernel::workflow::StepAgent::ById { id } => {
                        let aid: AgentId = id.parse().ok()?;
                        let entry = registry_ref.get(aid)?;
                        Some((aid, entry.name.clone()))
                    }
                    openraw_kernel::workflow::StepAgent::ByName { name } => {
    ...
    async fn list_schedules_text(&self) -> String {
        ...
        for job in &jobs {
            ...
            let sched = match &job.schedule {
                openraw_types::scheduler::CronSchedule::Cron { expr, .. } => expr.clone(),
                openraw_types::scheduler::CronSchedule::Every { every_secs } => {
                    format!("every {every_secs}s")
                }
                openraw_types::scheduler::CronSchedule::At { at } => {
    ...
    async fn manage_schedule_text(&self, action: &str, args: &[String]) -> String {
        match action {
            "add" => {
                ...
                let job = openraw_types::scheduler::CronJob {
                    id: openraw_types::scheduler::CronJobId::new(),
                    agent_id: agent.id,
                    name: format!("chat-{}", &agent.name),
                    enabled: true,
                    schedule: openraw_types::scheduler::CronSchedule::Cron {
                        expr: cron_expr.clone(),
                        tz: None,
                    },
                    action: openraw_types::scheduler::CronAction::AgentTurn {
                        message: message.clone(),
                        model_override: None,
                        timeout_secs: None,
                    },
                    delivery: openraw_types::scheduler::CronDelivery::None,
                ...
                match matched.len() {
                    ...
                    1 => {
                        ...
                        let message = match &j.action {
                            openraw_types::scheduler::CronAction::AgentTurn { message, .. } => {
                                message.clone()
                            }
    ...
    async fn channel_overrides(
        &self,
        channel_type: &str,
    ) -> Option<openraw_types::config::ChannelOverrides> {
    ...
    async fn authorize_channel_user(
        ...
        let auth_action = match action {
            "chat" => openraw_kernel::auth::Action::ChatWithAgent,
            "spawn" => openraw_kernel::auth::Action::SpawnAgent,
            "kill" => openraw_kernel::auth::Action::KillAgent,
            "install_skill" => openraw_kernel::auth::Action::InstallSkill,
            _ => openraw_kernel::auth::Action::ChatWithAgent,
        };
    ...
    async fn record_delivery(
        ...
        let receipt = if success {
            openraw_kernel::DeliveryTracker::sent_receipt(channel, recipient)
        } else {
            openraw_kernel::DeliveryTracker::failed_receipt(
                channel,
                recipient,
    ...
    async fn a2a_agents_text(&self) -> String {
        ...
        for (url, card) in agents.iter() {
            ...
            if !desc.is_empty() {
                let short = openraw_types::truncate_str(desc, 60);
...
fn parse_trigger_pattern(s: &str) -> Option<openraw_kernel::triggers::TriggerPattern> {
    use openraw_kernel::triggers::TriggerPattern;
...
pub async fn start_channel_bridge(kernel: Arc<OpenRawKernel>) -> Option<BridgeManager> {
...
pub async fn reload_channels_from_disk(
    ...
    let fresh_config = openraw_kernel::config::load_config(Some(&config_path));
