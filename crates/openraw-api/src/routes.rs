        ...
        Ok(resp) if resp.success => {
            if let Some(data) = &resp.data {
                ...
                if content.len() > 2000 {
                    content = format!(
                        "{}... (truncated)",
                        openraw_types::truncate_str(&content, 2000)
                    );
                }
            }
        }
...
pub async fn set_provider_url(
    ...
    let probe = openraw_runtime::provider_health::probe_provider(&name, &base_url).await;
...
fn audit_to_comms_event(
    ...
    let (kind, detail, target_label) = match action_str.as_str() {
            ...
            let detail = if entry.detail.starts_with("tokens_in=") {
                ...
                if entry.outcome == "ok" {
                    ...
                    format!(
                        ...
                        in_tok,
                        out_tok,
                        openraw_types::truncate_str(&entry.outcome, 80)
                    )
                }
            } else if entry.outcome != "ok" {
                format!(
                    "{} — {}",
                    openraw_types::truncate_str(&entry.detail, 80),
                    openraw_types::truncate_str(&entry.outcome, 80)
                )
            } else {
        ...
        "AgentSpawn" => (
            ...
            format!(
                "Agent spawned: {}",
                openraw_types::truncate_str(&entry.detail, 100)
            ),
            "",
        ),
        "AgentKill" => (
