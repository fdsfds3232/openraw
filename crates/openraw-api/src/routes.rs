//! Route handlers for the OpenRaw API.
...
use axum::Json;
use dashmap::DashMap;
use openraw_kernel::triggers::{TriggerId, TriggerPattern};
use openraw_kernel::workflow::{
    ErrorMode, StepAgent, StepMode, Workflow, WorkflowId, WorkflowStep,
};
use openraw_kernel::OpenRawKernel;
use openraw_runtime::kernel_handle::KernelHandle;
use openraw_runtime::tool_runner::builtin_tool_definitions;
use openraw_types::agent::{AgentId, AgentIdentity, AgentManifest};
...
pub struct AppState {
    pub kernel: Arc<OpenRawKernel>,
    pub started_at: Instant,
    /// Optional peer registry for OFP mesh networking status.
    pub peer_registry: Option<Arc<openraw_wire::registry::PeerRegistry>>,
    /// Channel bridge manager — held behind a Mutex so it can be swapped on hot-reload.
    pub bridge_manager: tokio::sync::Mutex<Option<openraw_channels::bridge::BridgeManager>>,
    /// Live channel config — updated on every hot-reload so list_channels() reflects reality.
    pub channels_config: tokio::sync::RwLock<openraw_types::config::ChannelsConfig>,
        ...
        match state.kernel.verify_signed_manifest(signed_json) {
            ...
            Err(e) => {
                ...
                state.kernel.audit_log.record(
                    "system",
                    openraw_runtime::audit::AuditAction::AuthAttempt,
...
pub async fn list_agents(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let agents: Vec<serde_json::Value> = state
        ...
        .map(|e| {
            ...
            let ready = matches!(e.state, openraw_types::agent::AgentState::Running)
                && auth_status != "missing";
...
) -> Vec<openraw_types::message::ContentBlock> {
    use base64::Engine;

    let upload_dir = std::env::temp_dir().join("openraw_uploads");
    ...
    for att in attachments {
        ...
        match std::fs::read(&file_path) {
            Ok(data) => {
                ...
                blocks.push(openraw_types::message::ContentBlock::Image {
...
pub fn inject_attachments_into_session(
    kernel: &OpenRawKernel,
    agent_id: AgentId,
    image_blocks: Vec<openraw_types::message::ContentBlock>,
) {
    use openraw_types::message::{Message, MessageContent, Role};
    ...
    let mut session = match kernel.memory.get_session(entry.session_id) {
        Ok(Some(s)) => s,
        _ => openraw_memory::session::Session {
            id: entry.session_id,
            agent_id,
            messages: Vec::new(),
            ...
            let messages: Vec<serde_json::Value> = session
                ...
                .filter_map(|m| {
                    ...
                    let content = match &m.content {
                        openraw_types::message::MessageContent::Text(t) => t.clone(),
                        openraw_types::message::MessageContent::Blocks(blocks) => {
                            // Extract human-readable text and tool info from blocks
                            let mut texts = Vec::new();
                            for b in blocks {
                                match b {
                                    openraw_types::message::ContentBlock::Text { text } => {
                                        texts.push(text.clone());
                                    }
                                    openraw_types::message::ContentBlock::Image { .. } => {
                                        texts.push("[Image]".to_string());
                                    }
                                    openraw_types::message::ContentBlock::ToolUse {
                                        name, ..
                                    } => {
                                    ...
                                    openraw_types::message::ContentBlock::ToolResult {
                                        content: result,
                                        is_error,
                                        ..
                                    } => {
...
pub async fn shutdown(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    state.kernel.audit_log.record(
        "system",
        openraw_runtime::audit::AuditAction::ConfigChange,
...
pub async fn list_profiles() -> impl IntoResponse {
    use openraw_types::agent::ToolProfile;
...
pub async fn send_message_stream(
    ...
    use futures::stream;
    use openraw_runtime::llm_driver::StreamEvent;
...
const CHANNEL_REGISTRY: &[ChannelMeta] = &[
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "user_id", label: "Bot User ID", field_type: FieldType::Text, env_var: None, required: false, placeholder: "@openraw:matrix.org", advanced: true },
    ...
    ChannelMeta {
        ...
        setup_type: "form",
        fields: &[
            ChannelField { key: "username", label: "Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "openraw_bot", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "username", label: "Bot Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "openraw_bot", advanced: false },
            ...
            ChannelField { key: "subreddits", label: "Subreddits", field_type: FieldType::List, env_var: None, required: false, placeholder: "openraw, rust", advanced: true },
            ChannelField { key: "default_agent", label: "Default Agent", field_type: FieldType::Text, env_var: None, required: false, placeholder: "assistant", advanced: true },
        ],
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "nick", label: "Nickname", field_type: FieldType::Text, env_var: None, required: true, placeholder: "openraw", advanced: false },
            ChannelField { key: "channels", label: "Channels", field_type: FieldType::List, env_var: None, required: false, placeholder: "#openraw, #general", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "nick", label: "Bot Nickname", field_type: FieldType::Text, env_var: None, required: true, placeholder: "openraw", advanced: false },
    ...
    ChannelMeta {
        ...
        setup_type: "form",
        fields: &[
            ChannelField { key: "topic", label: "Topic", field_type: FieldType::Text, env_var: None, required: true, placeholder: "openraw-alerts", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "username", label: "Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "openraw", advanced: false },
...
fn is_channel_configured(config: &openraw_types::config::ChannelsConfig, name: &str) -> bool {
    match name {
...
fn channel_config_values(
    config: &openraw_types::config::ChannelsConfig,
    name: &str,
...
pub async fn configure_channel(
    ...
    let home = openraw_kernel::config::openraw_home();
...
pub async fn whatsapp_qr_start() -> impl IntoResponse {
    ...
    if gateway_url.is_empty() {
        return Json(serde_json::json!({
            ...
            "help": "Run: npx openraw-whatsapp-gateway   (or set WHATSAPP_WEB_GATEWAY_URL)"
        }));
    }
...
pub async fn list_templates() -> impl IntoResponse {
    let agents_dir = openraw_kernel::config::openraw_home().join("agents");
...
pub async fn get_agent_kv(
    ...
    let agent_id = openraw_kernel::kernel::shared_memory_agent_id();
...
pub async fn health(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let shared_id = openraw_types::agent::AgentId(uuid::Uuid::from_bytes([
...
///
/// Returns counters and gauges for monitoring OpenRaw in production:
/// - `openraw_agents_active` — number of active agents
/// - `openraw_uptime_seconds` — seconds since daemon started
/// - `openraw_tokens_total` — total tokens consumed (per agent)
/// - `openraw_tool_calls_total` — total tool calls (per agent)
/// - `openraw_panics_total` — supervisor panic count
/// - `openraw_restarts_total` — supervisor restart count
pub async fn prometheus_metrics(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let mut out = String::with_capacity(2048);

    // Uptime
    let uptime = state.started_at.elapsed().as_secs();
    out.push_str("# HELP openraw_uptime_seconds Time since daemon started.\n");
    out.push_str("# TYPE openraw_uptime_seconds gauge\n");
    out.push_str(&format!("openraw_uptime_seconds {uptime}\n\n"));

    // Active agents
    let agents = state.kernel.registry.list();
    let active = agents
        .iter()
        .filter(|a| matches!(a.state, openraw_types::agent::AgentState::Running))
        .count();
    out.push_str("# HELP openraw_agents_active Number of active agents.\n");
    out.push_str("# TYPE openraw_agents_active gauge\n");
    out.push_str(&format!("openraw_agents_active {active}\n"));
    out.push_str("# HELP openraw_agents_total Total number of registered agents.\n");
    out.push_str("# TYPE openraw_agents_total gauge\n");
    out.push_str(&format!("openraw_agents_total {}\n\n", agents.len()));

    // Per-agent token and tool usage
    out.push_str("# HELP openraw_tokens_total Total tokens consumed (rolling hourly window).\n");
    out.push_str("# TYPE openraw_tokens_total gauge\n");
    out.push_str("# HELP openraw_tool_calls_total Total tool calls (rolling hourly window).\n");
    out.push_str("# TYPE openraw_tool_calls_total gauge\n");
    for agent in &agents {
        let name = &agent.name;
        ...
        if let Some((tokens, tools)) = state.kernel.scheduler.get_usage(agent.id) {
            out.push_str(&format!(
                "openraw_tokens_total{{agent=\"{name}\",provider=\"{provider}\",model=\"{model}\"}} {tokens}\n"
            ));
            out.push_str(&format!(
                "openraw_tool_calls_total{{agent=\"{name}\"}} {tools}\n"
            ));
        }
    }
    out.push('\n');
    ...
    // Supervisor health
    let health = state.kernel.supervisor.health();
    out.push_str("# HELP openraw_panics_total Total supervisor panics since start.\n");
    out.push_str("# TYPE openraw_panics_total counter\n");
    out.push_str(&format!("openraw_panics_total {}\n", health.panic_count));
    out.push_str("# HELP openraw_restarts_total Total supervisor restarts since start.\n");
    out.push_str("# TYPE openraw_restarts_total counter\n");
    out.push_str(&format!(
        "openraw_restarts_total {}\n\n",
        health.restart_count
    ));

    // Version info
    out.push_str("# HELP openraw_info OpenRaw version and build info.\n");
    out.push_str("# TYPE openraw_info gauge\n");
    out.push_str(&format!(
        "openraw_info{{version=\"{}\"}} 1\n",
    ...
    let skills: Vec<serde_json::Value> = registry
        ...
        .map(|s| {
            let source = match &s.manifest.source {
                Some(openraw_skills::SkillSource::ClawHub { slug, version }) => {
                    serde_json::json!({"type": "clawhub", "slug": slug, "version": version})
                }
                Some(openraw_skills::SkillSource::OpenClaw) => {
                    serde_json::json!({"type": "openclaw"})
                }
                Some(openraw_skills::SkillSource::Bundled) => {
                    serde_json::json!({"type": "bundled"})
                }
                Some(openraw_skills::SkillSource::Native) | None => {
...
pub async fn install_skill(
    ...
    let skills_dir = state.kernel.config.home_dir.join("skills");
    let config = openraw_skills::marketplace::MarketplaceConfig::default();
    let client = openraw_skills::marketplace::MarketplaceClient::new(config);
...
pub async fn clawhub_search(
    ...
    let client = openraw_skills::clawhub::ClawHubClient::new(cache_dir);
...
pub async fn clawhub_browse(
    ...
    let sort = match params.get("sort").map(|s| s.as_str()) {
        Some("downloads") => openraw_skills::clawhub::ClawHubSort::Downloads,
        Some("stars") => openraw_skills::clawhub::ClawHubSort::Stars,
        Some("updated") => openraw_skills::clawhub::ClawHubSort::Updated,
        Some("rating") => openraw_skills::clawhub::ClawHubSort::Rating,
        _ => openraw_skills::clawhub::ClawHubSort::Trending,
    };
...
fn clawhub_browse_entry_to_json(
    entry: &openraw_skills::clawhub::ClawHubBrowseEntry,
) -> serde_json::Value {
    let version = openraw_skills::clawhub::ClawHubClient::entry_version(entry);
    serde_json::json!({
        "slug": entry.slug,
...
pub async fn activate_hand(
    State(state): State<Arc<AppState>>,
    Path(hand_id): Path<String>,
    body: Option<Json<openraw_hands::ActivateHandRequest>>,
) -> impl IntoResponse {
            ...
            if let Some(agent_id) = instance.agent_id {
                ...
                if let Some(entry) = entry {
                    if !matches!(
                        ...
                        openraw_types::agent::ScheduleMode::Reactive
                    ) {
...
pub async fn hand_instance_browser(
    ...
    match state
        ...
        .send_command(
            &agent_id_str,
            openraw_runtime::browser::BrowserCommand::ReadPage,
        )
        .await
    {
        Ok(resp) if resp.success => {
            if let Some(data) = &resp.data {
                ...
                if content.len() > 2000 {
                    content = format!("{}... (truncated)", openraw_types::truncate_str(&content, 2000));
                }
            }
        }
    ...
    match state
        ...
        .send_command(
            &agent_id_str,
            openraw_runtime::browser::BrowserCommand::Screenshot,
        )
        .await
    {
        Ok(resp) if resp.success => {
...
pub async fn update_budget(
    ...
    let config_ptr = &state.kernel.config as *const openraw_types::config::KernelConfig
        as *mut openraw_types::config::KernelConfig;
...
pub async fn agent_budget_status(
    ...
    let usage_store = openraw_memory::usage::UsageStore::new(state.kernel.memory.usage_conn());
...
pub async fn delete_session(
    ...
    let session_id = match id.parse::<uuid::Uuid>() {
        Ok(u) => openraw_types::agent::SessionId(u),
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
...
pub async fn set_session_label(
    ...
    if let Some(lbl) = label {
        if let Err(e) = openraw_types::agent::SessionLabel::new(lbl) {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": e.to_string()})),
            );
...
pub async fn find_session_by_label(
    ...
    let agent_id = match agent_id_str.parse::<uuid::Uuid>() {
        Ok(u) => openraw_types::agent::AgentId(u),
        Err(_) => {
            // Try name lookup
...
pub async fn migrate_detect() -> impl IntoResponse {
    match openraw_migrate::openclaw::detect_openclaw_home() {
        Some(path) => {
            let scan = openraw_migrate::openclaw::scan_openclaw_workspace(&path);
            (
                StatusCode::OK,
                Json(serde_json::json!({
                    "detected": true,
...
pub async fn run_migrate(Json(req): Json<MigrateRequest>) -> impl IntoResponse {
    let source = match req.source.as_str() {
        "openclaw" => openraw_migrate::MigrateSource::OpenClaw,
        "langchain" => openraw_migrate::MigrateSource::LangChain,
        "autogpt" => openraw_migrate::MigrateSource::AutoGpt,
        other => {
            return (
                StatusCode::BAD_REQUEST,
                Json(
    ...
    let models: Vec<serde_json::Value> = catalog
        ...
        .filter(|m| {
            ...
            if available_only {
                ...
                if let Some(p) = provider {
                    if p.auth_status == openraw_types::model_catalog::AuthStatus::Missing {
                        return false;
                    }
                }
            }
        ...
        .map(|m| {
            ...
            let available = catalog
                ...
                .map(|p| p.auth_status != openraw_types::model_catalog::AuthStatus::Missing)
                .unwrap_or(m.tier == openraw_types::model_catalog::ModelTier::Custom);
            serde_json::json!({
                "id": m.id,
...
pub async fn list_providers(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let provider_list: Vec<openraw_types::model_catalog::ProviderInfo> = {
        let catalog = state
            .kernel
            .model_catalog
            .read()
    ...
    for p in &provider_list {
        ...
        if !p.key_required {
            ...
            let probe = openraw_runtime::provider_health::probe_provider(&p.id, &p.base_url).await;
...
///
/// Persists to `~/.openraw/custom_models.json` and makes the model immediately
...
pub async fn add_custom_model(
    ...
    let entry = openraw_types::model_catalog::ModelCatalogEntry {
        id: id.clone(),
        display_name: display,
        provider: provider.clone(),
        tier: openraw_types::model_catalog::ModelTier::Custom,
        context_window,
...
pub async fn a2a_agent_card(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    if let Some(first) = agents.first() {
        let card = openraw_runtime::a2a::build_agent_card(&first.manifest, &base_url);
        (
            StatusCode::OK,
        ...
        let card = serde_json::json!({
            "name": "openraw",
            "description": "OpenRaw Agent OS — no agents spawned yet",
...
pub async fn a2a_list_agents(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let cards: Vec<serde_json::Value> = agents
        .iter()
        .map(|entry| {
            let card = openraw_runtime::a2a::build_agent_card(&entry.manifest, &base_url);
...
pub async fn a2a_send_task(
    ...
    let task = openraw_runtime::a2a::A2aTask {
        id: task_id.clone(),
        session_id: session_id.clone(),
        status: openraw_runtime::a2a::A2aTaskStatus::Working,
        messages: vec![openraw_runtime::a2a::A2aMessage {
            role: "user".to_string(),
            parts: vec![openraw_runtime::a2a::A2aPart::Text {
    ...
    match state.kernel.send_message(agent.id, &message_text).await {
        Ok(result) => {
            let response_msg = openraw_runtime::a2a::A2aMessage {
        ...
        Err(e) => {
            let error_msg = openraw_runtime::a2a::A2aMessage {
                role: "agent".to_string(),
                parts: vec![openraw_runtime::a2a::A2aPart::Text {
                    text: format!("Error: {e}"),
                }],
...
pub async fn a2a_discover_external(
    ...
    let client = openraw_runtime::a2a::A2aClient::new();
...
pub async fn mcp_http(
    ...
    let mut tools = builtin_tool_definitions();
    {
        ...
        for skill_tool in registry.all_tool_definitions() {
            tools.push(openraw_types::tool::ToolDefinition {
    ...
    if method == "tools/call" {
        ...
        let kernel_handle: Arc<dyn openraw_runtime::kernel_handle::KernelHandle> =
            state.kernel.clone() as Arc<dyn openraw_runtime::kernel_handle::KernelHandle>;
        let result = openraw_runtime::tool_runner::execute_tool(
            "mcp-http",
            tool_name,
            &arguments,
            Some(&kernel_handle),
    ...
    let response = openraw_runtime::mcp_server::handle_mcp_request(&request, &tools).await;
    Json(response)
}
...
pub async fn switch_agent_session(
    ...
    let session_id = match session_id_str.parse::<uuid::Uuid>() {
        Ok(uuid) => openraw_types::agent::SessionId(uuid),
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": "Invalid session ID"})),
...
pub async fn get_agent_mcp_servers(
    ...
    if let Ok(mcp_tools) = state.kernel.mcp_tools.lock() {
        ...
        for tool in mcp_tools.iter() {
            if let Some(server) = openraw_runtime::mcp::extract_mcp_server(&tool.name) {
...
///
/// SECURITY: Writes to `~/.openraw/secrets.env`, sets env var in process,
...
pub async fn test_provider(
    ...
    let driver_config = openraw_runtime::llm_driver::DriverConfig {
        provider: name.clone(),
        api_key,
    ...
    match openraw_runtime::drivers::create_driver(&driver_config) {
        Ok(driver) => {
            // Send a minimal completion request to test connectivity
            let test_req = openraw_runtime::llm_driver::CompletionRequest {
                model: default_model.clone(),
                messages: vec![openraw_types::message::Message::user("Hi")],
                tools: vec![],
                max_tokens: 1,
                temperature: 0.0,
                system: None,
...
pub async fn set_provider_url(
    ...
    let probe =
        openraw_runtime::provider_health::probe_provider(&name, &base_url).await;
...
pub async fn create_skill(
    ...
    // Write skill.toml to ~/.openraw/skills/{name}/
    ...
    for info in registry.list_all_info() {
        ...
        let status = match &info.installed {
            ...
            Some(_) => match h.as_ref().map(|h| &h.status) {
                Some(openraw_extensions::IntegrationStatus::Ready) => "ready",
                Some(openraw_extensions::IntegrationStatus::Error(_)) => "error",
                _ => "installed",
            },
...
pub async fn add_integration(
    ...
    let install_err = {
        ...
        if registry.is_installed(&id) {
            ...
            let entry = openraw_extensions::InstalledIntegration {
                id: id.clone(),
...
/// Must match the value in `openraw-kernel/src/kernel.rs::shared_memory_agent_id()`.
...
const SCHEDULES_KEY: &str = "__openraw_schedules";
...
pub async fn upload_file(
    ...
    let transcription = if content_type.starts_with("audio/") {
        let attachment = openraw_types::media::MediaAttachment {
            media_type: openraw_types::media::MediaType::Audio,
            mime_type: content_type.clone(),
            source: openraw_types::media::MediaSource::FilePath {
...
pub async fn serve_upload(Path(file_id): Path<String>) -> impl IntoResponse {
    ...
    let file_path = std::env::temp_dir().join("openraw_uploads").join(&file_id);
...
pub async fn create_approval(
    ...
    use openraw_types::approval::{ApprovalRequest, RiskLevel};
...
pub async fn approve_request(
    ...
    match state.kernel.approval_manager.resolve(
        uuid,
        openraw_types::approval::ApprovalDecision::Approved,
        Some("api".to_string()),
    ) {
        Ok(resp) => (
            StatusCode::OK,
...
pub async fn reject_request(
    ...
    match state.kernel.approval_manager.resolve(
        uuid,
        openraw_types::approval::ApprovalDecision::Denied,
        Some("api".to_string()),
    ) {
        Ok(resp) => (
            StatusCode::OK,
...
pub async fn delete_cron_job(
    ...
    match uuid::Uuid::parse_str(&id) {
        Ok(uuid) => {
            let job_id = openraw_types::scheduler::CronJobId(uuid);
...
pub async fn webhook_wake(
    State(state): State<Arc<AppState>>,
    headers: axum::http::HeaderMap,
    Json(body): Json<openraw_types::webhook::WakePayload>,
) -> impl IntoResponse {
...
pub async fn webhook_agent(
    State(state): State<Arc<AppState>>,
    headers: axum::http::HeaderMap,
    Json(body): Json<openraw_types::webhook::AgentHookPayload>,
) -> impl IntoResponse {
    // Check if webhook triggers are enabled
    let wh_config = match &state.kernel.config.webhook_triggers {
        Some(c) if c.enabled => c,
...
pub async fn add_binding(
    State(state): State<Arc<AppState>>,
    Json(binding): Json<openraw_types::config::AgentBinding>,
) -> impl IntoResponse {
...
pub async fn pairing_request(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    match state.kernel.pairing.create_pairing_request() {
        Ok(req) => {
            let qr_uri = format!("openraw://pair?token={}", req.token);
            Json(serde_json::json!({
                "token": req.token,
                "qr_uri": qr_uri,
...
pub async fn pairing_complete(
    ...
    let push_token = body
        ...
        .and_then(|v| v.as_str())
        .map(String::from);
    let device_info = openraw_kernel::pairing::PairedDevice {
...
pub async fn copilot_oauth_start() -> impl IntoResponse {
    ...
    match openraw_runtime::copilot_oauth::start_device_flow().await {
        Ok(resp) => {
...
pub async fn copilot_oauth_poll(
    ...
    match openraw_runtime::copilot_oauth::poll_device_flow(&device_code).await {
        openraw_runtime::copilot_oauth::DeviceFlowStatus::Pending => (
            StatusCode::OK,
            Json(serde_json::json!({"status": "pending"})),
        ),
        openraw_runtime::copilot_oauth::DeviceFlowStatus::Complete { access_token } => {
            // Save to secrets.env
            let secrets_path = state.kernel.config.home_dir.join("secrets.env");
        ...
        openraw_runtime::copilot_oauth::DeviceFlowStatus::SlowDown { new_interval } => {
            // Update interval
        ...
        openraw_runtime::copilot_oauth::DeviceFlowStatus::Expired => {
            COPILOT_FLOWS.remove(&poll_id);
            (
                StatusCode::OK,
                Json(serde_json::json!({"status": "expired"})),
        ...
        openraw_runtime::copilot_oauth::DeviceFlowStatus::AccessDenied => {
            COPILOT_FLOWS.remove(&poll_id);
            (
                StatusCode::OK,
        ...
        openraw_runtime::copilot_oauth::DeviceFlowStatus::Error(e) => (
            StatusCode::OK,
            Json(serde_json::json!({"status": "error", "error": e})),
        ),
    }
...
pub async fn comms_topology(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    use openraw_types::comms::{EdgeKind, TopoEdge, TopoNode, Topology};
    ...
    for event in &events {
        if let openraw_types::event::EventPayload::Message(_) = &event.payload {
            if let openraw_types::event::EventTarget::Agent(target_id) = &event.target {
...
fn filter_to_comms_event(
    event: &openraw_types::event::Event,
    agents: &[openraw_types::agent::AgentEntry],
) -> Option<openraw_types::comms::CommsEvent> {
    use openraw_types::comms::{CommsEvent, CommsEventKind};
    use openraw_types::event::{EventPayload, EventTarget, LifecycleEvent};
    ...
    match &event.payload {
        EventPayload::Message(msg) => {
            ...
            Some(CommsEvent {
                ...
                detail: openraw_types::truncate_str(&msg.content, 200).to_string(),
            })
        }
...
fn audit_to_comms_event(
    entry: &openraw_runtime::audit::AuditEntry,
    agents: &[openraw_types::agent::AgentEntry],
) -> Option<openraw_types::comms::CommsEvent> {
    use openraw_types::comms::{CommsEvent, CommsEventKind};
        ...
        agents
            ...
            .unwrap_or_else(|| {
                if id.is_empty() || id == "system" {
                    ...
                    openraw_types::truncate_str(id, 12).to_string()
                }
            })
    };
        ...
        "AgentMessage" => {
            ...
            let detail = if entry.detail.starts_with("tokens_in=") {
                ...
                if entry.outcome == "ok" {
                    ...
                    format!("{} in / {} out — {}", in_tok, out_tok, openraw_types::truncate_str(&entry.outcome, 80))
                }
            } else if entry.outcome != "ok" {
                format!("{} — {}", openraw_types::truncate_str(&entry.detail, 80), openraw_types::truncate_str(&entry.outcome, 80))
            } else {
                openraw_types::truncate_str(&entry.detail, 200).to_string()
            };
            (CommsEventKind::AgentMessage, detail, "user")
        }
        "AgentSpawn" => (
            CommsEventKind::AgentSpawned,
            format!("Agent spawned: {}", openraw_types::truncate_str(&entry.detail, 100)),
            "",
        ),
        "AgentKill" => (
            CommsEventKind::AgentTerminated,
            format!("Agent killed: {}", openraw_types::truncate_str(&entry.detail, 100)),
            "",
        ),
        _ => return None,
    };
...
pub async fn comms_events(
    ...
    let mut comms_events: Vec<openraw_types::comms::CommsEvent> = bus_events
        .iter()
...
pub async fn comms_send(
    State(state): State<Arc<AppState>>,
    Json(req): Json<openraw_types::comms::CommsSendRequest>,
) -> impl IntoResponse {
    // Validate from agent exists
    let from_id: openraw_types::agent::AgentId = match req.from_agent_id.parse() {
        Ok(id) => id,
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
    ...
    let to_id: openraw_types::agent::AgentId = match req.to_agent_id.parse() {
        Ok(id) => id,
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
...
pub async fn comms_task(
    State(state): State<Arc<AppState>>,
    Json(req): Json<openraw_types::comms::CommsTaskRequest>,
) -> impl IntoResponse {
