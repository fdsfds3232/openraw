...
// ---------------------------------------------------------------------------
// OpenRaw output types (TOML)
// ---------------------------------------------------------------------------

/// OpenRaw config.toml structure for serialization.
#[derive(Serialize)]
struct OpenRawConfig {
    default_model: OpenRawModelConfig,
    memory: OpenRawMemorySection,
    network: OpenRawNetworkSection,
    #[serde(skip_serializing_if = "Option::is_none")]
    channels: Option<toml::Value>,
}

#[derive(Serialize)]
struct OpenRawModelConfig {
    provider: String,
    model: String,
    api_key_env: String,
    #[serde(skip_serializing_if = "Option::is_none")]
...
#[derive(Serialize)]
struct OpenRawMemorySection {
    decay_rate: f32,
}

#[derive(Serialize)]
struct OpenRawNetworkSection {
    listen_addr: String,
}
...
/// Map OpenClaw DM policy to OpenRaw DM policy string.
...
/// Map OpenClaw group policy to OpenRaw group policy string.
...
use openraw_types::tool_compat::{is_known_openraw_tool, map_tool_name};

/// Map OpenClaw tool profile to OpenRaw capability tool list.
...
/// Map OpenClaw provider name to OpenRaw provider name.
...
fn migrate_config_from_json(
    ...
    let of_config = OpenRawConfig {
        ...
        memory: OpenRawMemorySection { decay_rate: 0.05 },
        network: OpenRawNetworkSection {
    ...
    let config_content = format!(
        "# OpenRaw Agent OS configuration\n\
...
fn migrate_channels_from_json(
    ...
    // --- BlueBubbles (skip — no OpenRaw adapter) ---
    if oc_channels.bluebubbles.is_some() {
        report.skipped.push(SkippedItem {
            kind: ItemKind::Channel,
            name: "bluebubbles".to_string(),
            reason: "No OpenRaw adapter available — consider using the iMessage channel instead"
                .to_string(),
        });
    }
    ...
    for key in oc_channels.other.keys() {
        report.skipped.push(SkippedItem {
            kind: ItemKind::Channel,
            name: key.clone(),
            reason: format!("Unknown channel '{key}' — not mapped to any OpenRaw adapter"),
        });
    }
            ...
            Ok((toml_str, unmapped_tools)) => {
                ...
                for tool in &unmapped_tools {
                    report.warnings.push(format!(
                        "Agent '{id}': tool '{tool}' has no OpenRaw equivalent and was skipped"
                    ));
                }
...
fn convert_agent_from_json(
    ...
    let system_prompt = entry
        ...
        .unwrap_or_else(|| {
            format!(
                "You are {display_name}, an AI agent running on the OpenRaw Agent OS. You are helpful, concise, and accurate."
            )
        });
    ...
    toml_str.push_str("author = \"openraw\"\n");
...
fn report_skipped_features(root: &OpenClawRoot, source: &Path, report: &mut MigrationReport) {
    ...
    if root.cron.is_some() {
        report.skipped.push(SkippedItem {
            kind: ItemKind::Config,
            name: "cron".to_string(),
            reason: "Cron job scheduling not yet supported — use OpenRaw's ScheduleMode::Periodic instead".to_string(),
        });
    }
    ...
    if root.hooks.is_some() {
        report.skipped.push(SkippedItem {
            ...
            reason: "Webhook hooks not supported — use OpenRaw's event system instead".to_string(),
        });
    }
        ...
        if let Some(ref entries) = skills.entries {
            if !entries.is_empty() {
                report.skipped.push(SkippedItem {
                    ...
                    reason: "Skills must be reinstalled via `openraw skill install`".to_string(),
                });
            }
        }
    }
    ...
    if source.join("memory-search").join("index.db").exists() {
        report.skipped.push(SkippedItem {
            ...
            reason: "SQLite vector index not portable — OpenRaw will rebuild embeddings"
                .to_string(),
        });
    }
    ...
    if root.session.is_some() {
        report.skipped.push(SkippedItem {
            ...
            reason: "Session scope config differs — OpenRaw uses per-agent sessions by default"
                .to_string(),
        });
    }
    ...
    if root.memory.is_some() {
        report.skipped.push(SkippedItem {
            ...
            reason:
                "Memory backend config not migrated — OpenRaw uses SQLite with vector embeddings"
                    .to_string(),
        });
    }
}
    ...
    for entry in entries {
        ...
        match convert_legacy_agent(&agent_yaml, &agent_name) {
            Ok((toml_str, unmapped_tools)) => {
                ...
                for tool in &unmapped_tools {
                    report.warnings.push(format!(
                        "Agent '{agent_name}': tool '{tool}' has no OpenRaw equivalent and was skipped"
                    ));
                }
...
fn convert_legacy_agent(
    ...
    let system_prompt = oc.system_prompt.unwrap_or_else(|| {
        format!(
            "You are {}, an AI agent running on the OpenRaw Agent OS. {}",
            oc.name,
            ...
            for entry in entries.flatten() {
                ...
                if has_package_json && has_index {
                    report.skipped.push(SkippedItem {
                        kind: ItemKind::Skill,
                        name: name.clone(),
                        reason: "Node.js skill — run with `openraw skill install` after migration"
                            .to_string(),
                    });
                } else {
                    report.skipped.push(SkippedItem {
...
mod tests {
  ...
  agents: {
    ...
    #[test]
    fn test_is_known_openraw_tool() {
        assert!(is_known_openraw_tool("file_read"));
        assert!(is_known_openraw_tool("shell_exec"));
        assert!(is_known_openraw_tool("web_fetch"));
        assert!(!is_known_openraw_tool("Read"));
        assert!(!is_known_openraw_tool("unknown"));
    }
