...
impl MigrationReport {
    ...
    pub fn to_markdown(&self) -> String {
        ...
        out.push_str("1. Review imported agent manifests in `~/.openraw/agents/`\n");
        out.push_str(
            "2. Review `~/.openraw/secrets.env` — verify tokens were migrated correctly\n",
        );
        out.push_str("3. Set any remaining API keys referenced in `~/.openraw/config.toml`\n");
        out.push_str("4. Start the daemon: `openraw start`\n");
        out.push_str("5. Test your agents: `openraw agent list`\n");
    ...
    pub fn print_summary(&self) {
        ...
        if !self.dry_run {
            println!("\n  Next steps:");
            println!("    openraw start");
            println!("    openraw agent list");
        }
    }
}
    ...
    fn test_report_with_items() {
        let report = MigrationReport {
            ...
            imported: vec![MigrateItem {
                ...
                destination: "~/.openraw/agents/coder/agent.toml".to_string(),
            }],
