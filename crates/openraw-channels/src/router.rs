...
use openraw_types::config::{AgentBinding, BroadcastConfig, BroadcastStrategy};
use std::sync::Mutex;
use tracing::warn;
...
pub struct AgentRouter {
    /// Default agent per user (keyed by openraw_user or platform_id).
...
mod tests {
    ...
    fn test_binding_channel_match() {
        ...
        router.load_bindings(&[AgentBinding {
            ...
            match_rule: openraw_types::config::BindingMatchRule {
    ...
    fn test_binding_specificity_scores() {
        use openraw_types::config::BindingMatchRule;
