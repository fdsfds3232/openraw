//! Channel bridge — connects channel adapters to the OpenRaw kernel.
//!
//! Defines `ChannelBridgeHandle` (implemented by openraw-api on the kernel) and
...
use openraw_types::config::{ChannelOverrides, DmPolicy, GroupPolicy, OutputFormat};
use std::sync::Arc;
...
///
/// Defined here to avoid circular deps (openraw-channels can't depend on openraw-kernel).
/// Implemented in openraw-api on the actual kernel.
#[async_trait]
...
async fn dispatch_message(
    ...
    if router.has_broadcast(&message.sender.platform_id) {
        ...
        if !targets.is_empty() {
            ...
            match strategy {
                openraw_types::config::BroadcastStrategy::Parallel => {
                ...
                openraw_types::config::BroadcastStrategy::Sequential => {
                    for (name, maybe_id) in &targets {
                        if let Some(aid) = maybe_id {
    ...
    let agent_id = router.resolve(
        ...
        message.sender.openraw_user.as_deref(),
    );
...
async fn handle_command(
    ...
    match name {
        "start" => {
            ...
            let mut msg = "Welcome to OpenRaw! I connect you to AI agents.\n\nAvailable agents:\n"
                .to_string();
            if agents.is_empty() {
