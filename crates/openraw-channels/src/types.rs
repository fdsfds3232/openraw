...
pub struct ChannelUser {
    ...
    /// Optional mapping to an OpenRaw user identity.
    pub openraw_user: Option<String>,
}
...
// Re-export policy/format types from openraw-types for convenience.
pub use openraw_types::config::{DmPolicy, GroupPolicy, OutputFormat};

/// Trait that every channel adapter must implement.
///
/// A channel adapter bridges a messaging platform to the OpenRaw kernel by converting
...
pub fn split_message(text: &str, max_len: usize) -> Vec<&str> {
    ...
    while !remaining.is_empty() {
        ...
        let safe_end = openraw_types::truncate_str(remaining, max_len).len();
