//! IRC channel adapter for the OpenRaw channel bridge.
//!
...
pub struct IrcAdapter {
    ...
    /// IRC channels to join (e.g., ["#openraw", "#bots"]).
    channels: Vec<String>,
        ...
        *self.write_tx.write().await = Some(write_cmd_tx.clone());
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                registration.push_str(&format!("USER {nick} 0 * :OpenRaw Bot\r\n"));
                    ...
                    tokio::select! {
                        ...
                        _ = shutdown_rx.changed() => {
                            if *shutdown_rx.borrow() {
                                ...
                                let _ = writer.write_all(b"QUIT :OpenRaw shutting down\r\n").await;
                                return;
                            }
                        }
                    }
...
mod tests {
    ...
    fn test_parse_irc_line_privmsg() {
        let line = parse_irc_line(":alice!alice@host PRIVMSG #openraw :Hello everyone!").unwrap();
        assert_eq!(line.prefix.as_deref(), Some("alice!alice@host"));
        assert_eq!(line.command, "PRIVMSG");
        assert_eq!(line.params, vec!["#openraw"]);
    ...
    fn test_parse_irc_line_no_trailing() {
        let line = parse_irc_line(":alice!alice@host JOIN #openraw").unwrap();
    ...
    fn test_parse_privmsg_channel() {
        let line = IrcLine {
            ...
            params: vec!["#openraw".to_string()],
            trailing: Some("Hello from IRC!".to_string()),
        };

        let msg = parse_privmsg(&line, "openraw-bot").unwrap();
        assert_eq!(msg.channel, ChannelType::Custom("irc".to_string()));
        assert_eq!(msg.sender.display_name, "alice");
        assert_eq!(msg.sender.platform_id, "#openraw");
        assert!(msg.is_group);
    ...
    fn test_parse_privmsg_dm() {
        let line = IrcLine {
            ...
            command: "PRIVMSG".to_string(),
            params: vec!["openraw-bot".to_string()],
    ...
    fn test_parse_privmsg_skips_self() {
        let line = IrcLine {
            prefix: Some("openraw-bot!bot@host".to_string()),
            command: "PRIVMSG".to_string(),
            params: vec!["#openraw".to_string()],
            trailing: Some("My own message".to_string()),
        };

        let msg = parse_privmsg(&line, "openraw-bot");
        assert!(msg.is_none());
    }
