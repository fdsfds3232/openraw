//! OpenRaw Desktop — Native Tauri 2.0 wrapper for the OpenRaw Agent OS.
//!
...
use openraw_types::event::{EventPayload, LifecycleEvent, SystemEvent};
use std::sync::Arc;
...
pub fn run() {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "openraw=info,tauri=info".into()),
        )
        .init();

    info!("Starting OpenRaw Desktop...");

    // Boot kernel + embedded server (blocks until port is known)
    let server_handle = server::start_server().expect("Failed to start OpenRaw server");
    let port = server_handle.port;
    let kernel_for_notifications = server_handle.kernel.clone();

    info!("OpenRaw server running on port {port}");
    ...
    builder
        ...
        .setup(move |app| {
                        ...
                        Ok(event) => {
                            let (title, body) = match &event.payload {
                                ...
                                EventPayload::System(SystemEvent::KernelStopping) => (
                                    ...
                                    "OpenRaw kernel is shutting down".to_string(),
                                ),
            ...
            info!("OpenRaw Desktop window created");
            Ok(())
        })
