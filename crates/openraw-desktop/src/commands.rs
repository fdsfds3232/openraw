...
use openraw_kernel::config::openraw_home;
...
/// `~/.openraw/agents/{name}/agent.toml`, then spawns the agent.
#[tauri::command]
pub fn import_agent_toml(
    ...
    let agent_dir = openraw_home().join("agents").join(&agent_name);
...
///
/// Copies the selected file to `~/.openraw/skills/` and triggers a
...
pub fn import_skill_file(
    ...
    let skills_dir = openraw_home().join("skills");
...
/// Open the OpenRaw config directory (`~/.openraw/`) in the OS file manager.
#[tauri::command]
pub fn open_config_dir() -> Result<(), String> {
    let dir = openraw_home();
    std::fs::create_dir_all(&dir).map_err(|e| format!("Failed to create config dir: {e}"))?;
    open::that(&dir).map_err(|e| format!("Failed to open directory: {e}"))
}

/// Open the OpenRaw logs directory (`~/.openraw/logs/`) in the OS file manager.
#[tauri::command]
pub fn open_logs_dir() -> Result<(), String> {
    let dir = openraw_home().join("logs");
