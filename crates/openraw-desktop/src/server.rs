...
//!
//! Boots the OpenRaw kernel, binds to a random localhost port, and runs the
//! API server on a background thread with its own tokio runtime.

use openraw_api::server::build_router;
...
impl ServerHandle {
    ...
    pub fn shutdown(mut self) {
        ...
        info!("OpenRaw embedded server stopped");
    }
}
...
pub fn start_server() -> Result<ServerHandle, Box<dyn std::error::Error>> {
    ...
    let kernel = OpenRawKernel::boot(None)?;
    ...
    info!("OpenRaw embedded server bound to http://127.0.0.1:{port}");

    let (shutdown_tx, shutdown_rx) = watch::channel(false);
    let kernel_clone = kernel.clone();

    let server_thread = std::thread::Builder::new()
        .name("openraw-server".into())
        .spawn(move || {
...
async fn run_embedded_server(
    ...
    info!("OpenRaw embedded server listening on http://{listen_addr}");
