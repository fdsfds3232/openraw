...
use openraw_skills::registry::SkillRegistry;
use openraw_types::taint::{TaintLabel, TaintSink, TaintedValue};
use openraw_types::tool::{ToolDefinition, ToolResult};
...
pub async fn execute_tool(
    ...
    exec_policy: Option<&openraw_types::config::ExecPolicy>,
    tts_engine: Option<&crate::tts::TtsEngine>,
    docker_config: Option<&openraw_types::config::DockerSandboxConfig>,
    ...
    if let Some(kh) = kernel {
        if kh.requires_approval(tool_name) {
            ...
            let summary = format!(
                "{}: {}",
                tool_name,
                openraw_types::truncate_str(&input_str, 200)
            );
    ...
    let result = match tool_name {
        ...
        "shell_exec" => {
            ...
            let is_full_exec = exec_policy
                .is_some_and(|p| p.mode == openraw_types::config::ExecSecurityMode::Full);
            if !is_full_exec {
        ...
        other => {
            ...
            // Fallback 2: Skill registry tool providers
                ...
                if let Some(skill) = registry.find_tool_provider(other) {
                    ...
                    match openraw_skills::loader::execute_skill_tool(
                        &skill.manifest,
                        &skill.path,
                        other,
                        input,
    ...
    vec![
        ...
        ToolDefinition {
            ...
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "url": { "type": "string", "description": "Base URL of the remote OpenRaw/A2A-compatible agent (e.g., 'https://agent.example.com')" }
                },
                "required": ["url"]
            }),
        },
...
async fn tool_web_search_legacy(input: &serde_json::Value) -> Result<String, String> {
    ...
    let resp = client
        ...
        .query(&[("q", query)])
        .header("User-Agent", "Mozilla/5.0 (compatible; OpenRawAgent/0.1)")
        .send()
        .await
...
fn parse_entity_type(s: &str) -> openraw_types::memory::EntityType {
    use openraw_types::memory::EntityType;
...
fn parse_relation_type(s: &str) -> openraw_types::memory::RelationType {
    use openraw_types::memory::RelationType;
    match s.to_lowercase().as_str() {
...
async fn tool_knowledge_add_entity(
    ...
    let entity = openraw_types::memory::Entity {
...
async fn tool_knowledge_add_relation(
    ...
    let relation = openraw_types::memory::Relation {
...
async fn tool_knowledge_query(
    ...
    let pattern = openraw_types::memory::GraphPattern {
        source,
        relation,
        target,
        max_depth,
...
async fn tool_media_describe(
    ...
    let attachment = openraw_types::media::MediaAttachment {
        media_type: openraw_types::media::MediaType::Image,
        mime_type: mime.to_string(),
        source: openraw_types::media::MediaSource::Base64 {
...
async fn tool_image_generate(
    ...
    let model = match model_str {
        "dall-e-3" | "dalle3" | "dalle-3" => openraw_types::media::ImageGenModel::DallE3,
        "dall-e-2" | "dalle2" | "dalle-2" => openraw_types::media::ImageGenModel::DallE2,
        "gpt-image-1" | "gpt_image_1" => openraw_types::media::ImageGenModel::GptImage1,
        _ => {
            return Err(format!(
    ...
    let request = openraw_types::media::ImageGenRequest {
...
async fn tool_speech_to_text(
    ...
    use openraw_types::media::{MediaAttachment, MediaSource, MediaType};
...
mod tests {
    ...
    async fn test_canvas_present_tool() {
        ...
        let tmp = std::env::temp_dir().join("openraw_canvas_test");
