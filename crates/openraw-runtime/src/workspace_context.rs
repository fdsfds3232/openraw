...
//! context files (AGENTS.md, SOUL.md, TOOLS.md, IDENTITY.md, HEARTBEAT.md), and OpenRaw
...
pub struct WorkspaceContext {
    ...
    /// Whether .openraw/ directory exists.
    pub has_openraw_dir: bool,
...
impl WorkspaceContext {
    ...
    pub fn detect(root: &Path) -> Self {
        ...
        let has_openraw_dir = root.join(".openraw").exists();
...
/// Persistent workspace state, saved to `.openraw/workspace-state.json`.
...
mod tests {
    ...
    fn test_detect_rust_project() {
        let dir = std::env::temp_dir().join("openraw_ws_rust_test");
    ...
    fn test_detect_node_project() {
        let dir = std::env::temp_dir().join("openraw_ws_node_test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_detect_python_project() {
        let dir = std::env::temp_dir().join("openraw_ws_py_test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_detect_go_project() {
        let dir = std::env::temp_dir().join("openraw_ws_go_test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_detect_unknown_project() {
        let dir = std::env::temp_dir().join("openraw_ws_unk_test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_workspace_context_detect() {
        let dir = std::env::temp_dir().join("openraw_ws_ctx_test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_get_file_cache_hit() {
        let dir = std::env::temp_dir().join("openraw_ws_cache_test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_file_size_cap() {
        let dir = std::env::temp_dir().join("openraw_ws_cap_test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_build_context_section() {
        let dir = std::env::temp_dir().join("openraw_ws_section_test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(dir.join("Cargo.toml"), "[package]").unwrap();
        std::fs::create_dir_all(dir.join(".git")).unwrap();
    ...
    fn test_workspace_state_round_trip() {
        let dir = std::env::temp_dir().join("openraw_ws_state_test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_workspace_state_missing_file() {
        let dir = std::env::temp_dir().join("openraw_ws_state_missing");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
