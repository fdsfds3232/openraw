...
use openraw_types::config::{ExecPolicy, ExecSecurityMode};
...
/// This is the preferred way to clean up subprocesses spawned by OpenRaw.
...
pub async fn wait_or_kill_with_idle(
    ...
    grace_ms: u64,
) -> Result<(openraw_types::config::TerminationReason, String), String> {
    ...
    loop {
        ...
        if tokio::time::Instant::now() >= deadline {
            ...
            kill_child_tree(child, grace_ms).await?;
            return Ok((
                openraw_types::config::TerminationReason::AbsoluteTimeout,
                output,
            ));
        }
        ...
        if let Some(idle_dl) = idle_deadline {
            if tokio::time::Instant::now() >= idle_dl {
                ...
                kill_child_tree(child, grace_ms).await?;
                return Ok((
                    openraw_types::config::TerminationReason::NoOutputTimeout,
                    output,
                ));
            }
        }
                    ...
                    Ok(0) => {
                        ...
                        if stderr.is_none() {
                            ...
                            match tokio::time::timeout(
                                ...
                                Ok(Ok(status)) => {
                                    return Ok((
                                        openraw_types::config::TerminationReason::Exited(status.code().unwrap_or(-1)),
                                        output,
                                    ));
                                }
                                ...
                                Err(_) => {
                                    kill_child_tree(child, grace_ms).await?;
                                    return Ok((openraw_types::config::TerminationReason::AbsoluteTimeout, output));
                                }
                            }
                        }
                    }
