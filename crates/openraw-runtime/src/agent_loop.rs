...
use openraw_memory::session::Session;
...
pub struct AgentLoopResult {
    ...
    pub directives: openraw_types::message::ReplyDirectives,
}

/// Run the agent execution loop for a single user message.
///
/// This is the core of OpenRaw: it loads session context, recalls memories,
...
pub async fn run_agent_loop(
    ...
    if let Some(hook_reg) = hooks {
        let ctx = crate::hooks::HookContext {
            ...
            event: openraw_types::agent::HookEvent::BeforePromptBuild,
            ...
            StopReason::EndTurn | StopReason::StopSequence => {
                ...
                if text.trim() == "NO_REPLY" || parsed_directives.silent {
                    ...
                    return Ok(AgentLoopResult {
                        ...
                        cost_usd: None,
                        silent: true,
                        directives: openraw_types::message::ReplyDirectives {
                ...
                if let Some(hook_reg) = hooks {
                    let ctx = crate::hooks::HookContext {
                        agent_name: &manifest.name,
                        agent_id: agent_id_str.as_str(),
                        event: openraw_types::agent::HookEvent::AgentLoopEnd,
                        data: serde_json::json!({
                            "iterations": iteration + 1,
                ...
                for tool_call in &response.tool_calls {
                    ...
                    match &verdict {
                        LoopGuardVerdict::CircuitBreak(msg) => {
                            ...
                            return Err(OpenRawError::Internal(msg.clone()));
                        }
                    ...
                    if let Some(hook_reg) = hooks {
                        let ctx = crate::hooks::HookContext {
                            ...
                            event: openraw_types::agent::HookEvent::BeforeToolCall,
                            data: serde_json::json!({
                    ...
                    if let Some(hook_reg) = hooks {
                        let ctx = crate::hooks::HookContext {
                            ...
                            event: openraw_types::agent::HookEvent::AfterToolCall,
                            data: serde_json::json!({
                                "tool_name": &tool_call.name,
    ...
    Err(OpenRawError::MaxIterationsExceeded(max_iterations))
}
...
) -> OpenRawResult<crate::llm_driver::CompletionResponse> {
    ...
    if let (Some(provider), Some(cooldown)) = (provider, cooldown) {
        match cooldown.check(provider) {
            CooldownVerdict::Reject {
                ...
                return Err(OpenRawError::LlmDriver(format!(
    ...
    for attempt in 0..=MAX_RETRIES {
        match driver.complete(request.clone()).await {
            ...
            Err(e) => {
                ...
                return Err(OpenRawError::LlmDriver(user_msg));
            }
        }
    }

    Err(OpenRawError::LlmDriver(
...
mod tests {
    ...
    async fn test_empty_response_after_tool_use_returns_fallback() {
        let memory = openraw_memory::MemorySubstrate::open_in_memory(0.01).unwrap();
        let agent_id = openraw_types::agent::AgentId::new();
        let mut session = openraw_memory::session::Session {
            id: openraw_types::agent::SessionId::new(),
            agent_id,
            messages: Vec::new(),
