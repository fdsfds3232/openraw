...
//! - `build_agent_card` — expose OpenRaw agents via A2A
...
pub struct AgentCard {
    ...
    /// Skills this agent can perform (A2A skill descriptors, not OpenRaw skills).
...
/// A2A skill descriptor (not an OpenRaw skill — describes a capability).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub async fn discover_external_agents(
    agents: &[openraw_types::config::ExternalAgent],
...
// ---------------------------------------------------------------------------
// A2A Server — expose OpenRaw agents via A2A
// ---------------------------------------------------------------------------

/// Build an A2A Agent Card from an OpenRaw agent manifest.
...
impl A2aClient {
    ...
    pub async fn discover(&self, url: &str) -> Result<AgentCard, String> {
        ...
        let response = self
            .client
            .get(&agent_json_url)
            .header("User-Agent", "OpenRaw/0.1 A2A")
            .send()
            .await
...
mod tests {
    ...
    fn test_a2a_config_serde() {
        use openraw_types::config::{A2aConfig, ExternalAgent};
