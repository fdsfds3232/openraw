...
/// Result of converting a SKILL.md into OpenRaw format.
#[derive(Debug, Clone)]
pub struct ConvertedSkillMd {
    ...
    /// Tool name translations applied (openclaw_name → openraw_name).
...
/// Full conversion of a SKILL.md directory to OpenRaw format.
///
...
pub fn convert_skillmd(dir: &Path) -> Result<ConvertedSkillMd, SkillError> {
    ...
    if let Some(ref meta) = frontmatter.metadata.openclaw {
        ...
        // Convert commands to OpenRaw tool definitions
        for cmd in &meta.commands {
            ...
            let openraw_name = if let Some(mapped) = tool_compat::map_tool_name(&cmd.name) {
                tool_translations.push((cmd.name.clone(), mapped.to_string()));
                mapped.to_string()
            } else if tool_compat::is_known_openraw_tool(&cmd.name) {
                cmd.name.clone()
            } else {
...
/// Convert an in-memory SKILL.md string into OpenRaw format.
///
...
/// Convert an OpenClaw Node.js skill directory into an OpenRaw SkillManifest.
///
...
/// Write an OpenRaw skill.toml manifest for an OpenClaw skill.
pub fn write_openraw_manifest(dir: &Path, manifest: &SkillManifest) -> Result<(), SkillError> {
