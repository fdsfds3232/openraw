...
//!
//! Run with: GROQ_API_KEY=gsk_... cargo test -p openraw-kernel --test multi_agent_test -- --nocapture
...
fn test_config() -> KernelConfig {
    let tmp = std::env::temp_dir().join("openraw-multi-agent-test");
...
async fn test_six_agent_fleet() {
    ...
    let kernel = OpenRawKernel::boot_with_config(test_config()).expect("Kernel should boot");
...
"#,
    ...
    println!("\n{}", "=".repeat(60));
    println!("  OPENRAW MULTI-AGENT FLEET TEST");
