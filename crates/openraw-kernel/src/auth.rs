...
//! to OpenRaw users with roles, then enforces permission checks on actions.

use dashmap::DashMap;
use openraw_types::agent::UserId;
use openraw_types::config::UserConfig;
...
pub struct AuthManager {
    /// Known users by their OpenRaw user ID.
...
impl AuthManager {
    ...
    ///
    /// Returns the OpenRaw UserId if a matching channel binding exists,
    ...
    pub fn authorize(&self, user_id: UserId, action: &Action) -> OpenRawResult<()> {
        let identity = self
            .users
            .get(&user_id)
            .ok_or_else(|| OpenRawError::AuthDenied("Unknown user".to_string()))?;

        let required = action.required_role();
        if identity.role >= required {
            Ok(())
        } else {
            Err(OpenRawError::AuthDenied(format!(
