...
//!
//! Embeds the gateway JS at compile time, extracts it to `~/.openraw/whatsapp-gateway/`,
//! runs `npm install` if needed, and spawns `node index.js` as a managed child process
//! that auto-restarts on crash.

use crate::config::openraw_home;
...
fn gateway_dir() -> PathBuf {
    openraw_home().join("whatsapp-gateway")
}
...
pub async fn start_whatsapp_gateway(kernel: &Arc<super::kernel::OpenRawKernel>) {
    ...
    let openraw_url = format!("http://{api_listen}");
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let child = tokio::process::Command::new(node_cmd)
                ...
                .env("OPENRAW_URL", &openraw_url)
                .env("OPENRAW_DEFAULT_AGENT", &default_agent)
...
mod tests {
    ...
    fn test_embedded_files_not_empty() {
        ...
        assert!(GATEWAY_PACKAGE_JSON.contains("@openraw/whatsapp-gateway"));
    }
    ...
    #[test]
    fn test_gateway_dir_under_openraw_home() {
        let dir = gateway_dir();
        assert!(dir.ends_with("whatsapp-gateway"));
        assert!(dir.parent().unwrap().to_string_lossy().contains(".openraw"));
    }

    #[test]
    fn test_write_if_changed_creates_new_file() {
        let tmp = std::env::temp_dir().join("openraw_test_gateway");
