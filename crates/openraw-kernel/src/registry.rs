...
use dashmap::DashMap;
use openraw_types::agent::{AgentEntry, AgentId, AgentMode, AgentState};
...
impl AgentRegistry {
    ...
    pub fn register(&self, entry: AgentEntry) -> OpenRawResult<()> {
        if self.name_index.contains_key(&entry.name) {
            return Err(OpenRawError::AgentAlreadyExists(entry.name.clone()));
        }
        let id = entry.id;
    ...
    /// Update agent state.
    pub fn set_state(&self, id: AgentId, state: AgentState) -> OpenRawResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| OpenRawError::AgentNotFound(id.to_string()))?;
        entry.state = state;
    ...
    pub fn set_mode(&self, id: AgentId, mode: AgentMode) -> OpenRawResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| OpenRawError::AgentNotFound(id.to_string()))?;
    ...
    pub fn remove(&self, id: AgentId) -> OpenRawResult<AgentEntry> {
        let (_, entry) = self
            .agents
            .remove(&id)
            .ok_or_else(|| OpenRawError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_session_id(
        &self,
        id: AgentId,
        new_session_id: openraw_types::agent::SessionId,
    ) -> OpenRawResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_identity(
        &self,
        id: AgentId,
        identity: openraw_types::agent::AgentIdentity,
    ) -> OpenRawResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_model(&self, id: AgentId, new_model: String) -> OpenRawResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| OpenRawError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_skills(&self, id: AgentId, skills: Vec<String>) -> OpenRawResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| OpenRawError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_mcp_servers(&self, id: AgentId, servers: Vec<String>) -> OpenRawResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| OpenRawError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_system_prompt(&self, id: AgentId, new_prompt: String) -> OpenRawResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| OpenRawError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_name(&self, id: AgentId, new_name: String) -> OpenRawResult<()> {
        if self.name_index.contains_key(&new_name) {
            return Err(OpenRawError::AgentAlreadyExists(new_name));
        }
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_description(&self, id: AgentId, new_desc: String) -> OpenRawResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| OpenRawError::AgentNotFound(id.to_string()))?;
    ...
    pub fn mark_onboarding_complete(&self, id: AgentId) -> OpenRawResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| OpenRawError::AgentNotFound(id.to_string()))?;
