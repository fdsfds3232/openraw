//! OpenRawKernel — assembles all subsystems and provides the main API.
...
use openraw_memory::MemorySubstrate;
use openraw_runtime::agent_loop::{
    run_agent_loop, run_agent_loop_streaming, strip_provider_prefix, AgentLoopResult,
};
use openraw_runtime::audit::AuditLog;
use openraw_runtime::drivers;
use openraw_runtime::kernel_handle::{self, KernelHandle};
use openraw_runtime::llm_driver::{
    CompletionRequest, CompletionResponse, DriverConfig, LlmDriver, LlmError, StreamEvent,
};
use openraw_runtime::python_runtime::{self, PythonConfig};
use openraw_runtime::routing::ModelRouter;
use openraw_runtime::sandbox::{SandboxConfig, WasmSandbox};
use openraw_runtime::tool_runner::builtin_tool_definitions;
use openraw_types::agent::*;
use openraw_types::capability::Capability;
use openraw_types::config::KernelConfig;
use openraw_types::error::OpenRawError;
use openraw_types::event::*;
use openraw_types::memory::Memory;
use openraw_types::tool::ToolDefinition;
...
/// The main OpenRaw kernel — coordinates all subsystems.
...
pub struct OpenRawKernel {
    ...
    pub model_catalog: std::sync::RwLock<openraw_runtime::model_catalog::ModelCatalog>,
    /// Skill registry for plugin skills (RwLock for hot-reload on install/uninstall).
    pub skill_registry: std::sync::RwLock<openraw_skills::registry::SkillRegistry>,
    ...
    pub mcp_connections: tokio::sync::Mutex<Vec<openraw_runtime::mcp::McpConnection>>,
    ...
    pub a2a_task_store: openraw_runtime::a2a::A2aTaskStore,
    /// Discovered external A2A agent cards.
    pub a2a_external_agents: std::sync::Mutex<Vec<(String, openraw_runtime::a2a::AgentCard)>>,
    /// Web tools context (multi-provider search + SSRF-protected fetch + caching).
    pub web_ctx: openraw_runtime::web_search::WebToolsContext,
    /// Browser automation manager (Playwright bridge sessions).
    pub browser_ctx: openraw_runtime::browser::BrowserManager,
    /// Media understanding engine (image description, audio transcription).
    pub media_engine: openraw_runtime::media_understanding::MediaEngine,
    /// Text-to-speech engine.
    pub tts_engine: openraw_runtime::tts::TtsEngine,
    ...
    pub embedding_driver:
        Option<Arc<dyn openraw_runtime::embedding::EmbeddingDriver + Send + Sync>>,
    /// Hand registry — curated autonomous capability packages.
    pub hand_registry: openraw_hands::registry::HandRegistry,
    /// Extension/integration registry (bundled MCP templates + install state).
    pub extension_registry: std::sync::RwLock<openraw_extensions::registry::IntegrationRegistry>,
    /// Integration health monitor.
    pub extension_health: openraw_extensions::health::HealthMonitor,
    /// Effective MCP server list (manual config + extension-installed, merged at boot).
    pub effective_mcp_servers: std::sync::RwLock<Vec<openraw_types::config::McpServerConfigEntry>>,
    ...
    pub bindings: std::sync::Mutex<Vec<openraw_types::config::AgentBinding>>,
    /// Broadcast configuration.
    pub broadcast: openraw_types::config::BroadcastConfig,
    /// Auto-reply engine.
    pub auto_reply_engine: crate::auto_reply::AutoReplyEngine,
    /// Plugin lifecycle hook registry.
    pub hooks: openraw_runtime::hooks::HookRegistry,
    /// Persistent process manager for interactive sessions (REPLs, servers).
    pub process_manager: Arc<openraw_runtime::process_manager::ProcessManager>,
    /// OFP peer registry — tracks connected peers.
    pub peer_registry: Option<openraw_wire::PeerRegistry>,
    /// OFP peer node — the local networking node.
    pub peer_node: Option<Arc<openraw_wire::PeerNode>>,
    ...
    pub channel_adapters: dashmap::DashMap<String, Arc<dyn openraw_channels::types::ChannelAdapter>>,
    /// Hot-reloadable default model override (set via config hot-reload, read at agent spawn).
    pub default_model_override: std::sync::RwLock<Option<openraw_types::config::DefaultModelConfig>>,
    /// Weak self-reference for trigger dispatch (set after Arc wrapping).
    self_handle: OnceLock<Weak<OpenRawKernel>>,
}

/// Bounded in-memory delivery receipt tracker.
/// Stores up to `MAX_RECEIPTS` most recent delivery receipts per agent.
pub struct DeliveryTracker {
    receipts: dashmap::DashMap<AgentId, Vec<openraw_channels::types::DeliveryReceipt>>,
}
...
impl DeliveryTracker {
    ...
    pub fn record(&self, agent_id: AgentId, receipt: openraw_channels::types::DeliveryReceipt) {
    ...
    pub fn get_receipts(
        ...
        agent_id: AgentId,
        limit: usize,
    ) -> Vec<openraw_channels::types::DeliveryReceipt> {
        self.receipts
            .get(&agent_id)
    ...
    pub fn sent_receipt(
        channel: &str,
        recipient: &str,
    ) -> openraw_channels::types::DeliveryReceipt {
        openraw_channels::types::DeliveryReceipt {
            message_id: uuid::Uuid::new_v4().to_string(),
            channel: channel.to_string(),
            recipient: Self::sanitize_recipient(recipient),
            status: openraw_channels::types::DeliveryStatus::Sent,
    ...
    pub fn failed_receipt(
        ...
        openraw_channels::types::DeliveryReceipt {
            ...
            channel: channel.to_string(),
            recipient: Self::sanitize_recipient(recipient),
            status: openraw_channels::types::DeliveryStatus::Failed,
            timestamp: chrono::Utc::now(),
...
fn ensure_workspace(workspace: &Path) -> KernelResult<()> {
    for subdir in &["data", "output", "sessions", "skills", "logs", "memory"] {
        std::fs::create_dir_all(workspace.join(subdir)).map_err(|e| {
            KernelError::OpenRaw(OpenRawError::Internal(format!(
...
fn append_daily_memory_log(workspace: &Path, response: &str) {
    ...
    let summary = openraw_types::truncate_str(trimmed, 500);
...
fn read_identity_file(workspace: &Path, filename: &str) -> Option<String> {
    ...
    if content.len() > MAX_IDENTITY_FILE_BYTES {
        Some(openraw_types::truncate_str(&content, MAX_IDENTITY_FILE_BYTES).to_string())
    } else {
        Some(content)
    }
}
...
impl OpenRawKernel {
    ...
    pub fn boot_with_config(mut config: KernelConfig) -> KernelResult<Self> {
        use openraw_types::config::KernelMode;

        // Env var overrides — useful for Docker where config.toml is baked in.
        if let Ok(listen) = std::env::var("OPENRAW_LISTEN") {
        ...
        match config.mode {
            KernelMode::Stable => {
                info!("Booting OpenRaw kernel in STABLE mode — conservative defaults enforced");
            }
            KernelMode::Dev => {
                warn!("Booting OpenRaw kernel in DEV mode — experimental features enabled");
            }
            KernelMode::Default => {
                info!("Booting OpenRaw kernel...");
            }
        }
        ...
        let db_path = config
            ...
            .sqlite_path
            .clone()
            .unwrap_or_else(|| config.data_dir.join("openraw.db"));
        let memory = Arc::new(
        ...
        let driver: Arc<dyn LlmDriver> = if driver_chain.len() > 1 {
            Arc::new(openraw_runtime::drivers::fallback::FallbackDriver::new(
                driver_chain,
            ))
        ...
        let metering = Arc::new(MeteringEngine::new(Arc::new(
            openraw_memory::usage::UsageStore::new(memory.usage_conn()),
        )));
        ...
        let mut model_catalog = openraw_runtime::model_catalog::ModelCatalog::new();
        ...
        // Load user's custom models from ~/.openraw/custom_models.json
        ...
        let mut skill_registry = openraw_skills::registry::SkillRegistry::new(skills_dir);
        ...
        let hand_registry = openraw_hands::registry::HandRegistry::new();
        ...
        let mut extension_registry =
            openraw_extensions::registry::IntegrationRegistry::new(&config.home_dir);
        ...
        let health_config = openraw_extensions::health::HealthMonitorConfig {
        ...
        let extension_health = openraw_extensions::health::HealthMonitor::new(health_config);
        ...
        let web_cache = Arc::new(openraw_runtime::web_cache::WebCache::new(cache_ttl));
        let web_ctx = openraw_runtime::web_search::WebToolsContext {
            search: openraw_runtime::web_search::WebSearchEngine::new(
                config.web.clone(),
                web_cache.clone(),
            ),
            fetch: openraw_runtime::web_fetch::WebFetchEngine::new(
        ...
        let embedding_driver: Option<
            Arc<dyn openraw_runtime::embedding::EmbeddingDriver + Send + Sync>,
        > = {
            use openraw_runtime::embedding::create_embedding_driver;
        ...
        let browser_ctx = openraw_runtime::browser::BrowserManager::new(config.browser.clone());

        // Initialize media understanding engine
        let media_engine =
            openraw_runtime::media_understanding::MediaEngine::new(config.media.clone());
        let tts_engine = openraw_runtime::tts::TtsEngine::new(config.tts.clone());
        ...
        let kernel = Self {
            ...
            a2a_task_store: openraw_runtime::a2a::A2aTaskStore::default(),
            ...
            auto_reply_engine,
            hooks: openraw_runtime::hooks::HookRegistry::new(),
            process_manager: Arc::new(openraw_runtime::process_manager::ProcessManager::new(5)),
            peer_registry: None,
            peer_node: None,
                ...
                for entry in agents {
                    ...
                    if toml_path.exists() {
                        match std::fs::read_to_string(&toml_path) {
                            Ok(toml_str) => {
                                match toml::from_str::<openraw_types::agent::AgentManifest>(
                                    &toml_str,
                                ) {
                                    Ok(disk_manifest) => {
        ...
        if kernel.registry.list().is_empty() {
            ...
            let manifest = AgentManifest {
                ...
                model: openraw_types::agent::ModelConfig {
        ...
        info!("OpenRaw kernel booted successfully");
        Ok(kernel)
    }
    ...
    pub fn spawn_agent_with_parent(
        ...
        self.audit_log.record(
            agent_id.to_string(),
            openraw_runtime::audit::AuditAction::AgentSpawn,
    ...
    pub fn verify_signed_manifest(&self, signed_json: &str) -> KernelResult<String> {
        let signed: openraw_types::manifest_signing::SignedManifest =
            serde_json::from_str(signed_json).map_err(|e| {
                KernelError::OpenRaw(openraw_types::error::OpenRawError::Config(format!(
    ...
    pub async fn send_message_with_handle(
        ...
        let entry = self.registry.get(agent_id).ok_or_else(|| {
            KernelError::OpenRaw(OpenRawError::AgentNotFound(agent_id.to_string()))
        })?;
        ...
        match result {
            Ok(result) => {
                ...
                self.audit_log.record(
                    agent_id.to_string(),
                    openraw_runtime::audit::AuditAction::AgentMessage,
                    format!(
    ...
    pub fn send_message_streaming(
                    ...
                    Ok(result) => {
                        ...
                        let _ = tx
                            .send(StreamEvent::ContentComplete {
                                stop_reason: openraw_types::message::StopReason::EndTurn,
        ...
        let mut session = self
            ...
            .map_err(KernelError::OpenRaw)?
            .unwrap_or_else(|| openraw_memory::session::Session {
                id: entry.session_id,
                agent_id,
                messages: Vec::new(),
        ...
        // Build the structured system prompt via prompt_builder
        {
            ...
            let prompt_ctx = openraw_runtime::prompt_builder::PromptContext {
                ...
                workspace_context: manifest.workspace.as_ref().map(|w| {
                    let mut ws_ctx =
                        openraw_runtime::workspace_context::WorkspaceContext::detect(w);
            ...
            manifest.model.system_prompt =
                openraw_runtime::prompt_builder::build_system_prompt(&prompt_ctx);
            ...
            if let Some(cc_msg) =
                openraw_runtime::prompt_builder::build_canonical_context_message(&prompt_ctx)
            {
        ...
        let message_owned = if let Some(link_ctx) =
            openraw_runtime::link_understanding::build_link_context(message, &self.config.links)
        {
        ...
        let handle = tokio::spawn(async move {
            ...
            let phase_cb: openraw_runtime::agent_loop::PhaseCallback =
                std::sync::Arc::new(move |phase| {
                    use openraw_runtime::agent_loop::LoopPhase;
    ...
    async fn execute_wasm_agent(
        ...
        Ok(AgentLoopResult {
            response,
            total_usage: openraw_types::message::TokenUsage {
                input_tokens: 0,
                output_tokens: 0,
            },
            iterations: 1,
    ...
    async fn execute_llm_agent(
        ...
        let is_stable = self.config.mode == openraw_types::config::KernelMode::Stable;
        ...
        if is_stable {
            ...
            let probe = CompletionRequest {
                ...
                messages: vec![openraw_types::message::Message::user(message)],
                tools: tools.clone(),
        ...
        let _ = self.metering.record(&openraw_memory::usage::UsageRecord {
            agent_id,
            model: model.clone(),
        ...
        match self.config.usage_footer {
            openraw_types::config::UsageFooterMode::Off => {
                result.cost_usd = None;
            }
            openraw_types::config::UsageFooterMode::Cost
            | openraw_types::config::UsageFooterMode::Full => {
                result.cost_usd = if cost > 0.0 { Some(cost) } else { None };
            }
            openraw_types::config::UsageFooterMode::Tokens => {
    ...
    pub fn switch_agent_session(
        ...
        let session = self
            ...
            .map_err(KernelError::OpenRaw)?
            .ok_or_else(|| {
                KernelError::OpenRaw(OpenRawError::Internal("Session not found".to_string()))
            })?;

        if session.agent_id != agent_id {
            return Err(KernelError::OpenRaw(OpenRawError::Internal(
    ...
    fn save_session_summary(
        ...
        agent_id: AgentId,
        entry: &AgentEntry,
        session: &openraw_memory::session::Session,
    ) {
        use openraw_types::message::{MessageContent, Role};
        ...
        if !skills.is_empty() {
            ...
            for name in &skills {
                if !known.contains(name) {
                    return Err(KernelError::OpenRaw(OpenRawError::Internal(format!(
                        "Unknown skill: {name}"
                    ))));
                }
            }
        ...
        if !servers.is_empty() {
            if let Ok(mcp_tools) = self.mcp_tools.lock() {
                ...
                for tool in mcp_tools.iter() {
                    if let Some(s) = openraw_runtime::mcp::extract_mcp_server(&tool.name) {
                        known_servers.insert(s.to_string());
                    }
                }
                for name in &servers {
                    let normalized = openraw_runtime::mcp::normalize_name(name);
        ...
        let (input_tokens, output_tokens) = session
            .map(|s| {
                ...
                for msg in &s.messages {
                    ...
                    let tokens = len / 4;
                    match msg.role {
                        openraw_types::message::Role::User => input += tokens,
                        openraw_types::message::Role::Assistant => output += tokens,
                        openraw_types::message::Role::System => input += tokens,
                    }
                }
                (input, output)
            })
    ...
    pub async fn compact_agent_session(&self, agent_id: AgentId) -> KernelResult<String> {
        use openraw_runtime::compactor::{compact_session, needs_compaction, CompactionConfig};
        ...
        let result = compact_session(driver, &model, &session, &config)
            .await
            .map_err(|e| KernelError::OpenRaw(OpenRawError::Internal(e)))?;
        ...
        let (repaired_messages, repair_stats) =
            openraw_runtime::session_repair::validate_and_repair_with_stats(&result.kept_messages);
    ...
    pub fn context_report(
        &self,
        agent_id: AgentId,
    ) -> KernelResult<openraw_runtime::compactor::ContextReport> {
        use openraw_runtime::compactor::generate_context_report;
        use openraw_runtime::tool_runner::builtin_tool_definitions;
    ...
    pub fn kill_agent(&self, agent_id: AgentId) -> KernelResult<()> {
        ...
        self.audit_log.record(
            agent_id.to_string(),
            openraw_runtime::audit::AuditAction::AgentKill,
    ...
    pub fn activate_hand(
    ...
    ) -> KernelResult<openraw_hands::HandInstance> {
        use openraw_hands::HandError;

        let def = self
            .hand_registry
            .get_definition(hand_id)
            .ok_or_else(|| {
                KernelError::OpenRaw(OpenRawError::AgentNotFound(format!(
        ...
        let instance = self
            ...
            .map_err(|e| match e {
                HandError::AlreadyActive(id) => KernelError::OpenRaw(OpenRawError::Internal(
                    format!("Hand already active: {id}"),
                )),
                other => KernelError::OpenRaw(OpenRawError::Internal(other.to_string())),
            })?;
        ...
        let mut manifest = AgentManifest {
            ...
            exec_policy: if def.tools.iter().any(|t| t == "shell_exec") {
                Some(openraw_types::config::ExecPolicy {
                    mode: openraw_types::config::ExecSecurityMode::Full,
        ...
        let resolved = openraw_hands::resolve_settings(&def.settings, &instance.config);
        ...
        for req in &def.requires {
            match req.requirement_type {
                openraw_hands::RequirementType::ApiKey
                | openraw_hands::RequirementType::EnvVar => {
        ...
        self.hand_registry
            ...
            .map_err(|e| KernelError::OpenRaw(OpenRawError::Internal(e.to_string())))?;
    ...
    pub fn list_bindings(&self) -> Vec<openraw_types::config::AgentBinding> {
        self.bindings
            .lock()
            .unwrap_or_else(|e| e.into_inner())
            .clone()
    ...
    pub fn add_binding(&self, binding: openraw_types::config::AgentBinding) {
    ...
    pub fn remove_binding(&self, index: usize) -> Option<openraw_types::config::AgentBinding> {
        let mut bindings = self.bindings.lock().unwrap_or_else(|e| e.into_inner());
    ...
    fn apply_hot_actions(
        ...
        new_config: &openraw_types::config::KernelConfig,
    ) {
    ...
    pub fn register_trigger(
        ...
        if self.registry.get(agent_id).is_none() {
            return Err(KernelError::OpenRaw(OpenRawError::AgentNotFound(
                agent_id.to_string(),
            )));
        }
        Ok(self
    ...
    pub async fn run_workflow(
        ...
        let run_id = self
            ...
            .await
            .ok_or_else(|| {
                KernelError::OpenRaw(OpenRawError::Internal("Workflow not found".to_string()))
            })?;
        ...
        .map_err(|e| {
            KernelError::OpenRaw(OpenRawError::Internal(format!("Workflow failed: {e}")))
        })?;
    ...
    pub fn start_background_agents(self: &Arc<Self>) {
        ...
        let mut bg_agents: Vec<(openraw_types::agent::AgentId, String, ScheduleMode)> =
            Vec::new();
        ...
        // Probe local providers for reachability and model discovery
        {
            ...
            tokio::spawn(async move {
                ...
                for (provider_id, base_url) in &local_providers {
                    let result =
                        openraw_runtime::provider_health::probe_provider(provider_id, base_url)
                            .await;
                    if result.reachable {
                        info!(
            ...
            tokio::spawn(async move {
                ...
                loop {
                    ...
                    for job in due {
                        ...
                        match &job.action {
                            openraw_types::scheduler::CronAction::SystemEvent { text } => {
                            ...
                            openraw_types::scheduler::CronAction::AgentTurn {
                                message,
                                timeout_secs,
                                ..
                            } => {
                                ...
                                let kh: std::sync::Arc<dyn openraw_runtime::kernel_handle::KernelHandle> = kernel.clone();
        ...
        if let Some(ref a2a_config) = self.config.a2a {
            if a2a_config.enabled && !a2a_config.external_agents.is_empty() {
                ...
                tokio::spawn(async move {
                    let discovered = openraw_runtime::a2a::discover_external_agents(&agents).await;
    ...
    async fn start_ofp_node(self: &Arc<Self>) {
        use openraw_wire::{PeerConfig, PeerNode, PeerRegistry};
        ...
        let node_name = gethostname().unwrap_or_else(|| "openraw-node".to_string());
        ...
        let handle: Arc<dyn openraw_wire::peer::PeerHandle> = self.self_arc();
        ...
        match PeerNode::start(peer_config, registry.clone(), handle.clone()).await {
            Ok((node, _accept_task)) => {
                ...
                let self_ptr = Arc::as_ptr(self) as *mut OpenRawKernel;
                unsafe {
    ...
    pub fn shutdown(&self) {
        info!("Shutting down OpenRaw kernel...");
        ...
        info!(
            "OpenRaw kernel shut down ({} agents preserved)",
    ...
    fn resolve_driver(&self, manifest: &AgentManifest) -> KernelResult<Arc<dyn LlmDriver>> {
        ...
        if !manifest.fallback_models.is_empty() {
            ...
            let mut chain: Vec<(std::sync::Arc<dyn openraw_runtime::llm_driver::LlmDriver>, String)> =
            ...
            if chain.len() > 1 {
                return Ok(Arc::new(
                    openraw_runtime::drivers::fallback::FallbackDriver::with_models(chain),
                ));
            }
        }
    ...
    async fn connect_mcp_servers(self: &Arc<Self>) {
        use openraw_runtime::mcp::{McpConnection, McpServerConfig, McpTransport};
        use openraw_types::config::McpTransportEntry;
    ...
    fn available_tools(&self, agent_id: AgentId) -> Vec<ToolDefinition> {
        ...
        if let Ok(mcp_tools) = self.mcp_tools.lock() {
            if mcp_allowlist.is_empty() {
                ...
                let normalized: Vec<String> = mcp_allowlist
                    .iter()
                    .map(|s| openraw_runtime::mcp::normalize_name(s))
                    .collect();
                all_tools.extend(
                    mcp_tools
                        .iter()
                        .filter(|t| {
                            openraw_runtime::mcp::extract_mcp_server(&t.name)
        ...
        let exec_blocks_shell = entry.as_ref().is_some_and(|e| {
            e.manifest
                .exec_policy
                .as_ref()
                .is_some_and(|p| p.mode == openraw_types::config::ExecSecurityMode::Deny)
        });
    ...
    pub fn reload_skills(&self) {
        ...
        let mut fresh = openraw_skills::registry::SkillRegistry::new(skills_dir);
...
fn apply_budget_defaults(
    budget: &openraw_types::config::BudgetConfig,
...
async fn cron_deliver_response(
    ...
    agent_id: AgentId,
    response: &str,
    delivery: &openraw_types::scheduler::CronDelivery,
) {
    use openraw_types::scheduler::CronDelivery;
...
#[async_trait]
impl KernelHandle for OpenRawKernel {
    async fn spawn_agent(
        ...
        let content_hash = openraw_types::manifest_signing::hash_manifest(manifest_toml);
    ...
    fn kill_agent(&self, agent_id: &str) -> Result<(), String> {
        ...
        OpenRawKernel::kill_agent(self, id).map_err(|e| format!("Kill failed: {e}"))
    }
    ...
    async fn publish_event(
        ...
        OpenRawKernel::publish_event(self, event).await;
        Ok(())
    }

    async fn knowledge_add_entity(
        &self,
        entity: openraw_types::memory::Entity,
    ) -> Result<String, String> {
        self.memory
            .add_entity(entity)
            .await
    ...
    async fn knowledge_add_relation(
        &self,
        relation: openraw_types::memory::Relation,
    ) -> Result<String, String> {
        self.memory
    ...
    async fn knowledge_query(
        &self,
        pattern: openraw_types::memory::GraphPattern,
    ) -> Result<Vec<openraw_types::memory::GraphMatch>, String> {
        self.memory
            .query_graph(pattern)
            .await
    ...
    async fn cron_create(
        ...
        let aid = openraw_types::agent::AgentId(
    ...
    async fn cron_cancel(&self, job_id: &str) -> Result<(), String> {
        let id = openraw_types::scheduler::CronJobId(
    ...
    async fn request_approval(
        ...
        use openraw_types::approval::{ApprovalDecision, ApprovalRequest as TypedRequest};
    ...
    async fn send_channel_message(
        ...
        let user = openraw_channels::types::ChannelUser {
        ...
        adapter
            .send(&user, openraw_channels::types::ChannelContent::Text(message.to_string()))
            .await
    ...
    async fn spawn_agent_checked(
        ...
        manifest_toml: &str,
        parent_id: Option<&str>,
        parent_caps: &[openraw_types::capability::Capability],
    ) -> Result<(String, String), String> {
        ...
        openraw_types::capability::validate_capability_inheritance(parent_caps, &child_caps)?;
...
#[async_trait]
impl openraw_wire::peer::PeerHandle for OpenRawKernel {
    fn local_agents(&self) -> Vec<openraw_wire::message::RemoteAgentInfo> {
        self.registry
            .list()
            .iter()
            .map(|entry| openraw_wire::message::RemoteAgentInfo {
    ...
    fn discover_agents(&self, query: &str) -> Vec<openraw_wire::message::RemoteAgentInfo> {
        let q = query.to_lowercase();
        self.registry
            .list()
            .iter()
