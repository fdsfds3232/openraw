...
pub struct OpenRawKernel {
    ...
    pub channel_adapters:
        dashmap::DashMap<String, Arc<dyn openraw_channels::types::ChannelAdapter>>,
    /// Hot-reloadable default model override (set via config hot-reload, read at agent spawn).
    pub default_model_override:
        std::sync::RwLock<Option<openraw_types::config::DefaultModelConfig>>,
...
impl OpenRawKernel {
    ...
    pub fn activate_hand(
        ...
        for req in &def.requires {
            match req.requirement_type {
                openraw_hands::RequirementType::ApiKey | openraw_hands::RequirementType::EnvVar => {
    ...
    pub fn start_background_agents(self: &Arc<Self>) {
        ...
        let mut bg_agents: Vec<(openraw_types::agent::AgentId, String, ScheduleMode)> = Vec::new();
                    ...
                    for job in due {
                        ...
                        match &job.action {
                            ...
                            openraw_types::scheduler::CronAction::AgentTurn {
                                ...
                                let kh: std::sync::Arc<
                                    dyn openraw_runtime::kernel_handle::KernelHandle,
                                > = kernel.clone();
    ...
    fn resolve_driver(&self, manifest: &AgentManifest) -> KernelResult<Arc<dyn LlmDriver>> {
        ...
        if !manifest.fallback_models.is_empty() {
            ...
            let mut chain: Vec<(
                std::sync::Arc<dyn openraw_runtime::llm_driver::LlmDriver>,
                String,
...
impl KernelHandle for OpenRawKernel {
    ...
    async fn send_channel_message(
        ...
        adapter
            .send(
                &user,
                openraw_channels::types::ChannelContent::Text(message.to_string()),
            )
            .await
