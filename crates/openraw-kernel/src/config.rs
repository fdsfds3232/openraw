//! Configuration loading from `~/.openraw/config.toml` with defaults.
//!
...
///
/// Respects `OPENRAW_HOME` env var (e.g. `OPENRAW_HOME=/opt/openraw`).
pub fn default_config_path() -> PathBuf {
    openraw_home().join("config.toml")
}

/// Get the OpenRaw home directory.
///
/// Priority: `OPENRAW_HOME` env var > `~/.openraw`.
pub fn openraw_home() -> PathBuf {
