...
use openraw_memory::usage::{ModelUsage, UsageRecord, UsageStore, UsageSummary};
use openraw_types::agent::{AgentId, ResourceQuota};
...
impl MeteringEngine {
    ...
    pub fn check_quota(&self, agent_id: AgentId, quota: &ResourceQuota) -> OpenRawResult<()> {
        // Hourly check
        if quota.max_cost_per_hour_usd > 0.0 {
            ...
            if hourly_cost >= quota.max_cost_per_hour_usd {
                return Err(OpenRawError::QuotaExceeded(format!(
    ...
    pub fn budget_status(&self, budget: &openraw_types::config::BudgetConfig) -> BudgetStatus {
    ...
    pub fn get_summary(&self, agent_id: Option<AgentId>) -> OpenRawResult<UsageSummary> {
        self.store.query_summary(agent_id)
    }

    /// Get usage grouped by model.
    pub fn get_by_model(&self) -> OpenRawResult<Vec<ModelUsage>> {
    ...
    pub fn estimate_cost_with_catalog(
        catalog: &openraw_runtime::model_catalog::ModelCatalog,
        model: &str,
        input_tokens: u64,
        output_tokens: u64,
    ) -> f64 {
    ...
    pub fn cleanup(&self, days: u32) -> OpenRawResult<usize> {
