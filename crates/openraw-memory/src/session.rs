...
use chrono::Utc;
use openraw_types::agent::{AgentId, SessionId};
use openraw_types::error::{OpenRawError, OpenRawResult};
use openraw_types::message::{ContentBlock, Message, MessageContent, Role};
...
impl SessionStore {
        ...
        match result {
            Ok((agent_str, messages_blob, tokens, label)) => {
                ...
                let messages: Vec<Message> = rmp_serde::from_slice(&messages_blob)
                    .map_err(|e| OpenRawError::Serialization(e.to_string()))?;
                Ok(Some(Session {
                    id: session_id,
                    agent_id,
                    messages,
    ...
    pub fn list_sessions(&self) -> OpenRawResult<Vec<serde_json::Value>> {
        ...
        for row in rows {
            sessions.push(row.map_err(|e| OpenRawError::Memory(e.to_string()))?);
        }
        Ok(sessions)
    }
...
impl SessionStore {
    ...
    pub fn load_canonical(&self, agent_id: AgentId) -> OpenRawResult<CanonicalSession> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenRawError::Internal(e.to_string()))?;
    ...
    ) -> OpenRawResult<CanonicalSession> {
        let mut canonical = self.load_canonical(agent_id)?;
            ...
            if to_compact > canonical.compaction_cursor {
                ...
                for msg in compacting {
                    let role = match msg.role {
                        openraw_types::message::Role::User => "User",
                        openraw_types::message::Role::Assistant => "Assistant",
                        openraw_types::message::Role::System => "System",
                    };
                    let text = msg.content.text_content();
                    if !text.is_empty() {
                        // Truncate individual messages in summary to keep it compact (UTF-8 safe)
                        let truncated = if text.len() > 200 {
                            format!("{}...", openraw_types::truncate_str(&text, 200))
                        } else {
                            text
                        };
    ...
    ) -> OpenRawResult<(Option<String>, Vec<Message>)> {
    ...
    fn save_canonical(&self, canonical: &CanonicalSession) -> OpenRawResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenRawError::Internal(e.to_string()))?;
...
mod tests {
    ...
    fn test_jsonl_mirror_write() {
        ...
        session
            .messages
            .push(openraw_types::message::Message::user("Hello"));
        session
            .messages
            .push(openraw_types::message::Message::assistant("Hi there!"));
        store.save_session(&session).unwrap();
