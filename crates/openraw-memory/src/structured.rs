...
use chrono::Utc;
use openraw_types::agent::{AgentEntry, AgentId};
...
impl StructuredStore {
    ...
    pub fn get(&self, agent_id: AgentId, key: &str) -> OpenRawResult<Option<serde_json::Value>> {
        let conn = self
            .conn
            .lock()
    ...
    pub fn set(&self, agent_id: AgentId, key: &str, value: serde_json::Value) -> OpenRawResult<()> {
        let conn = self
            ...
            .map_err(|e| OpenRawError::Internal(e.to_string()))?;
        let blob =
            serde_json::to_vec(&value).map_err(|e| OpenRawError::Serialization(e.to_string()))?;
    ...
    pub fn delete(&self, agent_id: AgentId, key: &str) -> OpenRawResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenRawError::Internal(e.to_string()))?;
        ...
        match result {
            Ok((name, manifest_blob, state_str, created_str, session_id_str)) => {
                ...
                let session_id = session_id_str
                    ...
                    .map(openraw_types::agent::SessionId)
                    .unwrap_or_else(openraw_types::agent::SessionId::new);
                Ok(Some(AgentEntry {
                    id: agent_id,
                    name,
                    manifest,
    ...
    pub fn load_all_agents(&self) -> OpenRawResult<Vec<AgentEntry>> {
        ...
        for row in rows {
            ...
            let manifest: openraw_types::agent::AgentManifest = match rmp_serde::from_slice(
                &manifest_blob,
            ) {
                Ok(m) => m,
                Err(e) => {
    ...
    pub fn list_agents(&self) -> OpenRawResult<Vec<(String, String, String)>> {
        ...
        let mut agents = Vec::new();
        for row in rows {
            agents.push(row.map_err(|e| OpenRawError::Memory(e.to_string()))?);
        }
        Ok(agents)
    }
}
