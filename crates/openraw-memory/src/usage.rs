...
impl UsageStore {
    ...
    pub fn record(&self, record: &UsageRecord) -> OpenRawResult<()> {
        let conn = self
            .conn
            .lock()
    ...
    pub fn query_hourly(&self, agent_id: AgentId) -> OpenRawResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenRawError::Internal(e.to_string()))?;
    ...
    pub fn query_daily(&self, agent_id: AgentId) -> OpenRawResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenRawError::Internal(e.to_string()))?;
    ...
    pub fn query_monthly(&self, agent_id: AgentId) -> OpenRawResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenRawError::Internal(e.to_string()))?;
    ...
    pub fn query_global_hourly(&self) -> OpenRawResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenRawError::Internal(e.to_string()))?;
    ...
    pub fn query_global_monthly(&self) -> OpenRawResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenRawError::Internal(e.to_string()))?;
    ...
    pub fn query_summary(&self, agent_id: Option<AgentId>) -> OpenRawResult<UsageSummary> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenRawError::Internal(e.to_string()))?;
    ...
    pub fn query_by_model(&self) -> OpenRawResult<Vec<ModelUsage>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenRawError::Internal(e.to_string()))?;
        ...
        for row in rows {
            results.push(row.map_err(|e| OpenRawError::Memory(e.to_string()))?);
        }
        Ok(results)
    }

    /// Query daily usage breakdown for the last N days.
    pub fn query_daily_breakdown(&self, days: u32) -> OpenRawResult<Vec<DailyBreakdown>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenRawError::Internal(e.to_string()))?;
    ...
    pub fn query_first_event_date(&self) -> OpenRawResult<Option<String>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenRawError::Internal(e.to_string()))?;
    ...
    pub fn query_today_cost(&self) -> OpenRawResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenRawError::Internal(e.to_string()))?;
    ...
    pub fn cleanup_old(&self, days: u32) -> OpenRawResult<usize> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenRawError::Internal(e.to_string()))?;
