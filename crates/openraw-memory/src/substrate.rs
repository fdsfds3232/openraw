...
use openraw_types::agent::{AgentEntry, AgentId, SessionId};
use openraw_types::error::{OpenRawError, OpenRawResult};
use openraw_types::memory::{
...
impl MemorySubstrate {
    ...
    pub fn open(db_path: &Path, decay_rate: f32) -> OpenRawResult<Self> {
        let conn = Connection::open(db_path).map_err(|e| OpenRawError::Memory(e.to_string()))?;
        conn.execute_batch("PRAGMA journal_mode=WAL; PRAGMA busy_timeout=5000;")
            .map_err(|e| OpenRawError::Memory(e.to_string()))?;
    ...
    pub fn save_agent(&self, entry: &AgentEntry) -> OpenRawResult<()> {
        self.structured.save_agent(entry)
    }

    /// Load an agent entry from persistent storage.
    pub fn load_agent(&self, agent_id: AgentId) -> OpenRawResult<Option<AgentEntry>> {
        self.structured.load_agent(agent_id)
    }

    /// Remove an agent from persistent storage and cascade-delete sessions.
    pub fn remove_agent(&self, agent_id: AgentId) -> OpenRawResult<()> {
    ...
    pub fn load_all_agents(&self) -> OpenRawResult<Vec<AgentEntry>> {
        self.structured.load_all_agents()
    }

    /// List all saved agents.
    pub fn list_agents(&self) -> OpenRawResult<Vec<(String, String, String)>> {
    ...
    pub fn structured_get(
        ...
        agent_id: AgentId,
        key: &str,
    ) -> OpenRawResult<Option<serde_json::Value>> {
        self.structured.get(agent_id, key)
    }

    /// List all KV pairs for an agent.
    pub fn list_kv(&self, agent_id: AgentId) -> OpenRawResult<Vec<(String, serde_json::Value)>> {
        self.structured.list_kv(agent_id)
    }

    /// Delete a KV entry for an agent.
    pub fn structured_delete(&self, agent_id: AgentId, key: &str) -> OpenRawResult<()> {
    ...
    /// Get a session by ID.
    pub fn get_session(&self, session_id: SessionId) -> OpenRawResult<Option<Session>> {
        self.sessions.get_session(session_id)
    }

    /// Save a session.
    pub fn save_session(&self, session: &Session) -> OpenRawResult<()> {
        self.sessions.save_session(session)
    }

    /// Create a new empty session for an agent.
    pub fn create_session(&self, agent_id: AgentId) -> OpenRawResult<Session> {
        self.sessions.create_session(agent_id)
    }

    /// List all sessions with metadata.
    pub fn list_sessions(&self) -> OpenRawResult<Vec<serde_json::Value>> {
        self.sessions.list_sessions()
    }

    /// Delete a session by ID.
    pub fn delete_session(&self, session_id: SessionId) -> OpenRawResult<()> {
        self.sessions.delete_session(session_id)
    }

    /// Delete all sessions belonging to an agent.
    pub fn delete_agent_sessions(&self, agent_id: AgentId) -> OpenRawResult<()> {
        self.sessions.delete_agent_sessions(agent_id)
    }

    /// Delete the canonical (cross-channel) session for an agent.
    pub fn delete_canonical_session(&self, agent_id: AgentId) -> OpenRawResult<()> {
    ...
    pub fn find_session_by_label(
        ...
        agent_id: AgentId,
        label: &str,
    ) -> OpenRawResult<Option<Session>> {
        self.sessions.find_session_by_label(agent_id, label)
    }

    /// List all sessions for a specific agent.
    pub fn list_agent_sessions(&self, agent_id: AgentId) -> OpenRawResult<Vec<serde_json::Value>> {
    ...
    ) -> OpenRawResult<(Option<String>, Vec<openraw_types::message::Message>)> {
    ...
    pub fn store_llm_summary(
        ...
        agent_id: AgentId,
        summary: &str,
        kept_messages: Vec<openraw_types::message::Message>,
    ) -> OpenRawResult<()> {
        self.sessions
    ...
    pub fn append_canonical(
        &self,
        agent_id: AgentId,
        messages: &[openraw_types::message::Message],
    ...
    pub fn load_paired_devices(&self) -> OpenRawResult<Vec<serde_json::Value>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenRawError::Memory(e.to_string()))?;
        ...
        for row in rows {
            devices.push(row.map_err(|e| OpenRawError::Memory(e.to_string()))?);
        }
        Ok(devices)
    }
    ...
    pub fn remove_paired_device(&self, device_id: &str) -> OpenRawResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenRawError::Memory(e.to_string()))?;
    ...
    ) -> OpenRawResult<Vec<MemoryFragment>> {
        self.semantic
            .recall_with_embedding(query, limit, filter, query_embedding)
    }

    /// Update the embedding for an existing memory.
    pub fn update_embedding(&self, id: MemoryId, embedding: &[f32]) -> OpenRawResult<()> {
    ...
    pub async fn recall_with_embedding_async(
        ...
        .await
        .map_err(|e| OpenRawError::Internal(e.to_string()))?
    }
    ...
    pub async fn task_post(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let db = conn.lock().map_err(|e| OpenRawError::Internal(e.to_string()))?;
            db.execute(
    ...
    pub async fn task_claim(&self, agent_id: &str) -> OpenRawResult<Option<serde_json::Value>> {
        let conn = Arc::clone(&self.conn);
        ...
        tokio::task::spawn_blocking(move || {
            ...
            match result {
                ...
                Err(e) => Err(OpenRawError::Memory(e.to_string())),
            }
        })
        .await
        .map_err(|e| OpenRawError::Internal(e.to_string()))?
    ...
    pub async fn task_complete(&self, task_id: &str, result: &str) -> OpenRawResult<()> {
        let conn = Arc::clone(&self.conn);
        ...
        tokio::task::spawn_blocking(move || {
            ...
            if rows == 0 {
                return Err(OpenRawError::Internal(format!("Task not found: {task_id}")));
            }
            Ok(())
        })
        .await
    ...
    pub async fn task_list(&self, status: Option<&str>) -> OpenRawResult<Vec<serde_json::Value>> {
        let conn = Arc::clone(&self.conn);
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let mut stmt = db.prepare(sql).map_err(|e| OpenRawError::Memory(e.to_string()))?;
            ...
            for row in rows {
                tasks.push(row.map_err(|e| OpenRawError::Memory(e.to_string()))?);
            }
            Ok(tasks)
        })
        .await
...
impl Memory for MemorySubstrate {
    async fn get(&self, agent_id: AgentId, key: &str) -> OpenRawResult<Option<serde_json::Value>> {
    ...
    async fn delete(&self, agent_id: AgentId, key: &str) -> OpenRawResult<()> {
        let store = self.structured.clone();
        let key = key.to_string();
    ...
    async fn forget(&self, id: MemoryId) -> OpenRawResult<()> {
        let store = self.semantic.clone();
    ...
    async fn add_entity(&self, entity: Entity) -> OpenRawResult<String> {
    ...
    async fn add_relation(&self, relation: Relation) -> OpenRawResult<String> {
        let store = self.knowledge.clone();
    ...
    async fn query_graph(&self, pattern: GraphPattern) -> OpenRawResult<Vec<GraphMatch>> {
        let store = self.knowledge.clone();
    ...
    async fn consolidate(&self) -> OpenRawResult<ConsolidationReport> {
    ...
    async fn export(&self, format: ExportFormat) -> OpenRawResult<Vec<u8>> {
        let _ = format;
        Ok(Vec::new())
    }

    async fn import(&self, _data: &[u8], _format: ExportFormat) -> OpenRawResult<ImportReport> {
        Ok(ImportReport {
            entities_imported: 0,
