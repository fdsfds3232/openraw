...
use openraw_types::memory::{MemoryFilter, MemoryFragment, MemoryId, MemorySource};
...
impl SemanticStore {
    ...
    pub fn recall_with_embedding(
        ...
        for row_result in rows {
            let (
                ...
                access_count,
                embedding_bytes,
            ) = row_result.map_err(|e| OpenRawError::Memory(e.to_string()))?;

            let id = uuid::Uuid::parse_str(&id_str)
                .map(MemoryId)
                .map_err(|e| OpenRawError::Memory(e.to_string()))?;
            let agent_id = uuid::Uuid::parse_str(&agent_str)
                .map(openraw_types::agent::AgentId)
                .map_err(|e| OpenRawError::Memory(e.to_string()))?;
    ...
    pub fn forget(&self, id: MemoryId) -> OpenRawResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenRawError::Internal(e.to_string()))?;
