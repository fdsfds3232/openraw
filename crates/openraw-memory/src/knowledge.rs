...
use openraw_types::memory::{Entity, EntityType, GraphMatch, GraphPattern, Relation, RelationType};
...
impl KnowledgeStore {
    ...
    pub fn add_entity(&self, entity: Entity) -> OpenRawResult<String> {
        let conn = self
            .conn
            .lock()
    ...
    pub fn add_relation(&self, relation: Relation) -> OpenRawResult<String> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenRawError::Internal(e.to_string()))?;
    ...
    pub fn query_graph(&self, pattern: GraphPattern) -> OpenRawResult<Vec<GraphMatch>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| OpenRawError::Internal(e.to_string()))?;
        ...
        for row_result in rows {
            let r = row_result.map_err(|e| OpenRawError::Memory(e.to_string()))?;
