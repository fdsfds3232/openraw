//! Shared tool name mappings between OpenClaw and OpenRaw.
//!
//! These mappings are used by both the migration engine and the skill system
//! to normalize OpenClaw tool names into OpenRaw equivalents.

/// Map an OpenClaw tool name to its OpenRaw equivalent.
///
/// Returns `None` if the name has no known mapping (may already be
/// an OpenRaw tool name — check with [`is_known_openraw_tool`]).
...
/// Check if a tool name is a known OpenRaw built-in tool.
pub fn is_known_openraw_tool(name: &str) -> bool {
    matches!(
        name,
        "file_read"
            | "file_write"
...
mod tests {
    ...
    fn test_is_known_openraw_tool() {
        ...
        for tool in &known {
            assert!(is_known_openraw_tool(tool), "Expected {tool} to be known");
        }

        // Unknown
        assert!(!is_known_openraw_tool("unknown"));
        assert!(!is_known_openraw_tool("Read"));
        assert!(!is_known_openraw_tool("Bash"));
    }
}
