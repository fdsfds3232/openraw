//! Configuration types for the OpenRaw kernel.
...
impl Default for WebhookTriggerConfig {
    fn default() -> Self {
        Self {
            enabled: false,
            token_env: "OPENRAW_WEBHOOK_TOKEN".to_string(),
...
pub struct DockerSandboxConfig {
    ...
    pub image: String,
    /// Container name prefix. Default: "openraw-sandbox".
...
impl Default for DockerSandboxConfig {
    fn default() -> Self {
        Self {
            ...
            container_prefix: "openraw-sandbox".to_string(),
...
pub struct VaultConfig {
    ...
    pub enabled: bool,
    /// Custom vault file path (default: ~/.openraw/vault.enc).
...
pub struct KernelConfig {
    /// OpenRaw home directory (default: ~/.openraw).
    pub home_dir: PathBuf,
    /// Data directory for databases (default: ~/.openraw/data).
    ...
    /// Root directory for agent workspaces. Default: `~/.openraw/workspaces`
    #[serde(default)]
...
/// Resolve the OpenRaw home directory.
///
/// Priority: `OPENRAW_HOME` env var > `~/.openraw`.
fn openraw_home_dir() -> PathBuf {
...
pub struct MatrixConfig {
    ...
    /// Bot user ID (e.g., "@openraw:matrix.org").
    pub user_id: String,
...
pub struct IrcConfig {
    ...
    /// Channels to join (e.g., `["#openraw", "#general"]`).
...
mod tests {
    ...
    fn test_validate_missing_env_vars() {
        ...
        config.channels.discord = Some(DiscordConfig {
            bot_token_env: "OPENRAW_TEST_NONEXISTENT_VAR_DC".to_string(),
            ..Default::default()
        });
        let warnings = config.validate();
