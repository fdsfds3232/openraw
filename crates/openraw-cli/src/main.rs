...
"#;
    ...
    // --- Check 14: Extension registry health ---
    {
        ...
        let mut ext_registry =
            openraw_extensions::registry::IntegrationRegistry::new(&openraw_dir);
...
fn cmd_hand_install(path: &str) {
    ...
    println!("Use `openraw hand activate {}` to start it.", body["id"].as_str().unwrap_or("?"));
}
...
fn cmd_scaffold(kind: ScaffoldKind) {
    ...
    let result = match kind {
        ScaffoldKind::Skill => {
            openraw_extensions::installer::scaffold_skill(&cwd.join("my-skill"))
        }
