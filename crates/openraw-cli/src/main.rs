//! OpenRaw CLI — command-line interface for the OpenRaw Agent OS.
//!
//! When a daemon is running (`openraw start`), the CLI talks to it over HTTP.
...
use openraw_api::server::read_daemon_info;
use openraw_kernel::OpenRawKernel;
use openraw_types::agent::{AgentId, AgentManifest};
...
\x1b[1;36mExamples:\x1b[0m
  openraw init                 Initialize config and data directories
  openraw start                Start the kernel daemon
  openraw tui                  Launch the interactive terminal dashboard
  openraw chat                 Quick chat with the default agent
  openraw agent new coder      Spawn a new agent from a template
  openraw models list          Browse available LLM models
  openraw add github           Install the GitHub integration
  openraw doctor               Run diagnostic health checks
  openraw channel setup        Interactive channel setup wizard
  openraw cron list            List scheduled jobs
  openraw uninstall            Completely remove OpenRaw from your system

\x1b[1;36mQuick Start:\x1b[0m
  1. openraw init              Set up config + API key
  2. openraw start             Launch the daemon
  3. openraw chat              Start chatting!

\x1b[1;36mMore:\x1b[0m
  Docs:       https://github.com/RightNow-AI/openraw
  Dashboard:  http://127.0.0.1:4200/ (when daemon is running)";

/// OpenRaw — the open-source Agent Operating System.
#[derive(Parser)]
#[command(
    name = "openraw",
    version,
    about = "\u{1F40D} OpenRaw \u{2014} Open-source Agent Operating System",
    long_about = "\u{1F40D} OpenRaw \u{2014} Open-source Agent Operating System\n\n\
...
#[derive(Subcommand)]
enum Commands {
    /// Initialize OpenRaw (create ~/.openraw/ and default config).
    Init {
        /// Quick mode: no prompts, just write config + .env (for CI/scripts).
        #[arg(long)]
        quick: bool,
    },
    /// Start the OpenRaw kernel daemon (API server + kernel).
    Start,
    ...
    /// Migrate from another agent framework to OpenRaw.
    Migrate(MigrateArgs),
    ...
    /// Completely uninstall OpenRaw from your system.
    Uninstall {
...
enum ConfigCommands {
    ...
    /// Save an API key to ~/.openraw/.env (prompts interactively).
    SetKey {
        /// Provider name (groq, anthropic, openai, gemini, deepseek, etc.).
        provider: String,
    },
    /// Remove an API key from ~/.openraw/.env.
    DeleteKey {
        /// Provider name.
        provider: String,
    },
...
/// Get the OpenRaw home directory, respecting OPENRAW_HOME env var.
fn cli_openraw_home() -> std::path::PathBuf {
    if let Ok(home) = std::env::var("OPENRAW_HOME") {
...
fn main() {
    // Load ~/.openraw/.env into process environment (system env takes priority).
    dotenv::load_dotenv();
    ...
    match resp {
        Ok(r) => {
            ...
            if status.is_server_error() {
                ui::error_with_fix(
                    ...
                    "Check daemon logs: ~/.openraw/tui.log",
                );
            }
            body
        }
        Err(e) => {
            ...
            if msg.contains("timed out") || msg.contains("Timeout") {
                ui::error_with_fix(
                    "Request timed out",
                    "The agent may be processing a complex request. Try again, or check `openraw status`",
                );
            } else if msg.contains("Connection refused") || msg.contains("connect") {
                ui::error_with_fix(
                    "Cannot connect to daemon",
                    "Is the daemon running? Start it with: openraw start",
                );
            } else {
                ui::error_with_fix(
                    &format!("Daemon communication error: {msg}"),
                    "Check `openraw status` or restart: openraw start",
                );
            }
            std::process::exit(1);
        }
...
fn cmd_init(quick: bool) {
    ...
    let openraw_dir = cli_openraw_home();

    // --- Ensure directories exist ---
    if !openraw_dir.exists() {
        std::fs::create_dir_all(&openraw_dir).unwrap_or_else(|e| {
            ui::error_with_fix(
                &format!("Failed to create {}", openraw_dir.display()),
                &format!("Check permissions on {}", home.display()),
            );
            eprintln!("  {e}");
            std::process::exit(1);
        });
        restrict_dir_permissions(&openraw_dir);
    }

    for sub in ["data", "agents"] {
        let dir = openraw_dir.join(sub);
        if !dir.exists() {
    ...
    bundled_agents::install_bundled_agents(&openraw_dir.join("agents"));

    if quick {
        cmd_init_quick(&openraw_dir);
    } else {
        cmd_init_interactive(&openraw_dir);
    }
}

/// Quick init: no prompts, auto-detect, write config + .env, print next steps.
fn cmd_init_quick(openraw_dir: &std::path::Path) {
    ui::banner();
    ui::blank();

    let (provider, api_key_env, model) = detect_best_provider();

    write_config_if_missing(openraw_dir, provider, model, api_key_env);

    ui::blank();
    ui::success("OpenRaw initialized (quick mode)");
    ui::kv("Provider", provider);
    ui::kv("Model", model);
    ui::blank();
    ui::next_steps(&[
        "Start the daemon:  openraw start",
        "Chat:              openraw chat",
    ]);
}

/// Interactive 5-step onboarding wizard (ratatui TUI).
fn cmd_init_interactive(openraw_dir: &std::path::Path) {
    ...
    match init_wizard::run() {
        InitResult::Completed {
            ...
            ui::blank();
            ui::success("OpenRaw initialized!");
            ui::kv("Provider", &provider);
            ui::kv("Model", &model);
            ...
            match launch {
                LaunchChoice::Desktop => {
                    launch_desktop_app(openraw_dir);
                }
                LaunchChoice::Dashboard => {
                    if let Some(base) = find_daemon() {
                        ...
                        ui::error("Daemon is not running. Start it with: openraw start");
                    }
                }
                LaunchChoice::Chat => {
...
/// Launch the openraw-desktop Tauri app, connecting to the running daemon.
fn launch_desktop_app(_openraw_dir: &std::path::Path) {
    ...
    match desktop_bin {
        Some(ref path) if path.exists() => {
            ui::success("Launching OpenRaw Desktop...");
                ...
                .spawn()
            {
                ...
                Err(e) => {
                    ...
                    ui::hint("Try: openraw dashboard");
                }
            }
        }
        _ => {
            ui::error("Desktop app not found.");
            ui::hint("Install it with: cargo install openraw-desktop");
...
fn write_config_if_missing(
    openraw_dir: &std::path::Path,
    provider: &str,
    model: &str,
    api_key_env: &str,
) {
    let config_path = openraw_dir.join("config.toml");
    if config_path.exists() {
        ui::check_ok(&format!("Config already exists: {}", config_path.display()));
    } else {
        let default_config = format!(
            r#"# OpenRaw Agent OS configuration
# See https://github.com/RightNow-AI/openraw for documentation

# For Docker, change to "0.0.0.0:4200" or set OPENRAW_LISTEN env var.
...
fn cmd_start(config: Option<PathBuf>) {
    if let Some(base) = find_daemon() {
        ui::error_with_fix(
            ...
            "Use `openraw status` to check it, or stop it first",
        );
        std::process::exit(1);
    }
    ...
    rt.block_on(async {
        let kernel = match OpenRawKernel::boot(config.as_deref()) {
            Ok(k) => k,
            Err(e) => {
                boot_kernel_error(&e);
                std::process::exit(1);
        ...
        ui::kv("Model", &model);
        ui::blank();
        ui::hint("Open the dashboard in your browser, or run `openraw chat`");
        ui::hint("Press Ctrl+C to stop the daemon");
        ui::blank();

        if let Err(e) =
            openraw_api::server::run_daemon(kernel, &listen_addr, Some(&daemon_info_path)).await
        {
        ...
        ui::blank();
        println!("  OpenRaw daemon stopped.");
    });
}

/// Read the api_key from ~/.openraw/config.toml (if any).
fn read_api_key() -> Option<String> {
    let config_path = cli_openraw_home().join("config.toml");
    ...
    match find_daemon() {
        ...
        None => {
            ui::warn_with_fix(
                ...
                "Is it running? Check with: openraw status",
            );
        }
    }
}
...
fn cmd_agent_spawn(config: Option<PathBuf>, manifest_path: PathBuf) {
    if !manifest_path.exists() {
        ui::error_with_fix(
            ...
            "Use `openraw agent new` to spawn from a template instead",
        );
        std::process::exit(1);
    }
    ...
    if let Some(base) = find_daemon() {
        ...
        match kernel.spawn_agent(manifest) {
            Ok(id) => {
                ...
                println!("  For persistent agents, use `openraw start` first.");
            }
            Err(e) => {
    ...
    match field {
        "model" => {
            if let Some(base) = find_daemon() {
                ...
                eprintln!("No running daemon found. Start one with: openraw start");
                std::process::exit(1);
            }
        }
        _ => {
...
fn cmd_agent_new(config: Option<PathBuf>, template_name: Option<String>) {
    ...
    if all_templates.is_empty() {
        ui::error_with_fix(
            ...
            "Run `openraw init` to set up the agents directory",
        );
        std::process::exit(1);
    }
        ...
        Some(ref name) => match all_templates.iter().find(|t| t.name == *name) {
            ...
            None => {
                ui::error_with_fix(
                    ...
                    "Run `openraw agent new` to see available templates",
                );
                std::process::exit(1);
            }
        },
...
fn spawn_template_agent(config: Option<PathBuf>, template: &templates::AgentTemplate) {
    if let Some(base) = find_daemon() {
        ...
        if let Some(id) = body["agent_id"].as_str() {
            ...
            ui::blank();
            ui::hint(&format!("Chat: openraw chat {}", template.name));
        } else {
            ui::error(&format!(
                "Failed to spawn: {}",
                body["error"].as_str().unwrap_or("Unknown error")
        ...
        match kernel.spawn_agent(manifest) {
            Ok(id) => {
                ...
                ui::hint("For persistent agents, use `openraw start` first");
            }
            Err(e) => {
...
fn cmd_status(config: Option<PathBuf>, json: bool) {
    if let Some(base) = find_daemon() {
        ...
        ui::section("OpenRaw Daemon Status");
        ui::blank();
        ...
        ui::section("OpenRaw Status (in-process)");
        ui::blank();
        ...
        ui::blank();
        ui::hint("Run `openraw start` to launch the daemon");
...
fn cmd_doctor(json: bool, repair: bool) {
    ...
    if let Some(_h) = &home {
        ...
        // --- Check 1: OpenRaw directory ---
        if openraw_dir.exists() {
            if !json {
                ui::check_ok(&format!("OpenRaw directory: {}", openraw_dir.display()));
            }
            checks.push(serde_json::json!({"check": "openraw_dir", "status": "ok", "path": openraw_dir.display().to_string()}));
        } else if repair {
            if !json {
                ui::check_fail("OpenRaw directory not found.");
            }
            let answer = prompt_input("    Create it now? [Y/n] ");
            if answer.is_empty() || answer.starts_with('y') || answer.starts_with('Y') {
                if std::fs::create_dir_all(&openraw_dir).is_ok() {
                    restrict_dir_permissions(&openraw_dir);
                    for sub in ["data", "agents"] {
                        let _ = std::fs::create_dir_all(openraw_dir.join(sub));
                    }
                    if !json {
                        ui::check_ok("Created OpenRaw directory");
                    }
                    repaired = true;
                } else {
                    if !json {
            ...
            checks.push(serde_json::json!({"check": "openraw_dir", "status": if repaired { "repaired" } else { "fail" }}));
        } else {
            if !json {
                ui::check_fail("OpenRaw directory not found. Run `openraw init` first.");
            }
            checks.push(serde_json::json!({"check": "openraw_dir", "status": "fail"}));
            all_ok = false;
        }

        // --- Check 2: .env file exists + permissions ---
        let env_path = openraw_dir.join(".env");
        if env_path.exists() {
            #[cfg(unix)]
            {
                use std::os::unix::fs::PermissionsExt;
            ...
            if !json {
                ui::check_warn(
                    ".env file not found (create with: openraw config set-key <provider>)",
                );
            }
        ...
        if config_path.exists() {
            ...
            match toml::from_str::<toml::Value>(&config_content) {
                ...
                Err(e) => {
                    if !json {
                        ...
                        ui::hint("Fix with: openraw config edit");
                    }
            ...
            if answer.is_empty() || answer.starts_with('y') || answer.starts_with('Y') {
                let default_config = r#"# OpenRaw Agent OS configuration
# See https://github.com/RightNow-AI/openraw for documentation
...
decay_rate = 0.05
"#;
                let _ = std::fs::create_dir_all(&openraw_dir);
        ...
        let api_listen = {
            let cfg_path = openraw_dir.join("config.toml");
            if cfg_path.exists() {
                std::fs::read_to_string(&cfg_path)
                    .ok()
                    .and_then(|s| toml::from_str::<openraw_types::config::KernelConfig>(&s).ok())
                    .map(|c| c.api_listen)
        ...
        if let Some(ref base) = daemon_running {
            ...
            if !json {
                ui::check_warn("Daemon not running (start with `openraw start`)");
            }
        ...
        let daemon_json_path = openraw_dir.join("daemon.json");
        ...
        let db_path = openraw_dir.join("data").join("openraw.db");
        if db_path.exists() {
        ...
        #[cfg(unix)]
        {
            if let Ok(output) = std::process::Command::new("df")
                .args(["-m", &openraw_dir.display().to_string()])
                .output()
            {
        ...
        let agents_dir = openraw_dir.join("agents");
    ...
    if !any_key_set {
        if !json {
            ...
            ui::blank();
            ui::hint("Or run: openraw config set-key groq");
        }
        all_ok = false;
    }
    ...
    // --- Check 12: Config deserialization into KernelConfig ---
    {
        ...
        if config_path.exists() {
            ...
            let config_content = std::fs::read_to_string(&config_path).unwrap_or_default();
            match toml::from_str::<openraw_types::config::KernelConfig>(&config_content) {
                Ok(cfg) => {
                    if !json {
                    ...
                    if !cfg.include.is_empty() {
                        ...
                        for inc in &cfg.include {
                            let inc_path = openraw_dir.join(inc);
                            if inc_path.exists() {
                                if !json {
                    ...
                    if !cfg.mcp_servers.is_empty() {
                        ...
                        for server in &cfg.mcp_servers {
                            ...
                            match &server.transport {
                                openraw_types::config::McpTransportEntry::Stdio {
                                    command, ..
                                } => {
                                    if command.is_empty() {
                                        if !json {
                                ...
                                openraw_types::config::McpTransportEntry::Sse { url } => {
                                    if url.is_empty() {
                                        if !json {
                                            ui::check_warn(&format!(
    ...
    // --- Check 13: Skill registry health ---
    {
        ...
        let skills_dir = cli_openraw_home().join("skills");
        let mut skill_reg = openraw_skills::registry::SkillRegistry::new(skills_dir.clone());
        ...
        for skill in &skills {
            if let Some(ref prompt) = skill.manifest.prompt_context {
                let warnings = openraw_skills::verify::SkillVerifier::scan_prompt_content(prompt);
                let has_critical = warnings.iter().any(|w| {
                    matches!(
                        w.severity,
                        openraw_skills::verify::WarningSeverity::Critical
                    )
                });
                if has_critical {
    ...
    // --- Check 14: Extension registry health ---
    {
        ...
        let openraw_dir = cli_openraw_home();
        let mut ext_registry = openraw_extensions::registry::IntegrationRegistry::new(&openraw_dir);
    ...
    if json {
        ...
        println!();
        if all_ok {
            ui::success("All checks passed! OpenRaw is ready.");
            ui::hint("Start the daemon: openraw start");
        } else if repaired {
            ui::success("Repairs applied. Re-run `openraw doctor` to verify.");
        } else {
            ui::error("Some checks failed.");
            if !repair {
                ui::hint("Run `openraw doctor --repair` to attempt auto-fix");
            }
        }
    }
}
        ...
        match start_daemon_background() {
            ...
            Err(e) => {
                ui::error_with_fix(
                    ...
                    "Start it manually: openraw start",
                );
                std::process::exit(1);
            }
        }
...
fn cmd_completion(shell: clap_complete::Shell) {
    ...
    clap_complete::generate(shell, &mut cmd, "openraw", &mut std::io::stdout());
}
...
fn require_daemon(command: &str) -> String {
    find_daemon().unwrap_or_else(|| {
        ui::error_with_fix(
            &format!("`openraw {command}` requires a running daemon"),
            "Start the daemon: openraw start",
        );
        ui::hint("Or try `openraw chat` which works without a daemon");
        std::process::exit(1);
    })
}

fn boot_kernel(config: Option<PathBuf>) -> OpenRawKernel {
    match OpenRawKernel::boot(config.as_deref()) {
        Ok(k) => k,
        Err(e) => {
            boot_kernel_error(&e);
            std::process::exit(1);
...
fn cmd_migrate(args: MigrateArgs) {
    let source = match args.from {
        MigrateSourceArg::Openclaw => openraw_migrate::MigrateSource::OpenClaw,
        MigrateSourceArg::Langchain => openraw_migrate::MigrateSource::LangChain,
        MigrateSourceArg::Autogpt => openraw_migrate::MigrateSource::AutoGpt,
    };
    ...
    let source_dir = args.source_dir.unwrap_or_else(|| {
        ...
        match source {
            openraw_migrate::MigrateSource::OpenClaw => home.join(".openclaw"),
            openraw_migrate::MigrateSource::LangChain => home.join(".langchain"),
            openraw_migrate::MigrateSource::AutoGpt => home.join("Auto-GPT"),
        }
    });

    let target_dir = cli_openraw_home();
    ...
    let options = openraw_migrate::MigrateOptions {
        source,
        source_dir,
        target_dir,
    ...
    match openraw_migrate::run_migration(&options) {
        Ok(report) => {
...
fn cmd_skill_install(source: &str) {
    ...
    if source_path.exists() && source_path.is_dir() {
        ...
        if !manifest_path.exists() {
            ...
            if openraw_skills::openclaw_compat::detect_openclaw_skill(&source_path) {
                println!("Detected OpenClaw skill format. Converting...");
                match openraw_skills::openclaw_compat::convert_openclaw_skill(&source_path) {
                    Ok(manifest) => {
                        let dest = skills_dir.join(&manifest.skill.name);
                        // Copy skill directory
                        copy_dir_recursive(&source_path, &dest);
                        if let Err(e) = openraw_skills::openclaw_compat::write_openraw_manifest(
                            &dest, &manifest,
                        ) {
        ...
        let manifest: openraw_skills::SkillManifest =
        ...
        let rt = tokio::runtime::Runtime::new().unwrap();
        let client = openraw_skills::marketplace::MarketplaceClient::new(
            openraw_skills::marketplace::MarketplaceConfig::default(),
        );
...
fn cmd_skill_list() {
    ...
    let mut registry = openraw_skills::registry::SkillRegistry::new(skills_dir);
...
"#
    ...
    println!("  2. Test locally: openraw skill test");
    println!(
        "  3. Install: openraw skill install {}",
        skill_dir.display()
    );
}
...
fn cmd_channel_list() {
    ...
    if !config_path.exists() {
        println!("No configuration found. Run `openraw init` first.");
        return;
    }
    ...
    println!("\nUse `openraw channel setup <channel>` to configure a channel.");
}
    ...
    match channel.as_str() {
        "telegram" => {
            ...
            match dotenv::save_env_key("TELEGRAM_BOT_TOKEN", &token) {
                Ok(()) => ui::success("Token saved to ~/.openraw/.env"),
            ...
            for (key, val) in [
                ...
                if !val.is_empty() {
                    match dotenv::save_env_key(key, val) {
                        Ok(()) => ui::success(&format!("{key} saved to ~/.openraw/.env")),
        ...
        "email" => {
            ...
            if !password.is_empty() {
                match dotenv::save_env_key("EMAIL_PASSWORD", &password) {
                    Ok(()) => ui::success("Password saved to ~/.openraw/.env"),
                    Err(_) => println!("    export EMAIL_PASSWORD=your_app_password"),
                }
            } else {
                ui::hint("Set later: openraw config set-key email (or export EMAIL_PASSWORD=...)");
            }
        ...
        "signal" => {
            ...
            if !phone.is_empty() {
                match dotenv::save_env_key("SIGNAL_PHONE", &phone) {
                    Ok(()) => ui::success("Phone saved to ~/.openraw/.env"),
        ...
        "matrix" => {
            ...
            println!("     (e.g., register @openraw-bot:matrix.org)");
            println!("  2. Obtain an access token:");
            println!("     curl -X POST https://matrix.org/_matrix/client/r0/login \\");
            println!("       -d '{{\"type\":\"m.login.password\",\"user\":\"openraw-bot\",\"password\":\"...\"}}'");
...
fn maybe_write_channel_config(channel: &str, config_block: &str) {
    ...
    if !config_path.exists() {
        ui::hint("No config.toml found. Run `openraw init` first.");
        return;
    }
...
fn cmd_channel_test(channel: &str) {
    if let Some(base) = find_daemon() {
        ...
        eprintln!("Channel test requires a running daemon. Start with: openraw start");
        std::process::exit(1);
    }
}
...
fn cmd_channel_toggle(channel: &str, enable: bool) {
    ...
    if let Some(base) = find_daemon() {
        ...
        println!("Edit ~/.openraw/config.toml to persist this change.");
    }
}
...
fn cmd_hand_install(path: &str) {
    ...
    println!(
        "Use `openraw hand activate {}` to start it.",
        body["id"].as_str().unwrap_or("?")
    );
}
...
fn cmd_hand_list() {
    ...
    if let Some(arr) = Some(&arr_val) {
        ...
        println!("\nUse `openraw hand activate <id>` to activate a hand.");
    }
}
...
/// Spawn `openraw start` as a detached background process.
///
...
fn cmd_config_show() {
    ...
    if !config_path.exists() {
        ...
        println!("Run `openraw init` to create one.");
        return;
    }
...
fn cmd_config_get(key: &str) {
    ...
    if !config_path.exists() {
        ui::error_with_fix("No config file found", "Run `openraw init` first");
        std::process::exit(1);
    }
    ...
    let table: toml::Value = toml::from_str(&content).unwrap_or_else(|e| {
        ui::error_with_fix(
            ...
            "Fix your config.toml syntax, or run `openraw config edit`",
        );
        std::process::exit(1);
    });
...
fn cmd_config_set_key(provider: &str) {
    ...
    match dotenv::save_env_key(&env_var, &key) {
        Ok(()) => {
            ui::success(&format!("Saved {env_var} to ~/.openraw/.env"));
            // Test the key
            print!("  Testing key... ");
...
fn cmd_config_delete_key(provider: &str) {
    ...
    match dotenv::remove_env_key(&env_var) {
        Ok(()) => ui::success(&format!("Removed {env_var} from ~/.openraw/.env")),
        Err(e) => {
...
fn cmd_config_test_key(provider: &str) {
    ...
    if std::env::var(&env_var).is_err() {
        ...
        ui::hint(&format!("Set it: openraw config set-key {provider}"));
        std::process::exit(1);
    }
    ...
    if test_api_key(provider, &env_var) {
        ...
        ui::hint(&format!("Update key: openraw config set-key {provider}"));
        std::process::exit(1);
    }
}
...
pub(crate) fn openraw_home() -> PathBuf {
    if let Ok(home) = std::env::var("OPENRAW_HOME") {
...
// ---------------------------------------------------------------------------
// Integration commands (openraw add/remove/integrations)
// ---------------------------------------------------------------------------

fn cmd_integration_add(name: &str, key: Option<&str>) {
    let home = openraw_home();
    let mut registry = openraw_extensions::registry::IntegrationRegistry::new(&home);
    ...
    let vault = if vault_path.exists() {
        let mut v = openraw_extensions::vault::CredentialVault::new(vault_path);
        if v.unlock().is_ok() {
            Some(v)
        } else {
            None
    ...
    let mut resolver =
        openraw_extensions::credentials::CredentialResolver::new(vault, Some(&dotenv_path))
    ...
    match openraw_extensions::installer::install_integration(
        &mut registry,
        &mut resolver,
        name,
        &provided_keys,
        ...
        Ok(result) => {
            match &result.status {
                openraw_extensions::IntegrationStatus::Ready => {
                    ui::success(&result.message);
                }
                openraw_extensions::IntegrationStatus::Setup => {
                    println!("{}", result.message.yellow());
                    println!("\nTo add credentials:");
                    for env in &template.required_env {
                        if env.is_secret {
                            println!("  openraw vault set {}  # {}", env.name, env.help);
...
fn cmd_integration_remove(name: &str) {
    ...
    match openraw_extensions::installer::remove_integration(&mut registry, name) {
        Ok(msg) => {
            ui::success(&msg);
            // Hot-reload daemon
            if let Some(base_url) = find_daemon() {
...
fn cmd_integrations_list(query: Option<&str>) {
    ...
    let dotenv_path = home.join(".env");
    let resolver =
        openraw_extensions::credentials::CredentialResolver::new(None, Some(&dotenv_path));

    let entries = if let Some(q) = query {
        openraw_extensions::installer::search_integrations(&registry, q)
    } else {
        openraw_extensions::installer::list_integrations(&registry, &resolver)
    };
    ...
    let mut by_category: std::collections::BTreeMap<
        String,
        Vec<&openraw_extensions::installer::IntegrationListEntry>,
    ...
    for (category, items) in &by_category {
        ...
        for item in items {
            let status_badge = match &item.status {
                openraw_extensions::IntegrationStatus::Ready => "[Ready]".green().to_string(),
                openraw_extensions::IntegrationStatus::Setup => "[Setup]".yellow().to_string(),
                openraw_extensions::IntegrationStatus::Available => {
                    "[Available]".dimmed().to_string()
                }
                openraw_extensions::IntegrationStatus::Error(msg) => {
                    format!("[Error: {msg}]").red().to_string()
                }
                openraw_extensions::IntegrationStatus::Disabled => {
    ...
    println!("  Use `openraw add <name>` to install an integration.");
}

// ---------------------------------------------------------------------------
// Vault commands (openraw vault init/set/list/remove)
// ---------------------------------------------------------------------------

fn cmd_vault_init() {
    let home = openraw_home();
    let vault_path = home.join("vault.enc");
    let mut vault = openraw_extensions::vault::CredentialVault::new(vault_path);
...
fn cmd_vault_set(key: &str) {
    ...
    if !vault.exists() {
        ui::error("Vault not initialized. Run: openraw vault init");
        std::process::exit(1);
    }
...
fn cmd_vault_list() {
    ...
    if !vault.exists() {
        println!("Vault not initialized. Run: openraw vault init");
        return;
    }
...
// ---------------------------------------------------------------------------
// Scaffold commands (openraw new skill/integration)
// ---------------------------------------------------------------------------

fn cmd_scaffold(kind: ScaffoldKind) {
    let cwd = std::env::current_dir().unwrap_or_default();
    let result = match kind {
        ScaffoldKind::Skill => openraw_extensions::installer::scaffold_skill(&cwd.join("my-skill")),
        ScaffoldKind::Integration => {
            openraw_extensions::installer::scaffold_integration(&cwd.join("my-integration"))
        }
    };
    match result {
...
fn cmd_models_list(provider_filter: Option<&str>, json: bool) {
    if let Some(base) = find_daemon() {
        ...
        let catalog = openraw_runtime::model_catalog::ModelCatalog::new();
...
fn pick_model() -> String {
    ...
    let mut by_provider: std::collections::BTreeMap<
        String,
        Vec<&openraw_types::model_catalog::ModelCatalogEntry>,
    > = std::collections::BTreeMap::new();
    for m in models {
...
fn cmd_logs(lines: usize, follow: bool) {
    let log_path = cli_openraw_home().join("tui.log");
...
fn cmd_health(json: bool) {
    match find_daemon() {
        ...
        None => {
            ...
            ui::hint("Start it with: openraw start");
            std::process::exit(1);
        }
    }
}
...
fn cmd_devices_pair() {
    ...
    if let Some(qr) = body["qr_data"].as_str() {
        ...
        println!("  Scan this QR code with the OpenRaw mobile app:");
        ui::blank();
        println!("  {qr}");
        ui::blank();
...
fn cmd_system_info(json: bool) {
    if let Some(base) = find_daemon() {
        ...
        ui::section("OpenRaw System Info");
        ui::blank();
        ...
        ui::kv("Version", env!("CARGO_PKG_VERSION"));
        ui::kv_warn("Daemon", "NOT RUNNING");
        ui::hint("Start with: openraw start");
    }
}
...
fn cmd_system_version(json: bool) {
    ...
    println!("openraw {}", env!("CARGO_PKG_VERSION"));
}
...
fn cmd_reset(confirm: bool) {
    ...
    if !confirm {
        println!("  This will delete all data in {}", openraw_dir.display());
    ...
    match std::fs::remove_dir_all(&openraw_dir) {
        Ok(()) => ui::success(&format!("Removed {}", openraw_dir.display())),
        Err(e) => {
            ui::error(&format!("Failed to remove {}: {e}", openraw_dir.display()));
            std::process::exit(1);
        }
    }
}
...
fn cmd_uninstall(confirm: bool, keep_config: bool) {
    ...
    println!(
        "  {}",
        "This will completely uninstall OpenRaw from your system."
            .bold()
            .red()
    );
    println!();
    if openraw_dir.exists() {
        if keep_config {
            ...
            println!("  • Remove {}", openraw_dir.display());
        }
    }
    ...
    if find_daemon().is_some() {
        ...
        if find_daemon().is_some() {
            if let Some(info) = read_daemon_info(&openraw_dir) {
                force_kill_pid(info.pid);
                let _ = std::fs::remove_file(openraw_dir.join("daemon.json"));
            }
        }
    }
    ...
    // Step 6: Remove ~/.openraw/ data
    if openraw_dir.exists() {
        if keep_config {
            remove_dir_except_config(&openraw_dir);
            ui::success("Removed data (kept config files)");
        } else {
            match std::fs::remove_dir_all(&openraw_dir) {
                Ok(()) => ui::success(&format!("Removed {}", openraw_dir.display())),
                Err(e) => ui::error(&format!("Failed to remove {}: {e}", openraw_dir.display())),
            }
        }
    }
    ...
    println!();
    ui::success("OpenRaw has been uninstalled. Goodbye!");
}
...
fn remove_autostart_entries(home: &std::path::Path) {
    ...
    #[cfg(target_os = "macos")]
    {
        let plist = home.join("Library/LaunchAgents/ai.openraw.desktop.plist");
        if plist.exists() {
            // Unload first
    ...
    #[cfg(target_os = "linux")]
    {
        let desktop_file = home.join(".config/autostart/OpenRaw.desktop");
        ...
        let service_file = home.join(".config/systemd/user/openraw.service");
        if service_file.exists() {
            let _ = std::process::Command::new("systemctl")
                .args(["--user", "disable", "--now", "openraw.service"])
                .output();
...
/// Remove lines from shell config files that add openraw to PATH.
#[allow(unused_variables)]
fn clean_path_entries(home: &std::path::Path, openraw_dir: &str) {
    #[cfg(not(windows))]
    {
        let shell_files = [
            home.join(".bashrc"),
        ...
        for path in &shell_files {
            ...
            let filtered: Vec<&str> = content
                .lines()
                .filter(|line| !is_openraw_path_line(line, openraw_dir))
                .collect();
    ...
    #[cfg(windows)]
    {
        // Read User PATH via PowerShell, filter out openraw entries, write back
        ...
        if let Ok(out) = output {
            if out.status.success() {
                ...
                if !current.is_empty() {
                    let dir_lower = openraw_dir.to_lowercase();
                    let filtered: Vec<&str> = current
                        .split(';')
                        .filter(|entry| {
                            let e = entry.trim().to_lowercase();
                            !e.is_empty() && !e.contains("openraw") && !e.contains(&dir_lower)
                        })
                        .collect();
...
/// Returns true if a shell config line is an openraw PATH export.
/// Must match BOTH an openraw reference AND a PATH-setting pattern.
#[cfg(any(not(windows), test))]
fn is_openraw_path_line(line: &str, openraw_dir: &str) -> bool {
    let lower = line.to_lowercase();
    let has_openraw = lower.contains("openraw") || lower.contains(&openraw_dir.to_lowercase());
    if !has_openraw {
        return false;
    }
...
/// Remove everything in ~/.openraw/ except config files.
fn remove_dir_except_config(openraw_dir: &std::path::Path) {
    let keep = ["config.toml", ".env", "secrets.env"];
    let Ok(entries) = std::fs::read_dir(openraw_dir) else {
        return;
    };
    for entry in entries.flatten() {
...
mod tests {
    ...
    fn test_doctor_skill_registry_loads_bundled() {
        let skills_dir = std::env::temp_dir().join("openraw-doctor-test-skills");
        let mut skill_reg = openraw_skills::registry::SkillRegistry::new(skills_dir);
    ...
    fn test_doctor_extension_registry_loads_bundled() {
        let tmp = std::env::temp_dir().join("openraw-doctor-test-ext");
        let _ = std::fs::create_dir_all(&tmp);
        let mut ext_reg = openraw_extensions::registry::IntegrationRegistry::new(&tmp);
    ...
    fn test_doctor_config_deser_default() {
        ...
        let config = openraw_types::config::KernelConfig::default();
        let toml_str = toml::to_string_pretty(&config).unwrap();
        let parsed: openraw_types::config::KernelConfig = toml::from_str(&toml_str).unwrap();
...
api_key_env = "GROQ_API_KEY"
"#;
        let config: openraw_types::config::KernelConfig = toml::from_str(config_toml).unwrap();
...
"#;
        ...
        assert_eq!(
            ...
            openraw_types::config::ExecSecurityMode::Allowlist
        );
...
"#;
        ...
        match &config.mcp_servers[0].transport {
            openraw_types::config::McpTransportEntry::Stdio { command, args } => {
    ...
    fn test_doctor_skill_injection_scan_clean() {
        ...
        let warnings = openraw_skills::verify::SkillVerifier::scan_prompt_content(clean_content);
    ...
    fn test_doctor_hook_event_variants() {
        ...
        use openraw_types::agent::HookEvent;
        let events = [
    ...
    fn test_uninstall_path_line_filter() {
        use super::is_openraw_path_line;
        let dir = "/home/user/.openraw/bin";

        // Should match: openraw PATH exports
        assert!(is_openraw_path_line(
            r#"export PATH="$HOME/.openraw/bin:$PATH""#,
            dir
        ));
        assert!(is_openraw_path_line(
            r#"export PATH="/home/user/.openraw/bin:$PATH""#,
            dir
        ));
        assert!(is_openraw_path_line(
            "set -gx PATH $HOME/.openraw/bin $PATH",
            dir
        ));
        assert!(is_openraw_path_line(
            "fish_add_path $HOME/.openraw/bin",
            dir
        ));
        ...
        // Should NOT match: openraw lines that aren't PATH-related
        assert!(!is_openraw_path_line("# openraw config", dir));
        assert!(!is_openraw_path_line("alias of=openraw", dir));
    }
}
