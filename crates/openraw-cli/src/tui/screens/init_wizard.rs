...
//!
//! Launched by `openraw init` (without `--quick`). Takes over the terminal,
...
use crate::tui::theme;
use openraw_runtime::model_catalog::ModelCatalog;
use openraw_types::model_catalog::ModelTier;
...
struct State {
    ...
    openclaw_scan: Option<openraw_migrate::openclaw::ScanResult>,
    migration_report: Option<openraw_migrate::report::MigrationReport>,
...
pub fn run() -> InitResult {
    ...
    let (migrate_tx, migrate_rx) =
        std::sync::mpsc::channel::<Result<openraw_migrate::report::MigrationReport, String>>();
...
fn handle_migration_key(
    state: &mut State,
    code: KeyCode,
    migrate_tx: &std::sync::mpsc::Sender<Result<openraw_migrate::report::MigrationReport, String>>,
) {
                ...
                if yes {
                    ...
                    std::thread::spawn(move || {
                        let options = openraw_migrate::MigrateOptions {
                            source: openraw_migrate::MigrateSource::OpenClaw,
                            source_dir,
                            target_dir,
                            dry_run: false,
                        };
                        let result =
                            openraw_migrate::run_migration(&options).map_err(|e| format!("{e}"));
...
fn save_config(state: &mut State) {
    ...
    let openraw_dir = if let Ok(h) = std::env::var("OPENRAW_HOME") {
        PathBuf::from(h)
    } else {
        match dirs::home_dir() {
            Some(h) => h.join(".openraw"),
            None => {
    ...
    let _ = std::fs::create_dir_all(openraw_dir.join("agents"));
    let _ = std::fs::create_dir_all(openraw_dir.join("data"));
    crate::restrict_dir_permissions(&openraw_dir);
...
/// Check if the `openraw-desktop` binary exists next to the current exe.
...
fn draw(f: &mut Frame, area: Rect, state: &mut State) {
    ...
    // Header: "OpenRaw Init  Step X of 7"
...
fn draw_migration_done(f: &mut Frame, area: Rect, state: &State) {
    ...
    if let Some(ref error) = state.migration_error {
        ...
        use openraw_migrate::report::ItemKind;
...
fn draw_complete(f: &mut Frame, area: Rect, state: &mut State) {
    ...
    f.render_widget(
        Paragraph::new(Line::from(vec![Span::styled(
            "  How do you want to use OpenRaw?",
            Style::default()
                .fg(theme::ACCENT)
                .add_modifier(Modifier::BOLD),
        )])),
