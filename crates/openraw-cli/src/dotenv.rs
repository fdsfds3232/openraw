//! Minimal `.env` file loader/saver for `~/.openraw/.env`.
//!
...
fn dotenv_openraw_home() -> Option<PathBuf> {
    if let Ok(home) = std::env::var("OPENRAW_HOME") {
        return Some(PathBuf::from(home));
    }
    dirs::home_dir().map(|h| h.join(".openraw"))
}

/// Return the path to `~/.openraw/.env`.
pub fn env_file_path() -> Option<PathBuf> {
    dotenv_openraw_home().map(|h| h.join(".env"))
}

/// Load `~/.openraw/.env` and `~/.openraw/secrets.env` into `std::env`.
///
...
/// Return the path to `~/.openraw/secrets.env`.
pub fn secrets_env_path() -> Option<PathBuf> {
    dotenv_openraw_home().map(|h| h.join("secrets.env"))
}
...
/// List key names (without values) from `~/.openraw/.env`.
#[allow(dead_code)]
...
fn write_env_file(path: &PathBuf, entries: &BTreeMap<String, String>) -> Result<(), String> {
    let mut content = String::from("# OpenRaw environment — managed by `openraw config set-key`\n");
