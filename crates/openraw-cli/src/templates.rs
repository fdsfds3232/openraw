...
/// 2. `~/.openraw/agents/` (installed templates)
/// 3. `OPENRAW_AGENTS_DIR` env var
pub fn discover_template_dirs() -> Vec<PathBuf> {
    ...
    // Installed templates (respects OPENRAW_HOME)
    let of_home = if let Ok(h) = std::env::var("OPENRAW_HOME") {
        PathBuf::from(h)
    } else if let Some(home) = dirs::home_dir() {
        home.join(".openraw")
    } else {
        std::env::temp_dir().join(".openraw")
    };
    {
    ...
    if let Ok(env_dir) = std::env::var("OPENRAW_AGENTS_DIR") {
