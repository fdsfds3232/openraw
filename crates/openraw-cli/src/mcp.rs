//! MCP (Model Context Protocol) server for OpenRaw.
//!
//! Exposes running agents as MCP tools over JSON-RPC 2.0 stdio.
//! Each agent becomes a callable tool named `openraw_agent_{name}`.
//!
...
impl McpBackend {
    ...
    /// Find agent ID by tool name (strip `openraw_agent_` prefix, match by name).
    fn resolve_tool_agent(&self, tool_name: &str) -> Option<String> {
        let agent_name = tool_name.strip_prefix("openraw_agent_")?.replace('_', "-");
        ...
        let agent_name_underscore = tool_name.strip_prefix("openraw_agent_")?;
        for (id, name, _) in &agents {
        ...
        "tools/list" => {
            ...
            let tools: Vec<Value> = agents
                ...
                .map(|(_, name, description)| {
                    let tool_name = format!("openraw_agent_{}", name.replace('-', "_"));
                    let desc = if description.is_empty() {
                        format!("Send a message to OpenRaw agent '{name}'")
                    } else {
                        description.clone()
                    };
                    json!({
...
mod tests {
    ...
    fn test_handle_initialize() {
        ...
        assert_eq!(resp["result"]["serverInfo"]["name"], "openraw");
    }
