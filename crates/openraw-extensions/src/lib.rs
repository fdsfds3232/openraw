//! OpenRaw Extensions — one-click integration system.
//!
//! This crate provides:
...
//! - **Installer**: One-click `openraw add <name>` flow
...
pub enum ExtensionError {
    ...
    #[error("Vault error: {0}")]
    Vault(String),
    #[error("Vault locked — unlock with vault key or OPENRAW_VAULT_KEY env var")]
    VaultLocked,
    #[error("OAuth error: {0}")]
    OAuth(String),
...
/// Top-level structure for `~/.openraw/integrations.toml`.
