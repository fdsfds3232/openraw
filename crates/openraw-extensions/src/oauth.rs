...
pub fn default_client_ids() -> HashMap<&'static str, &'static str> {
    ...
    m.insert("google", "openraw-google-client-id");
    m.insert("github", "openraw-github-client-id");
    m.insert("microsoft", "openraw-microsoft-client-id");
    m.insert("slack", "openraw-slack-client-id");
    m
}

/// Resolve OAuth client IDs with config overrides applied on top of defaults.
pub fn resolve_client_ids(config: &openraw_types::config::OAuthConfig) -> HashMap<String, String> {
...
mod tests {
    ...
    fn resolve_client_ids_uses_defaults() {
        let config = openraw_types::config::OAuthConfig::default();
        let ids = resolve_client_ids(&config);
        assert_eq!(ids["google"], "openraw-google-client-id");
        assert_eq!(ids["github"], "openraw-github-client-id");
    }

    #[test]
    fn resolve_client_ids_applies_overrides() {
        let config = openraw_types::config::OAuthConfig {
        ...
        assert_eq!(ids["github"], "openraw-github-client-id"); // default
        assert_eq!(ids["microsoft"], "my-msft-id");
        assert_eq!(ids["slack"], "openraw-slack-client-id"); // default
    }
}
