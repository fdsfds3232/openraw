...
//!
//! Stores secrets in `~/.openraw/vault.enc`, with the master key sourced from
//! the OS keyring (Windows Credential Manager / macOS Keychain / Linux Secret Service)
//! or the `OPENRAW_VAULT_KEY` env var for headless/CI environments.
...
impl CredentialVault {
    ...
    pub fn unlock(&mut self) -> ExtensionResult<()> {
        ...
        if !self.path.exists() {
            return Err(ExtensionError::Vault(
                "Vault not initialized. Run `openraw vault init`.".to_string(),
            ));
        }
...
fn machine_fingerprint() -> Vec<u8> {
    ...
    hasher.update(b"openraw-vault-v1");
