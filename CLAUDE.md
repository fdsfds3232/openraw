# OpenRaw — Agent Instructions

## Project Overview
OpenRaw is an open-source Agent Operating System written in Rust (14 crates).
- Config: `~/.openraw/config.toml`
- Default API: `http://127.0.0.1:4200`
- CLI binary: `target/release/openraw.exe` (or `target/debug/openraw.exe`)
...
```bash
GROQ_API_KEY=<key> target/release/openraw.exe start &
...
## Architecture Notes
- **Don't touch `openraw-cli`** — user is actively building the interactive CLI
...
## Common Gotchas
- `openraw.exe` may be locked if daemon is running — use `--lib` flag or kill daemon first
