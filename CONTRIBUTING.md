...
Thank you for your interest in contributing to OpenRaw. This guide covers everything you need to get started, from setting up your development environment to submitting pull requests.
...
```bash
cargo test -p openraw-kernel
cargo test -p openraw-runtime
cargo test -p openraw-memory
```
...
- **Naming**:
  - Types: `PascalCase` (e.g., `OpenRawKernel`, `AgentManifest`)
  - Functions/methods: `snake_case`
  - Constants: `SCREAMING_SNAKE_CASE`
  - Crate names: `openraw-{name}` (kebab-case)
...
OpenRaw is organized as a Cargo workspace with 14 crates:

| Crate | Role |
|-------|------|
| `openraw-types` | Shared type definitions, taint tracking, manifest signing (Ed25519), model catalog, MCP/A2A config types |
| `openraw-memory` | SQLite-backed memory substrate with vector embeddings, usage tracking, canonical sessions, JSONL mirroring |
| `openraw-runtime` | Agent loop, 3 LLM drivers (Anthropic/Gemini/OpenAI-compat), 38 built-in tools, WASM sandbox, MCP client/server, A2A protocol |
| `openraw-hands` | Hands system (curated autonomous capability packages), 7 bundled hands |
| `openraw-extensions` | Integration registry (25 bundled MCP templates), AES-256-GCM credential vault, OAuth2 PKCE |
| `openraw-kernel` | Assembles all subsystems: workflow engine, RBAC auth, heartbeat monitor, cron scheduler, config hot-reload |
| `openraw-api` | REST/WS/SSE API (Axum 0.8), 76 endpoints, 14-page SPA dashboard, OpenAI-compatible `/v1/chat/completions` |
| `openraw-channels` | 40 channel adapters (Telegram, Discord, Slack, WhatsApp, and 36 more), formatter, rate limiter |
| `openraw-wire` | OFP (OpenRaw Protocol): TCP P2P networking with HMAC-SHA256 mutual authentication |
| `openraw-cli` | Clap CLI with daemon auto-detect (HTTP mode vs. in-process fallback), MCP server |
| `openraw-migrate` | Migration engine for importing from OpenClaw (and future frameworks) |
| `openraw-skills` | Skill system: 60 bundled skills, FangHub marketplace, OpenClaw compatibility, prompt injection scanning |
| `openraw-desktop` | Tauri 2.0 native desktop app (WebView + system tray + single-instance + notifications) |
| `xtask` | Build automation tasks |

### Key Architectural Patterns

- **`KernelHandle` trait**: Defined in `openraw-runtime`, implemented on `OpenRawKernel` in `openraw-kernel`. This avoids circular crate dependencies while enabling inter-agent tools.
- **Shared memory**: A fixed UUID (`AgentId(Uuid::from_bytes([0..0, 0x01]))`) provides a cross-agent KV namespace.
- **Daemon detection**: The CLI checks `~/.openraw/daemon.json` and pings the health endpoint. If a daemon is running, commands use HTTP; otherwise, they boot an in-process kernel.
...
```bash
openraw agent spawn agents/my-agent/agent.toml
```
...
Channel adapters live in `crates/openraw-channels/src/`. Each adapter implements the `ChannelAdapter` trait.

### Steps

1. Create a new file: `crates/openraw-channels/src/myplatform.rs`
...
3. Register the module in `crates/openraw-channels/src/lib.rs`:

```rust
pub mod myplatform;
```

4. Wire it up in the channel bridge (`crates/openraw-api/src/channel_bridge.rs`) so the daemon starts it alongside other adapters.

5. Add configuration support in `openraw-types` config structs (add a `[channels.myplatform]` section).

6. Add CLI setup wizard instructions in `crates/openraw-cli/src/main.rs` under `cmd_channel_setup`.
...
Built-in tools are defined in `crates/openraw-runtime/src/tool_runner.rs`.
...
- Open a [GitHub Discussion](https://github.com/RightNow-AI/openraw/discussions) for questions.
- Open a [GitHub Issue](https://github.com/RightNow-AI/openraw/issues) for bugs or feature requests.
