...
OpenRaw auto-detects which providers have API keys configured at boot. Any model whose provider is authenticated becomes immediately available. Local providers (Ollama, vLLM, LM Studio) require no key at all.
...
**Notes:** OpenRaw auto-discovers models from a running Ollama instance and merges them into the catalog with `Local` tier and zero cost. Any model you pull becomes usable immediately.
...
OpenRaw can automatically select the cheapest model capable of handling each query. This is configured per-agent via `ModelRoutingConfig`.
...
OpenRaw tracks the cost of every LLM call and can enforce per-agent spending quotas.
