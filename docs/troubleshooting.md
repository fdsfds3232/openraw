...
Common issues, diagnostics, and answers to frequently asked questions about OpenRaw.
...
OpenRaw uses `tracing` for structured logging. Set the log level via environment:

```bash
RUST_LOG=info openraw start          # Default
RUST_LOG=debug openraw start         # Verbose
RUST_LOG=openraw=debug openraw start  # Only OpenRaw debug, deps at info
```
...
### `openraw` command not found after install
...
**Common causes**:
- No API key provided: `docker run -e GROQ_API_KEY=... ghcr.io/RightNow-AI/openraw`
...
**Fix**: Run `openraw init` to create the default config:
```bash
openraw init
```

This creates `~/.openraw/config.toml` with sensible defaults.
...
```bash
RUST_LOG=openraw_channels=debug openraw start
```
...
**Automatic protection**: OpenRaw has a built-in loop guard:
...
**Check**:
1. TOML manifest is valid: `openraw agent spawn --dry-run manifest.toml`
...
2. Set the model to `openraw:agent-name` (e.g., `openraw:coder`)
...
3. Try deleting `~/.openraw/daemon.json` and restarting
...
If slower:
- Check database size (`~/.openraw/data/openraw.db`)
...
1. Add the channel config to `~/.openraw/config.toml` under `[channels]`
2. Set the required environment variables (tokens, secrets)
3. Restart the daemon

### How do I update OpenRaw?

```bash
# From source
cd openraw && git pull && cargo install --path crates/openraw-cli
...
Only LLM API calls go to the provider's servers. All agent data, memory, sessions, and configuration are stored locally in SQLite (`~/.openraw/data/openraw.db`). The OFP wire protocol uses HMAC-SHA256 mutual authentication for P2P communication.

### How do I back up my data?

Back up these files:
- `~/.openraw/config.toml` (configuration)
- `~/.openraw/data/openraw.db` (all agent data, memory, sessions)
- `~/.openraw/skills/` (installed skills)

### How do I reset everything?

```bash
rm -rf ~/.openraw
openraw init  # Start fresh
```

### Can I run OpenRaw without an internet connection?
...
### What's the difference between OpenRaw and OpenClaw?

| Aspect | OpenRaw | OpenClaw |
|--------|----------|----------|
...
OpenRaw can import OpenClaw configs: `openraw migrate --from openclaw`
...
```bash
RUST_LOG=openraw_runtime=debug,openraw_channels=info openraw start
```

### Can I use OpenRaw as a library?

Yes. Each crate is independently usable:
```toml
[dependencies]
openraw-runtime = { path = "crates/openraw-runtime" }
openraw-memory = { path = "crates/openraw-memory" }
```

The `openraw-kernel` crate assembles everything, but you can use individual crates for custom integrations.
