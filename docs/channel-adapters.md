...
OpenRaw connects to messaging platforms through **40 channel adapters**, allowing users to interact with their agents across every major communication platform. Adapters span consumer messaging, enterprise collaboration, social media, community platforms, privacy-focused protocols, and generic webhooks.
...
All channel configurations live in `~/.openraw/config.toml` under the `[channels]` section. Each channel is a subsection:
...
- `bot_token_env` / `token_env` -- The environment variable holding the bot/access token. OpenRaw reads the token from this env var at startup. All secrets are stored as `Zeroizing<String>` and wiped from memory on drop.
...
The `formatter` module (`openraw-channels/src/formatter.rs`) converts Markdown output from the LLM into platform-native formats:
...
The `ChannelRateLimiter` (`openraw-channels/src/rate_limiter.rs`) uses a `DashMap` to track per-user message counts. When `rate_limit_per_user` is set on a channel's overrides, the limiter enforces a sliding-window cap of N messages per minute. Excess messages receive a polite rejection.
...
user_id = "@openraw-bot:matrix.org"
default_agent = "assistant"
```
...
To add support for a new messaging platform, implement the `ChannelAdapter` trait. The trait is defined in `crates/openraw-channels/src/types.rs`.
...
Create `crates/openraw-channels/src/myplatform.rs`:
...
In `crates/openraw-channels/src/lib.rs`:
...
In `crates/openraw-api/src/channel_bridge.rs`, add initialization logic for your adapter alongside the existing adapters.

### 4. Add Config Support

In `openraw-types`, add a config struct:
...
In `crates/openraw-cli/src/main.rs`, add a case to `cmd_channel_setup` with step-by-step instructions for your platform.
