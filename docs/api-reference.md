...
OpenRaw exposes a REST API, WebSocket endpoints, and SSE streaming when the daemon is running. The default listen address is `http://127.0.0.1:4200`.

All responses include security headers (CSP, X-Frame-Options, X-Content-Type-Options, HSTS) and are protected by a GCRA cost-aware rate limiter with per-IP token bucket tracking and automatic stale entry cleanup. OpenRaw implements 16 security systems including Merkle audit trails, taint tracking, WASM dual metering, Ed25519 manifest signing, SSRF protection, subprocess sandboxing, and secret zeroization.
  ...
  "templates": [
    {
      "name": "hello-world",
      "description": "A friendly greeting agent",
      "path": "/home/user/.openraw/agents/hello-world/agent.toml"
    },
    {
      "name": "coder",
      "description": "Expert coding assistant",
      "path": "/home/user/.openraw/agents/coder/agent.toml"
    }
  ],
  "total": 30
}
...
```json
{
  "status": "running",
  "agent_count": 2,
  "data_dir": "/home/user/.openraw/data",
...
List OFP (OpenRaw Protocol) wire peers and their connection status.
...
OpenRaw maintains a built-in catalog of 51+ models across 20 providers. These endpoints allow you to browse available models, check provider authentication status, and resolve model aliases.
...
```json
{
  "status": "created",
  "skill": "my-skill",
  "path": "/home/user/.openraw/skills/my-skill"
}
```
...
Install a skill from ClawHub. Downloads, verifies SHA256 checksum, scans for prompt injection, and converts SKILL.md format to OpenRaw skill.toml automatically.
...
OpenRaw supports both Model Context Protocol (MCP) for tool interoperability and Agent-to-Agent (A2A) protocol for cross-system agent communication.
...
MCP HTTP transport endpoint. Accepts JSON-RPC 2.0 requests and exposes OpenRaw tools via the MCP protocol to external clients.
...
{
  "name": "OpenRaw",
  "description": "OpenRaw Agent Operating System",
...
OpenRaw maintains a Merkle hash chain audit trail for all security-relevant operations. These endpoints allow inspection and verification of the audit log integrity.
...
OpenRaw exposes an OpenAI-compatible API for drop-in integration with tools that support the OpenAI API format (Cursor, Continue, Open WebUI, etc.).
...
**Model resolution** (the `model` field maps to an OpenRaw agent):

| Format | Example | Behavior |
|--------|---------|----------|
| `openraw:<name>` | `openraw:coder` | Find agent by name |
