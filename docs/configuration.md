# OpenRaw Configuration Reference

Complete reference for `config.toml`, covering every configurable field in the OpenRaw Agent OS.
...
OpenRaw reads its configuration from a single TOML file:
...
The simplest working configuration only needs an LLM provider API key set as an environment variable. With no config file at all, OpenRaw boots with Anthropic as the default provider:
...
```toml
# ============================================================
# OpenRaw Agent OS -- Complete Configuration Reference
# ============================================================

# --- Top-level fields ---
home_dir = "~/.openraw"             # OpenRaw home directory
data_dir = "~/.openraw/data"        # SQLite databases and data files
...
# --- Memory ---
[memory]
# sqlite_path = "~/.openraw/data/openraw.db"  # Auto-resolved if omitted
...
|-------|------|---------|-------------|
| `home_dir` | path | `~/.openraw` | OpenRaw home directory. Stores config, agents, skills. |
| `data_dir` | path | `~/.openraw/data` | Directory for SQLite databases and persistent data. |
...
```toml
[memory]
# sqlite_path = "/custom/path/openraw.db"
embedding_model = "all-MiniLM-L6-v2"
consolidation_threshold = 10000
decay_rate = 0.1
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `sqlite_path` | path or null | `null` | Explicit path to the SQLite database file. When `null`, defaults to `{data_dir}/openraw.db`. |
...
Configures the OFP (OpenRaw Protocol) peer-to-peer networking layer with HMAC-SHA256 mutual authentication.
...
| `user_id` | string | `""` | Bot user ID (e.g., `"@openraw:matrix.org"`). |
...
| `nick` | string | `"openraw"` | Bot nickname. |
| `password_env` | string or null | `null` | Env var holding the server password (optional). |
| `channels` | list of strings | `[]` | IRC channels to join (e.g., `["#openraw", "#general"]`). |
...
| `nick` | string | `"openraw"` | Bot nickname in Twitch chat. |
| `default_agent` | string or null | `null` | Agent name to route messages to. |
...
| `username` | string | `"openraw"` | Bot username in Mumble. |
...
Agent-to-Agent protocol configuration, enabling inter-agent communication across OpenRaw instances.
