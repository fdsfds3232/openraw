...
The OpenRaw Desktop App is a native desktop wrapper built with [Tauri 2.0](https://v2.tauri.app/) that packages the entire OpenRaw Agent OS into a single, installable application. Instead of running a CLI daemon and opening a browser, users get a native window with system tray integration, OS notifications, and single-instance enforcement -- all powered by the same kernel and API server that the headless deployment uses.

**Crate:** `openraw-desktop`
**Identifier:** `ai.openraw.desktop`
**Product name:** OpenRaw
...
1. **Tracing init** -- `tracing_subscriber` is configured with `RUST_LOG` env, defaulting to `openraw=info,tauri=info`.
2. **Kernel boot** -- `OpenRawKernel::boot(None)` loads the default configuration (from `config.toml` or defaults), wrapped in `Arc`. `set_self_handle()` is called to enable self-referencing kernel operations.
...
4. **Server thread** -- A dedicated OS thread named `"openraw-server"` is spawned. It creates its own `tokio::runtime::Builder::new_multi_thread()` runtime and runs:
   - `kernel.start_background_agents()` -- heartbeat monitor, autonomous agents, etc.
   - `run_embedded_server()` -- builds the axum router via `openraw_api::server::build_router()`, converts the `std::net::TcpListener` to a `tokio::net::TcpListener`, and serves with graceful shutdown.
...
| **Open Config Directory** | Opens `~/.openraw/` in the OS file manager |
| **Quit OpenRaw** | Logs the quit event and calls `app.exit(0)` |

The tray tooltip reads **"OpenRaw Agent OS"**.
...
On desktop platforms, `tauri-plugin-single-instance` prevents multiple copies of OpenRaw from running simultaneously. When a second instance attempts to launch, the existing instance's main window is shown, unminimized, and focused:
...
To actually quit, use the **"Quit OpenRaw"** option in the system tray menu.
...
Opens a native file picker for `.toml` files. Validates the selected file as an `AgentManifest`, copies it to `~/.openraw/agents/{name}/agent.toml`, and spawns the agent. Returns the agent name on success.

### `import_skill_file`

Opens a native file picker for skill files (`.md`, `.toml`, `.py`, `.js`, `.wasm`). Copies the file to `~/.openraw/skills/` and triggers a hot-reload of the skill registry.

### `get_autostart` / `set_autostart`

Check or toggle whether OpenRaw launches at OS login. Uses `tauri-plugin-autostart` (launchd on macOS, registry on Windows, systemd on Linux).
...
Opens `~/.openraw/` or `~/.openraw/logs/` in the OS file manager.
...
| Variable | Effect |
|----------|--------|
| `RUST_LOG` | Controls tracing verbosity. Defaults to `openraw=info,tauri=info` if unset. |

All other OpenRaw environment variables (API keys, configuration) apply as normal since the desktop app boots the same kernel as the headless daemon.
