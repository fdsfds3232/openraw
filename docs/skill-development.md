...
Skills are pluggable tool bundles that extend agent capabilities in OpenRaw. A skill packages one or more tools with their implementation, letting agents do things that built-in tools do not cover. This guide covers skill creation, the manifest format, Python and WASM runtimes, publishing to FangHub, and CLI management.
...
Skills are installed to `~/.openraw/skills/` and made available to agents through the skill registry. OpenRaw ships with **60 bundled skills** that are compiled into the binary and available immediately.
...
OpenRaw includes 60 expert knowledge skills compiled into the binary (no installation needed):
...
1. OpenRaw sends a JSON payload to the script's stdin:
...
def summarize_url(url: str) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "OpenRaw-Skill/1.0"})
...
### Using the OpenRaw Python SDK

For more advanced skills, use the Python SDK (`sdk/python/openraw_sdk.py`):

```python
#!/usr/bin/env python3
from openraw_sdk import SkillHandler
...
```bash
openraw skill install /path/to/my-skill
```

This reads the `skill.toml`, validates the manifest, and copies the skill to `~/.openraw/skills/my-skill/`.

### From FangHub

```bash
openraw skill install web-summarizer
```
...
```bash
openraw skill install https://github.com/user/openraw-skill-example.git
```
...
```bash
openraw skill remove web-summarizer
```
...
FangHub is the community skill marketplace for OpenRaw.
...
```bash
openraw skill search "web scraping"
```
...
OpenRaw can install and run OpenClaw-format skills. The skill installer auto-detects OpenClaw skills (by looking for `package.json` + `index.ts`/`index.js`) and converts them.

### Automatic Conversion

```bash
openraw skill install /path/to/openclaw-skill
```

If the directory contains an OpenClaw-style skill (Node.js package), OpenRaw:

1. Detects the OpenClaw format.
2. Generates a `skill.toml` manifest from `package.json`.
3. Maps tool names to OpenRaw conventions.
4. Copies the skill to the OpenRaw skills directory.
...
```bash
openraw skill install /path/to/skill-directory
```

Skills imported via `openraw migrate --from openclaw` are also scanned and reported in the migration report, with instructions for manual reinstallation.
