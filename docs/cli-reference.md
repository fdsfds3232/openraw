# OpenRaw CLI Reference

Complete command-line reference for `openraw`, the CLI tool for the OpenRaw Agent OS.

## Overview

The `openraw` binary is the primary interface for managing the OpenRaw Agent OS. It supports two modes of operation:

- **Daemon mode** -- When a daemon is running (`openraw start`), CLI commands communicate with it over HTTP. This is the recommended mode for production use.
...
Running `openraw` with no subcommand launches the interactive TUI (terminal user interface) built with ratatui, which provides a full dashboard experience in the terminal.
...
```bash
cargo build --release -p openraw-cli
# Binary: target/release/openraw (or openraw.exe on Windows)
```

### Docker

```bash
docker run -it openraw/openraw:latest
```

### Shell installer

```bash
curl -fsSL https://get.openraw.ai | sh
```
...
| Option | Description |
|---|---|
| `--config <PATH>` | Path to a custom config file. Overrides the default `~/.openraw/config.toml`. |
| `--help` | Print help information for any command or subcommand. |
| `--version` | Print the version of the `openraw` binary. |

**Environment variables:**

| Variable | Description |
|---|---|
| `RUST_LOG` | Controls log verbosity (e.g. `info`, `debug`, `openraw_kernel=trace`). |
| `OPENRAW_AGENTS_DIR` | Override the agent templates directory. |
| `EDITOR` / `VISUAL` | Editor used by `openraw config edit`. Falls back to `notepad` (Windows) or `vi` (Unix). |

---

## Command Reference

### openraw (no subcommand)
...
The TUI provides a full-screen terminal interface with panels for agents, chat, workflows, channels, skills, settings, and more. Tracing output is redirected to `~/.openraw/tui.log` to avoid corrupting the terminal display.
...
Initialize the OpenRaw workspace. Creates `~/.openraw/` with subdirectories (`data/`, `agents/`) and a default `config.toml`.
...
Start the OpenRaw daemon (kernel + API server).

```
openraw start [--config <PATH>]
```

**Behavior:**

- Checks if a daemon is already running; exits with an error if so.
- Boots the OpenRaw kernel (loads config, initializes SQLite database, loads agents, connects MCP servers, starts background tasks).
- Starts the HTTP API server on the address specified in `config.toml` (default: `127.0.0.1:4200`).
- Writes `daemon.json` to `~/.openraw/` so other CLI commands can discover the running daemon.
- Blocks until interrupted with `Ctrl+C`.

**Output:**

```
  OpenRaw Agent OS v0.1.0
  ...
  hint: Open the dashboard in your browser, or run `openraw chat`
...
openraw start --config /path/to/config.toml
```
...
openraw status --json | jq '.agent_count'
```
...
Run diagnostic checks on the OpenRaw installation.

```
openraw doctor [--json] [--repair]
```
...
1. **OpenRaw directory** -- `~/.openraw/` exists
...
9. **Agent manifests** -- validates all `.toml` files in `~/.openraw/agents/`
...
```bash
# Bash
openraw completion bash > ~/.bash_completion.d/openraw

# Zsh
openraw completion zsh > ~/.zfunc/_openraw

# Fish
openraw completion fish > ~/.config/fish/completions/openraw.fish

# PowerShell
openraw completion powershell > openraw.ps1
```
...
- Templates are discovered from: the repo `agents/` directory (dev builds), `~/.openraw/agents/` (installed), and `OPENRAW_AGENTS_DIR` (env override).
...
# Spawn by name
openraw agent new coder

# Spawn the assistant template
openraw agent new assistant
```
...
```bash
openraw agent spawn ./my-agent/agent.toml
```
...
openraw agent list --json | jq '.[].name'
```
...
| Argument | Description |
|---|---|
| `<AGENT_ID>` | Agent UUID. Obtain from `openraw agent list`. |
...
```bash
openraw agent chat a1b2c3d4-e5f6-7890-abcd-ef1234567890
```
...
```bash
openraw agent kill a1b2c3d4-e5f6-7890-abcd-ef1234567890
```
...
### openraw workflow create

Create a workflow from a JSON definition file.

```
openraw workflow create <FILE>
```
...
```bash
openraw workflow create ./my-workflow.json
```

---

### openraw workflow run

Execute a workflow by ID.

```
openraw workflow run <WORKFLOW_ID> <INPUT>
```
...
| Argument | Description |
|---|---|
| `<WORKFLOW_ID>` | Workflow UUID. Obtain from `openraw workflow list`. |
| `<INPUT>` | Input text to pass to the workflow. |

**Example:**

```bash
openraw workflow run abc123 "Analyze this code for security issues"
```
...
### openraw trigger list

List all event triggers.

```
openraw trigger list [--agent-id <ID>]
```
...
### openraw trigger create

Create an event trigger for an agent.

```
openraw trigger create <AGENT_ID> <PATTERN_JSON> [--prompt <TEMPLATE>] [--max-fires <N>]
```
...
openraw trigger create <AGENT_ID> '{"lifecycle":{}}'

# Fire when a specific agent is spawned
openraw trigger create <AGENT_ID> '{"agent_spawned":{"name_pattern":"*"}}'

# Fire on agent termination
openraw trigger create <AGENT_ID> '{"agent_terminated":{}}'

# Fire on all events (limited to 10 fires)
openraw trigger create <AGENT_ID> '{"all":{}}' --max-fires 10
```

---

### openraw trigger delete

Delete a trigger by ID.

```
openraw trigger delete <TRIGGER_ID>
```
...
Loads skills from `~/.openraw/skills/` plus bundled skills compiled into the binary.

---

### openraw skill install

Install a skill from a local directory, git URL, or FangHub marketplace.

```
openraw skill install <SOURCE>
```
...
openraw skill install ./my-skill/

# Install from FangHub
openraw skill install web-search

# Install an OpenClaw-format skill
openraw skill install ./openclaw-skill/
```

---

### openraw skill remove

Remove an installed skill.

```
openraw skill remove <NAME>
```
...
```bash
openraw skill remove web-search
```

---

### openraw skill search

Search the FangHub marketplace for skills.

```
openraw skill search <QUERY>
```
...
```bash
openraw skill search "docker kubernetes"
```

---

### openraw skill create
...
Creates a directory under `~/.openraw/skills/<name>/` with:
...
### openraw channel setup

Interactive setup wizard for a channel integration.

```
openraw channel setup [<CHANNEL>]
```
...
3. Saves tokens to `~/.openraw/.env` with owner-only permissions.
...
# Direct setup
openraw channel setup telegram
openraw channel setup discord
openraw channel setup slack
```

---

### openraw channel test

Send a test message through a configured channel.

```
openraw channel test <CHANNEL>
```
...
```bash
openraw channel test telegram
```

---

### openraw channel enable

Enable a channel integration.

```
openraw channel enable <CHANNEL>
```
...
### openraw channel disable

Disable a channel without removing its configuration.

```
openraw channel disable <CHANNEL>
```
...
Prints the contents of `~/.openraw/config.toml` with the file path as a header comment.
...
```bash
openraw config get default_model.provider
# groq

openraw config get api_listen
# 127.0.0.1:4200

openraw config get memory.decay_rate
# 0.05
```
...
```bash
openraw config set default_model.provider anthropic
openraw config set default_model.model claude-sonnet-4-20250514
openraw config set api_listen "0.0.0.0:4200"
```

---

### openraw config set-key

Save an LLM provider API key to `~/.openraw/.env`.

```
openraw config set-key <PROVIDER>
```
...
- Saves to `~/.openraw/.env` as `<PROVIDER_NAME>_API_KEY=<value>`.
...
```bash
openraw config set-key groq
# Paste your groq API key: gsk_...
# [ok] Saved GROQ_API_KEY to ~/.openraw/.env
# Testing key... OK
```

---

### openraw config delete-key

Remove an API key from `~/.openraw/.env`.

```
openraw config delete-key <PROVIDER>
```
...
```bash
openraw config delete-key openai
```

---

### openraw config test-key

Test provider connectivity with the stored API key.

```
openraw config test-key <PROVIDER>
```
...
- Reads the API key from the environment (loaded from `~/.openraw/.env`).
...
- **Daemon mode:** Finds the agent by name or ID among running agents. If no agent name is given, uses the first available agent. If no agents exist, suggests `openraw agent new`.
...
openraw chat a1b2c3d4-e5f6-7890-abcd-ef1234567890
```
...
```
openraw migrate --from <FRAMEWORK> [--source-dir <PATH>] [--dry-run]
```
...
- Converts agent configurations, YAML manifests, and settings from the source framework into OpenRaw format.
- Saves imported data to `~/.openraw/`.
...
- Exposes running OpenRaw agents as MCP tools via JSON-RPC 2.0 over stdin/stdout with Content-Length framing.
- Each agent becomes a callable tool named `openraw_agent_<name>` (hyphens replaced with underscores).
...
1. **Read `daemon.json`:** On startup, the daemon writes `~/.openraw/daemon.json` containing the listen address (e.g. `127.0.0.1:4200`). The CLI reads this file to learn where the daemon is.
...
```
openraw start          # Starts daemon, writes daemon.json
                        # Other CLI instances detect daemon.json
openraw status         # Connects to daemon via HTTP
Ctrl+C                  # Daemon shuts down, daemon.json removed

openraw doctor --repair  # Cleans up stale daemon.json from crashes
```
...
OpenRaw loads `~/.openraw/.env` into the process environment on every CLI invocation. System environment variables take priority over `.env` values.
...
openraw agent spawn ./agents/custom-agent/agent.toml

# List running agents
openraw agent list

# Chat with an agent by UUID
openraw agent chat <UUID>

# Kill an agent
openraw agent kill <UUID>
```
...
```bash
# Create a workflow
openraw workflow create ./review-pipeline.json

# List workflows
openraw workflow list

# Run a workflow
openraw workflow run <WORKFLOW_ID> "Review the latest PR"
```
...
openraw trigger list --agent-id <AGENT_ID>
...
```bash
# Search FangHub
openraw skill search "code review"

# Install a skill
openraw skill install code-reviewer
...
# Remove a skill
openraw skill remove code-reviewer
```
...
openraw channel enable discord
openraw channel disable slack
```
...
openraw config set default_model.provider anthropic
openraw config set default_model.model claude-sonnet-4-20250514
openraw config set default_model.api_key_env ANTHROPIC_API_KEY

# Manage API keys
openraw config set-key anthropic
openraw config test-key anthropic
openraw config delete-key openai
...
openraw migrate --from langchain --source-dir ~/.langchain
```
...
openraw completion bash >> ~/.bashrc
openraw completion zsh > "${fpath[1]}/_openraw"
openraw completion fish > ~/.config/fish/completions/openraw.fish
```
...
The following providers are recognized by `openraw config set-key` and `openraw doctor`:
