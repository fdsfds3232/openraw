...
OpenRaw implements both the **Model Context Protocol (MCP)** and **Agent-to-Agent (A2A)** protocol, enabling deep interoperability with external tools, IDEs, and other agent frameworks.
...
- [Part 1: MCP (Model Context Protocol)](#part-1-mcp-model-context-protocol)
  ...
  - [MCP Server -- Exposing OpenRaw via MCP](#mcp-server)
...
The Model Context Protocol (MCP) is a JSON-RPC 2.0 based protocol that standardizes how LLM applications discover and invoke tools. OpenRaw supports MCP in both directions:

- **As a client**: OpenRaw connects to external MCP servers (GitHub, filesystem, databases, Puppeteer, etc.) and makes their tools available to all agents.
- **As a server**: OpenRaw exposes its own agents as MCP tools, so IDEs like Cursor, VS Code, and Claude Desktop can call OpenRaw agents directly.

OpenRaw implements MCP protocol version `2024-11-05`.

**Source files:**
- Client: `crates/openraw-runtime/src/mcp.rs`
- Server handler: `crates/openraw-runtime/src/mcp_server.rs`
- CLI server: `crates/openraw-cli/src/mcp.rs`
- Config types: `crates/openraw-types/src/config.rs` (`McpServerConfigEntry`, `McpTransportEntry`)

---

### MCP Client

The MCP client (`McpConnection` in `openraw-runtime`) allows OpenRaw to connect to any MCP-compatible server and use its tools as if they were built-in.
...
OpenRaw supports two MCP transports, defined by `McpTransport`:
...
Helper functions (exported from `openraw_runtime::mcp`):
...
OpenRaw can also act as an MCP server, exposing its agents as callable tools to external MCP clients.

#### How It Works

Each OpenRaw agent becomes an MCP tool named `openraw_agent_{name}` (with hyphens replaced by underscores). The tool accepts a single `message` string parameter and returns the agent's response.

For example, an agent named `code-reviewer` becomes the MCP tool `openraw_agent_code_reviewer`.

#### CLI: `openraw mcp`

The primary way to run the MCP server is the `openraw mcp` command, which starts a stdio-based MCP server:
...
This command:
1. Checks if an OpenRaw daemon is running (via `find_daemon()`)
...
- `McpBackend::Daemon` -- forwards requests to a running OpenRaw daemon via HTTP
- `McpBackend::InProcess` -- boots a full kernel when no daemon is available

#### HTTP MCP Endpoint

OpenRaw also exposes an MCP endpoint over HTTP at `POST /mcp`. Unlike the stdio server (which only exposes agents), the HTTP endpoint exposes the full tool set (built-in + skills + MCP tools) and executes tools via the kernel's `execute_tool()` pipeline. This means the HTTP MCP endpoint supports:
...
```json
{
  ...
  "result": {
    "protocolVersion": "2024-11-05",
    "capabilities": { "tools": {} },
    "serverInfo": { "name": "openraw", "version": "0.1.0" }
  }
}
```
...
```json
{
  ...
  "method": "tools/call",
  "params": {
    "name": "openraw_agent_code_reviewer",
    "arguments": {
...
After configuration, all OpenRaw agents appear as tools in the IDE. For example, you can ask Claude Desktop to "use the openraw code-reviewer agent to review this file."
...
OpenRaw implements A2A in both directions:
...
**Source files:**
- Protocol types and logic: `crates/openraw-runtime/src/a2a.rs`
- API routes: `crates/openraw-api/src/routes.rs`
- Config types: `crates/openraw-types/src/config.rs` (`A2aConfig`, `ExternalAgent`)
...
pub struct AgentCapabilities {
    pub streaming: bool,                 // true -- OpenRaw supports streaming
...
**AgentSkill** (not the same as OpenRaw skills -- these are A2A capability descriptors):

```rust
pub struct AgentSkill {
    pub id: String,           // matches the OpenRaw tool name
...
Agent Cards are built from OpenRaw agent manifests via `build_agent_card()`. Each tool in the agent's capability list becomes an A2A skill descriptor. Example card:
...
OpenRaw serves A2A requests through the REST API. The server-side implementation involves:
...
**External Agent Discovery**: The `A2aClient` uses a 30-second timeout and sends a `User-Agent: OpenRaw/0.1 A2A` header. Failed discoveries are logged but do not block kernel boot.
