# OpenRaw Security Architecture

This document provides a comprehensive technical reference for every security
system in the OpenRaw Agent Operating System.  All struct names, function
...
OpenRaw implements **defense-in-depth** security.  No single mechanism is
...
|---|--------|-------|------------------|
| 1 | Capability-Based Security | `openraw-types` | Unauthorized actions by agents |
| 2 | WASM Dual Metering | `openraw-runtime` | Infinite loops, CPU DoS |
| 3 | Merkle Audit Trail | `openraw-runtime` | Tampered audit logs |
| 4 | Taint Tracking | `openraw-types` | Prompt injection, data exfiltration |
| 5 | Ed25519 Manifest Signing | `openraw-types` | Supply chain attacks |
| 6 | SSRF Protection | `openraw-runtime` | Server-Side Request Forgery |
| 7 | Secret Zeroization | `openraw-runtime`, `openraw-channels` | Memory forensics, key leakage |
| 8 | OFP Mutual Auth | `openraw-wire` | Unauthorized peer connections |
| 9 | Security Headers | `openraw-api` | XSS, clickjacking, MIME sniffing |
| 10 | GCRA Rate Limiter | `openraw-api` | API abuse, denial of service |
| 11 | Path Traversal Prevention | `openraw-runtime` | Directory traversal attacks |
| 12 | Subprocess Sandbox | `openraw-runtime` | Secret leakage via child processes |
| 13 | Prompt Injection Scanner | `openraw-skills` | Malicious skill prompts |
| 14 | Loop Guard | `openraw-runtime` | Stuck agent tool loops |
| 15 | Session Repair | `openraw-runtime` | Corrupted LLM conversation history |
| 16 | Health Endpoint Redaction | `openraw-api` | Information leakage |

---

## 2. Capability-Based Security

**Source:** `openraw-types/src/capability.rs`

OpenRaw uses capability-based security.  An agent can only perform actions
...
**Source:** `openraw-runtime/src/sandbox.rs`
...
**Source:** `openraw-runtime/src/audit.rs`
...
**Source:** `openraw-types/src/taint.rs`

OpenRaw implements a lattice-based taint propagation model that prevents
...
**Source:** `openraw-types/src/manifest_signing.rs`
...
**Source:** `openraw-runtime/src/host_functions.rs`
...
OpenRaw uses `Zeroizing<String>` from the `zeroize` crate on every field
...
**LLM Drivers** (`openraw-runtime/src/drivers/`):
...
**Channel Adapters** (`openraw-channels/src/`):
...
**Web Search** (`openraw-runtime/src/web_search.rs`):
...
**Embedding** (`openraw-runtime/src/embedding.rs`):
...
**Source:** `openraw-wire/src/peer.rs`

The OpenRaw Wire Protocol (OFP) uses HMAC-SHA256 with nonce-based mutual
...
**Source:** `openraw-api/src/middleware.rs`
...
**Source:** `openraw-api/src/rate_limiter.rs`

OpenRaw uses the Generic Cell Rate Algorithm (GCRA) for cost-aware API
...
**Source:** `openraw-runtime/src/subprocess_sandbox.rs`
...
**Source:** `openraw-skills/src/verify.rs`
...
**Source:** `openraw-runtime/src/loop_guard.rs`
...
**Source:** `openraw-runtime/src/session_repair.rs`
...
**Source:** `openraw-api/src/routes.rs`

OpenRaw provides two health endpoints with different information levels.
