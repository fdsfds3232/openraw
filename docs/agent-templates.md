...
OpenRaw ships with **30 pre-built agent templates** organized into 4 performance tiers. Each template is a ready-to-spawn `agent.toml` manifest located in the `agents/` directory. Templates cover software engineering, business operations, personal productivity, and everyday tasks.
...
```bash
openraw spawn orchestrator
openraw spawn coder
openraw spawn --template agents/writer/agent.toml
```
...
```bash
openraw spawn security-auditor
...
> General-purpose assistant. The default OpenRaw agent for everyday tasks, questions, and conversations.
...
```bash
openraw spawn customer-support
...
```bash
openraw spawn meeting-assistant
...
```bash
openraw spawn personal-finance
...
openraw spawn coder --name "backend-coder"

# Spawn from a TOML file path
openraw spawn --template agents/custom/my-agent.toml

# List running agents
openraw agents

# Send a message
openraw message <agent-id> "Write a function to parse TOML files"

# Kill an agent
openraw kill <agent-id>
```
