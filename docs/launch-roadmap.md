...
1. **Add token estimation & context guard** (`crates/openraw-runtime/src/compactor.rs`)
...
2. **Add in-loop token guard** (`crates/openraw-runtime/src/agent_loop.rs`)
...
3. **Filter tools by profile in kernel** (`crates/openraw-kernel/src/kernel.rs`)
...
4. **Raise default token quota** (`crates/openraw-types/src/agent.rs`)
   - Change `max_llm_tokens_per_hour` from 100_000 to 1_000_000
   - 100K is too low — a single system prompt is 30-40K tokens

5. **Token-based compaction trigger** (`crates/openraw-kernel/src/kernel.rs`)
...
6. **Compact system prompt injections** (`crates/openraw-kernel/src/kernel.rs`)
...
**Problem:** Desktop app may show Tauri default icons. Branding assets exist at `~/Downloads/openraw/output/` but aren't installed.

**What to do:**

1. Generate all required icon sizes from source PNG (`openraw-logo-transparent.png`, 2000x2000)
2. Place into `crates/openraw-desktop/icons/`:
   - `icon.png` (1024x1024)
   - `icon.ico` (multi-size: 256, 128, 64, 48, 32, 16)
   - `32x32.png`
   - `128x128.png`
   - `128x128@2x.png` (256x256)
3. Replace web UI logo at `crates/openraw-api/static/logo.png`
4. Update favicon if one exists

**Assets available:**
- `openraw-logo-transparent.png` (328KB, 2000x2000) — primary source
- `openraw-logo-black-bg.png` (312KB) — for dark contexts
- `openraw-vector-transparent.svg` (293KB) — scalable vector
- `openraw-animated.svg` (310KB) — for loading screens

**Done when:**
- Desktop app shows OpenRaw logo in taskbar, title bar, and installer
...
**Status: COMPLETE** — Generated Ed25519 signing keypair via `cargo tauri signer generate --ci`. Public key installed in `tauri.conf.json`. Private key at `~/.tauri/openraw.key`. Set `TAURI_SIGNING_PRIVATE_KEY_PATH` in CI secrets.
...
These close the gaps that would make users pick OpenClaw over OpenRaw.
...
**Files:** `crates/openraw-api/static/js/pages/chat.js`, `crates/openraw-runtime/src/tool_runner.rs`
...
**Files:** `crates/openraw-api/static/js/pages/skills.js`, `crates/openraw-api/src/routes.rs`
...
**Problem:** `openraw.sh` domain isn't set up. Users can't do `curl -sSf https://openraw.sh | sh`.

**What to do:**
1. Set up GitHub Pages or Cloudflare Worker for openraw.sh
...
**Done when:**
- `curl -sSf https://openraw.sh | sh` installs the latest release
- `irm https://openraw.sh/install.ps1 | iex` works on Windows PowerShell
...
These are features where OpenRaw can leapfrog OpenClaw.
...
**Status: COMPLETE** — Created `sdk/javascript/` (@openraw/sdk) with full REST client: agent CRUD, streaming via SSE, sessions, workflows, skills, channels, memory KV, triggers, schedules + TypeScript declarations. Created `sdk/python/openraw_client.py` (zero-dependency stdlib urllib) with same coverage. Both include basic + streaming examples. Python `setup.py` for pip install.
...
1. Create `sdks/javascript/` — thin wrapper around REST API
   ...
   - Publish to npm as `@openraw/sdk`
2. Create `sdks/python/` — thin wrapper with httpx
   - Same operations
   - Publish to PyPI as `openraw`
3. Include usage examples in README

**Done when:**
- `npm install @openraw/sdk` works
- `pip install openraw` works
...
**Status: COMPLETE** — Added `GET /api/metrics` endpoint returning Prometheus text format. Metrics: `openraw_uptime_seconds`, `openraw_agents_active`, `openraw_agents_total`, `openraw_tokens_total{agent,provider,model}`, `openraw_tool_calls_total{agent}`, `openraw_panics_total`, `openraw_restarts_total`, `openraw_info{version}`.

**Problem (was):** No way to monitor OpenRaw in production (no Prometheus, no OpenTelemetry).

**What to do:**
1. Add `/api/metrics` endpoint with Prometheus format
   - `openraw_agents_active` gauge
   - `openraw_messages_total` counter (by agent, by channel)
   - `openraw_tokens_total` counter (by provider, by model)
   - `openraw_request_duration_seconds` histogram
   - `openraw_tool_calls_total` counter (by tool name)
   - `openraw_errors_total` counter (by type)
2. Optional: OTLP export for tracing spans

**Files:** `crates/openraw-api/src/routes.rs`, new `metrics.rs` module
...
**Problem (was):** Both OpenRaw and OpenClaw define workflows in TOML/config only. No visual builder exists in either. First to ship this wins.
...
**Files:** `crates/openraw-kernel/src/kernel.rs`, `routes.rs`, `ws.rs`, `index_body.html`
...
**What to do:**
1. Watch `~/.openraw/config.toml` for changes (notify crate)
2. On change: re-parse, diff, apply only changed sections
3. Log what was reloaded
4. UI notification: "Config reloaded"

**Files:** `crates/openraw-api/src/server.rs`, `crates/openraw-types/src/config.rs`
...
**Status: ALL CODE COMPLETE** — All 18 code items done. 1751 tests passing. Production audit completed: 2 critical bugs fixed (API delete alias, config/set route), CSP hardened (Tauri + middleware), Tauri signing key installed. Remaining for release: tag v0.1.0, build release artifacts, set up openraw.sh domain.
...
| Feature | OpenClaw | OpenRaw | Winner |
|---------|----------|----------|--------|
| Language/Performance | Node.js (~200MB) | Rust (~30MB single binary) | **OpenRaw** |
| Channels | ~15 | **40** | **OpenRaw** |
| Built-in Tools | ~19 | **41** | **OpenRaw** |
| Security Systems | Token + sandbox | **16 defense systems** | **OpenRaw** |
| Agent Templates | Manual config | **30 pre-configured** | **OpenRaw** |
| Hands (autonomous) | None | **7 packages** | **OpenRaw** |
| Workflow Engine | Cron + webhooks | **Full DAG with parallel/loops** | **OpenRaw** |
| Knowledge Graph | Flat vector store | **Entity-relation graph** | **OpenRaw** |
| P2P Networking | None | **OFP wire protocol** | **OpenRaw** |
| WASM Sandbox | Docker only | **Dual-metered WASM** | **OpenRaw** |
| Desktop App | Electron (~200MB) | **Tauri (~30MB)** | **OpenRaw** |
| Migration | N/A | **`migrate --from openclaw`** | **OpenRaw** |
| Skills | 54 bundled | **60 bundled** | **OpenRaw** |
| LLM Providers | ~15 | **27 providers, 130+ models** | **OpenRaw** |
...
| Visual Workflow Builder | None | **Drag-and-drop builder** | **OpenRaw** |

**OpenRaw wins 15/18 categories.** The remaining gaps are: mobile apps (OpenClaw), voice wake word (OpenClaw slight edge).
...
Sprint 2: 4/5 COMPLETE
  ...
  2.4 Install script domain ........ PENDING (infra: set up openraw.sh domain)
