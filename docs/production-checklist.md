...
cargo tauri signer generate -w ~/.tauri/openraw.key
```
...
Your private key was saved to: ~/.tauri/openraw.key
```
...
Open `crates/openraw-desktop/tauri.conf.json` and replace:
...
|---|---|---|
| `TAURI_SIGNING_PRIVATE_KEY` | Contents of `~/.tauri/openraw.key` | Yes |
...
The following icon files must exist in `crates/openraw-desktop/icons/`:
...
## 5. Set Up the `openraw.sh` Domain

**Status:** BLOCKING for install scripts — users run `curl -sSf https://openraw.sh | sh`.

Options:
- **GitHub Pages**: Point `openraw.sh` to a GitHub Pages site that redirects `/` to `scripts/install.sh` and `/install.ps1` to `scripts/install.ps1` from the repo's latest release.
- **Cloudflare Workers / Vercel**: Serve the install scripts with proper `Content-Type: text/plain` headers.
- **Raw GitHub redirect**: Use `openraw.sh` as a CNAME to `raw.githubusercontent.com/RightNow-AI/openraw/main/scripts/install.sh` (less reliable).

The install scripts reference:
- `https://openraw.sh` → serves `scripts/install.sh`
- `https://openraw.sh/install.ps1` → serves `scripts/install.ps1`

Until the domain is set up, users can install via:
```bash
curl -sSf https://raw.githubusercontent.com/RightNow-AI/openraw/main/scripts/install.sh | sh
```
...
```bash
docker build -t openraw:local .
docker run --rm openraw:local --version
docker run --rm -p 4200:4200 -v openraw-data:/data openraw:local start
```
...
grep '"version"' crates/openraw-desktop/tauri.conf.json
...
Visit: `https://github.com/RightNow-AI/openraw/releases/latest/download/latest.json`
...
docker pull ghcr.io/RightNow-AI/openraw:0.1.0

# Verify both architectures
docker run --rm ghcr.io/RightNow-AI/openraw:latest --version
```
...
4. Open the v0.1.0 app — after 10 seconds it should:
   - Show "OpenRaw Updating..." notification
...
openraw --version  # Should print v0.1.0
