...
OpenRaw is organized as a Cargo workspace with 14 crates (13 code crates + xtask). Dependencies flow downward (lower crates depend on nothing above them).

```
openraw-cli            CLI interface, daemon auto-detect, MCP server
    |
openraw-desktop        Tauri 2.0 desktop app (WebView + system tray)
    |
openraw-api            REST/WS/SSE API server (Axum 0.8), 76 endpoints
    |
openraw-kernel         Kernel: assembles all subsystems, workflow engine, RBAC, metering
    |
    +-- openraw-runtime    Agent loop, 3 LLM drivers, 23 tools, WASM sandbox, MCP, A2A
    +-- openraw-channels   40 channel adapters, bridge, formatter, rate limiter
    +-- openraw-wire       OFP peer-to-peer networking with HMAC-SHA256 auth
    +-- openraw-migrate    Migration engine (OpenClaw YAML->TOML)
    +-- openraw-skills     60 bundled skills, FangHub marketplace, ClawHub client
    |
openraw-memory         SQLite memory substrate, sessions, semantic search, usage tracking
    |
openraw-types          Shared types: Agent, Capability, Event, Memory, Message, Tool, Config,
...
When `OpenRawKernel::boot_with_config()` is called (either by the daemon or in-process by the CLI/desktop app), the following sequence executes:

```
1. Load configuration
   - Read ~/.openraw/config.toml (or specified path)
   - Apply #[serde(default)] defaults for missing fields
   - Validate config and log warnings (missing API keys, etc.)

2. Create data directory
   - Ensure ~/.openraw/data/ exists

3. Initialize memory substrate
   - Open SQLite database (openraw.db)
...
The memory substrate (`openraw-memory`) provides six layers of storage:
...
The `LlmDriver` trait (`openraw-runtime`) provides a unified interface for all LLM providers:
...
pub trait LlmDriver: Send + Sync {
    ...
    ) -> Result<LlmResponse, OpenRawError>;
...
The `ModelCatalog` (`openraw-runtime/src/model_catalog.rs`) provides a registry of all known models, providers, and aliases.
...
OpenRaw implements 16 security systems organized into critical fixes and state-of-the-art defenses:
...
`taint.rs` in `openraw-types` implements taint labels and taint sets. Data from external sources carries taint labels that propagate through operations, enabling information flow analysis.
...
The channel system (`openraw-channels`) provides 40 adapters for messaging platform integration.
...
The skill system (`openraw-skills`) provides 60 bundled skills and supports external skill installation.
...
- **FangHub**: Native OpenRaw marketplace (`FangHubClient`).
...
- **Tool Compat**: 21 OpenClaw-to-OpenRaw tool name mappings in `tool_compat.rs`.
...
OpenRaw implements both MCP client and server:
...
- **MCP Server** (`mcp_server.rs`): Exposes OpenRaw's 23 built-in tools via the MCP protocol. Enables external tools to use OpenRaw as a tool provider.
...
The OpenRaw Protocol (OFP) enables peer-to-peer agent communication across machines.
...
The desktop app (`openraw-desktop`) wraps the full OpenRaw stack in a native Tauri 2.0 application.
...
+-------------------------------------------------------------------+
    ...
    v                        v                   v         v
+------------------+  +--------------+  +--------+  +-----------+
| openraw-runtime |  | openraw-    |  | open-  |  | openraw- |
