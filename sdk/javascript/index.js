/**
 * @openraw/sdk — Official JavaScript client for the OpenRaw Agent OS REST API.
 *
 * Usage:
 *   const { OpenRaw } = require("@openraw/sdk");
 *   const client = new OpenRaw("http://localhost:3000");
 *
...
class OpenRawError extends Error {
  constructor(message, status, body) {
    super(message);
    this.name = "OpenRawError";
    this.status = status;
    this.body = body;
  }
}
...
class OpenRaw {
  /**
   * @param {string} baseUrl - OpenRaw server URL (e.g. "http://localhost:3000")
   * @param {object} [opts]
  ...
  async _request(method, path, body) {
    ...
    if (!res.ok) {
      ...
      throw new OpenRawError("HTTP " + res.status + ": " + text, res.status, text);
    }
...
class AgentResource {
  ...
  async upload(id, file, filename) {
    ...
    if (!res.ok) throw new OpenRawError("Upload failed: " + res.status, res.status);
    return res.json();
  }
...
module.exports = { OpenRaw: OpenRaw, OpenRawError: OpenRawError };
