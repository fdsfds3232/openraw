"""
OpenRaw Python Client — REST API client for controlling OpenRaw remotely.

Usage:

    from openraw_client import OpenRaw

    client = OpenRaw("http://localhost:3000")
...
Note: This is the REST API *client* library.
      For writing Python agents that run inside OpenRaw, see openraw_sdk.py instead.
"""
...
class _Resource:
    def __init__(self, client: "OpenRaw"):
        self._c = client


class OpenRaw:
    """OpenRaw REST API client. Zero dependencies — uses only stdlib urllib."""
    ...
    def _request(self, method: str, path: str, body: Any = None) -> Any:
        ...
        try:
            ...
            raise OpenRawError(f"HTTP {e.code}: {body_text}", e.code, body_text) from e
