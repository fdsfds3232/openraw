"""
OpenRaw Python SDK — helper library for writing Python agents.

Usage:

    from openraw_sdk import Agent
...
Or for simple scripts without the decorator pattern:
    ...
    from openraw_sdk import read_input, respond
...
def read_input() -> Dict[str, Any]:
    """Read the input JSON from stdin (sent by the OpenRaw kernel)."""
    line = sys.stdin.readline().strip()
    if not line:
        # Fallback: check environment variables
        agent_id = os.environ.get("OPENRAW_AGENT_ID", "")
        message = os.environ.get("OPENRAW_MESSAGE", "")
        return {
            "type": "message",
            "agent_id": agent_id,
            "message": message,
...
def respond(text: str, metadata: Optional[Dict[str, Any]] = None) -> None:
    """Send a response back to the OpenRaw kernel via stdout."""
...
def log(message: str, level: str = "info") -> None:
    """Log a message to stderr (visible in OpenRaw daemon logs)."""
...
if __name__ == "__main__":
    print("OpenRaw Python SDK")
    print("==================")
    print()
    print("Import this module in your agent scripts:")
    print()
    print("  from openraw_sdk import Agent")
    print()
    print("  agent = Agent()")
    print()
