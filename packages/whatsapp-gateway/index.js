...
const OPENRAW_URL = (process.env.OPENRAW_URL || 'http://127.0.0.1:4200').replace(/\/+$/, '');
const DEFAULT_AGENT = process.env.OPENRAW_DEFAULT_AGENT || 'assistant';
...
async function startConnection() {
  ...
  // Incoming messages → forward to OpenRaw
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Forward to OpenRaw agent
      try {
        const response = await forwardToOpenRaw(text, phone, pushName);
        if (response && sock) {
...
// ---------------------------------------------------------------------------
// Forward incoming message to OpenRaw API, return agent response
// ---------------------------------------------------------------------------
function forwardToOpenRaw(text, phone, pushName) {
  return new Promise((resolve, reject) => {
    ...
    const url = new URL(`${OPENRAW_URL}/api/agents/${encodeURIComponent(DEFAULT_AGENT)}/message`);
    ...
    req.on('timeout', () => {
      req.destroy();
      reject(new Error('OpenRaw API timeout'));
    });
    req.write(payload);
    req.end();
  });
...
// ---------------------------------------------------------------------------
// Send a message via Baileys (called by OpenRaw for outgoing)
// ---------------------------------------------------------------------------
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log(`[gateway] OpenRaw URL: ${OPENRAW_URL}`);
