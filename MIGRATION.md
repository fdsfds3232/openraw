# Migrating to OpenRaw

This guide covers migrating from OpenClaw (and other frameworks) to OpenRaw. The migration engine handles config conversion, agent import, memory transfer, channel re-configuration, and skill scanning.
...
This auto-detects your OpenClaw workspace at `~/.openclaw/` and imports everything into `~/.openraw/`.

### Options

```bash
# Specify a custom source directory
openraw migrate --from openclaw --source-dir /path/to/openclaw/workspace
...
After a successful migration, a `migration_report.md` file is saved to `~/.openraw/` with a summary of everything that was imported, skipped, or needs manual attention.
...
```bash
openraw migrate --from langchain   # Coming soon
openraw migrate --from autogpt     # Coming soon
```
...
| Item | Source (OpenClaw) | Destination (OpenRaw) | Status |
|------|-------------------|------------------------|--------|
| **Config** | `~/.openclaw/config.yaml` | `~/.openraw/config.toml` | Fully automated |
| **Agents** | `~/.openclaw/agents/*/agent.yaml` | `~/.openraw/agents/*/agent.toml` | Fully automated |
| **Memory** | `~/.openclaw/agents/*/MEMORY.md` | `~/.openraw/agents/*/imported_memory.md` | Fully automated |
| **Channels** | `~/.openclaw/messaging/*.yaml` | `~/.openraw/channels_import.toml` | Automated (manual merge) |
...
Channel configurations (Telegram, Discord, Slack) are exported to a `channels_import.toml` file. You must manually merge the `[channels]` section into your `~/.openraw/config.toml`.
...
```bash
openraw skill install <skill-name-or-path>
```

OpenRaw automatically detects OpenClaw-format skills and converts them during installation.
...
### 1. Initialize OpenRaw

```bash
openraw init
```

This creates `~/.openraw/` with a default `config.toml`.
...
**OpenRaw** (`~/.openraw/agents/coder/agent.toml`):
```toml
name = "coder"
version = "0.1.0"
description = "A coding assistant"
...
**OpenRaw** (add to `~/.openraw/config.toml`):
```toml
[channels.telegram]
bot_token_env = "TELEGRAM_BOT_TOKEN"
default_agent = "coder"
...
Copy any `MEMORY.md` files from OpenClaw agents to OpenRaw agent directories:

```bash
cp ~/.openclaw/agents/coder/MEMORY.md ~/.openraw/agents/coder/imported_memory.md
```
...
| Aspect | OpenClaw | OpenRaw |
|--------|----------|----------|
| Format | YAML | TOML |
| Config location | `~/.openclaw/config.yaml` | `~/.openraw/config.toml` |
...
Tools were renamed between OpenClaw and OpenRaw for consistency. The migration engine handles this automatically.

| OpenClaw Tool | OpenRaw Tool | Notes |
|---------------|---------------|-------|
...
| OpenClaw Profile | OpenRaw Tools |
|------------------|----------------|
...
| OpenClaw Name | OpenRaw Name | API Key Env Var |
|---------------|---------------|-----------------|
...
| **Networking** | None | OFP (OpenRaw Protocol) peer-to-peer |
...
```bash
openraw migrate --from openclaw --source-dir /path/to/your/workspace
```
...
```bash
openraw skill install /path/to/openclaw/skills/my-skill
```
...
```bash
cat ~/.openraw/channels_import.toml
# Copy the [channels.*] sections into ~/.openraw/config.toml
```
